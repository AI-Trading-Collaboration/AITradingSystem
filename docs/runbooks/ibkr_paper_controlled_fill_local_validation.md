...
- 默认必须在美股 regular trading session 运行。脚本会读取 IBKR contract details 的
  `tradingHours` / `liquidHours`，只有当前时间落在 `liquidHours` 内才提交订单。
...
在 TWS / Gateway Paper 窗口确认当前行情、账户、regular session 和 limit price 后，
人工运行：
...
## Normal Trading Hours 判断

controlled fill 的目标是采集可解释的小额 Paper fill 样本，因此默认只在
`market_session_status=REGULAR` 时提交订单。脚本连接 IBKR Paper 后、`submit_order`
前会读取 contract details：

- `tradingHours`：IBKR 认为该合约可接受或可路由交易的扩展时段。
- `liquidHours`：IBKR 标记的常规、流动性更可靠的 regular trading session。
- `timeZoneId` 或 local config 的 `exchange_timezone`：默认 `US/Eastern`。

报告中的判断方式：

- `REGULAR`：当前 exchange time 落在 `liquidHours` 内，可以提交 controlled fill。
- `OUTSIDE_RTH`：当前时间在 `tradingHours` 内但不在 `liquidHours` 内，默认阻断。
- `CLOSED`：`tradingHours` 显示 `CLOSED`，或当前时间不在当日交易窗口内，默认阻断。
- `UNKNOWN`：缺 contract details、缺 `tradingHours` / `liquidHours`、timezone 不可解析或
  IBKR hours 格式不可解析，默认阻断。

`CLOSED` / `OUTSIDE_RTH` / `UNKNOWN` 不适合 controlled fill，因为 Paper matching、
routing、订单排队和成交回报在这些状态下可能与 regular session 不同。此时即使
`BUY LIMIT` 高于 TWS / Gateway 显示的 ask，订单也可能停留在 `PreSubmitted`，不能把
no-fill 归因于 `PaperBroker` fill model。
...
- `exchange_timezone=US/Eastern` 或 contract details 返回的交易所 timezone
- `trading_hours_source=ibkr_contract_details`
- `liquid_hours_source=ibkr_contract_details`
- `controlled_fill_submission=submitted`
- `outside_rth_override=false`
- `session_guard_reason` 说明当前时间在 `liquidHours` 内

## No-Fill 的正常判定

非美股正常交易时段下，session guard 默认会在提交前阻断。如果历史报告或显式诊断
override 产生 no-fill，`BUY LIMIT DAY` 订单可能停留在 `PreSubmitted`。即使人工
...
- 若报告包含 `outside_regular_trading_hours`，必须同时把 fill interpretation 视为
  limited，不能归因于 `PaperBroker`
...
## Session Guard 阻断后的处理

如果报告显示：

- `controlled_fill_submission=blocked_by_session_guard`
- `market_session_status=CLOSED` / `OUTSIDE_RTH` / `UNKNOWN`
- issue 为 `market_session_closed` / `outside_regular_trading_hours` /
  `market_session_unknown`

则本次没有提交订单，`fill_seen=false` 不代表 broker fill failure，也不代表
`PaperBroker` fill model 有问题。处理方式：

- 保留 `BLOCK` 或更保守状态。
- 不修改 `PaperBroker` fill model。
- 不把该报告作为 fill-tested calibration evidence。
- 等到 `market_session_status=REGULAR` 后重新运行。

## `--allow-outside-rth-diagnostic` 使用条件

该参数只用于人工诊断已知 `OUTSIDE_RTH` 状态下 IBKR Paper 的订单生命周期表现。默认
不要使用。可接受场景：

- 已确认连接的是 TWS / Gateway Paper 和 `DUP` account。
- 目标是观察 submit/open/cancel 路径，不是验证 fill model。
- 接受报告 `test_status=LIMITED`，且 `outside_rth_override=true`、
  `fill_interpretation=limited`。
- 结果不会接入 daily-run、dashboard、paper signal quality、shadow impact 或任何
  production conclusion。

该参数不应用于 `CLOSED` 或 `UNKNOWN` 状态。若 guard 因缺 contract details 或 timezone
不可解析而阻断，先修复 IBKR contract details / 配置问题，而不是覆盖。
...
|`market_session_closed`|IBKR `tradingHours` 显示 CLOSED 或当前时间不在交易窗口|等待 regular session 后重跑；不解释为 fill failure|
|`outside_regular_trading_hours`|当前时间在 `tradingHours` 内但不在 `liquidHours` 内|默认等待 regular session；仅人工诊断 lifecycle 时可显式使用 `--allow-outside-rth-diagnostic`|
|`market_session_unknown`|缺 contract details、缺 hours 字段或 timezone / hours 解析失败|先修复 TWS / Gateway contract details 或 local `exchange_timezone`，不要提交订单|
