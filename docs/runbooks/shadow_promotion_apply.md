...
TRADING-018E3 已实现 explicit rollback command。018E3 需要单独 rollback approval、
单独 danger flag、current profile hash 与 apply 后状态一致、rollback-current snapshot
创建成功，并以 weights-only restore 原子恢复 target profile。

详见 `docs/runbooks/shadow_promotion_rollback.md`。
