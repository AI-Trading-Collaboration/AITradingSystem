...
|任务|名称|状态|验收重点|
|---|---|---|---|
|TRADING-281|Price / Rate Cache Refresh Executor|VALIDATING|`smoothed-source-refresh plan/execute/report` 和 validator 可运行；默认 dry-run，真实刷新必须显式 `--execute-refresh`，并记录 before/after/latest/checksum audit。|
|TRADING-282|Post-refresh Freshness Validation|VALIDATING|`smoothed-post-refresh-validate run/report` 重新运行 validate-data 等价门禁与 smoothed preflight，输出 `retry_decision`。|
|TRADING-283|Smoothed Retry Resume Runner|VALIDATING|`smoothed-retry-resume run/report` 只在 `RETRY_READY` 时 resume；未 ready 时 blocked，不更新 outcome。|
|TRADING-284|Forward Sample Growth Dashboard|VALIDATING|`smoothed-sample-growth build/report` 输出 before/after/delta 和 target-level progress。|
|TRADING-285|Owner Data Readiness Status Pack|VALIDATING|`smoothed-data-readiness pack/report` 给 owner 当前状态、推荐动作、progress 和 no broker/no production 边界。|
