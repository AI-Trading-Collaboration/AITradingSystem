...
### 阶段 6：daily-run 复盘报告前置

- 交易日 `ops daily-run` 在 `score_daily` 之后、`reports_dashboard` 之前生成同日
  `market_feedback_optimization_YYYY-MM-DD.md`、`feedback_loop_review_YYYY-MM-DD.md`
  和 `investment_weekly_review_YYYY-MM-DD.md`。
- 这些步骤只读读取已生成评分、outcome、因果链、学习队列、规则实验、参数 replay/candidate
  和任务登记，不改评分、仓位、回测、规则或生产结论。
- 休市日仍跳过 dashboard 和这些 dashboard 前置复盘报告，避免把没有新 daily_score
  的日期展示成完整投资结论。
- 若复盘报告生成失败，daily-run 应在运行报告中暴露失败，不再生成带 `NOT_CONNECTED`
  的新 dashboard 冒充完整展示；已完成的 `score_daily` 结论仍以日报和 trace 为准。

验收标准：

- `daily-plan` 显示三个复盘报告前置步骤，顺序在 `score_daily` 之后、`reports_dashboard`
  之前。
- `reports_dashboard` 在同一 daily-run 中读取刚生成的三份复盘报告，JSON 中
  `feedback_review.market_feedback.connected`、`feedback_review.feedback_loop.connected`
  和 `feedback_review.investment_review.connected` 均为 true。
- 真实 latest completed trading day `daily-run` 通过，run bundle 收录三份复盘报告、
  dashboard HTML/JSON、ops health 和 secret hygiene 报告。
...
- 2026-05-14：根据 owner 复核 2026-05-13 dashboard 结果回到 `IN_PROGRESS`。
  该 run 中 dashboard 能读取已有 `market_feedback_optimization`，但未生成同日
  `feedback_loop_review` 和 `investment_weekly_review`，导致中栏和右栏显示
  `NOT_CONNECTED`。本阶段目标是把三份复盘报告纳入交易日 `daily-run` 的
  dashboard 前置流程，并用真实 latest completed trading day 验证。
- 2026-05-14：阶段 6 进入 `VALIDATING`。`ops daily-plan` 交易日步骤扩展为
  `score_daily -> market_feedback_optimization -> feedback_loop_review ->
  investment_weekly_review -> reports_dashboard`，休市日继续跳过这些展示层步骤。
  `cli_direct` 已支持三个新增子命令，避免 daily-run 子进程回到 Typer 全局解析。
  真实 `aits ops daily-run` 默认 as-of 2026-05-13 已 14/14 PASS，run bundle
  `outputs/runs/daily/20260514T144059Z/as_of_2026-05-13__daily_ops_run_2026-05-13_20260514T144059Z`
  收录三份复盘报告；dashboard JSON 中 `market_feedback`、`feedback_loop` 和
  `investment_review` 均 `connected=true`。验证通过 `ruff check src tests`、
  目标测试 35 passed、全量 `pytest -q` 525 passed 和 `git diff --check`。
- 2026-06-07：收口为 `DONE`。最新真实
  `aits ops daily-run --as-of 2026-06-05 --run-id codex_20260605_20260607103901`
  中 `reports_dashboard` step 在 `score_daily` 后、Reader Brief 链前 PASS；
  canonical run bundle 收录 `evidence_dashboard_2026-06-05.html/json`，
  dashboard JSON 声明 `production_effect=none`，Reader Brief quality
  `failed=0`。后续若要改 dashboard/Reader Brief 职责边界，应新建报告任务。
