...
- 2026-05-11：run bundle 输出隔离修复进入实现。`daily-run` 的 canonical
  run bundle 默认路径是 `outputs/runs/<as_of>/<run_id>/reports/`，但复核发现
  休市日 `reports_dashboard` 被标记 `SKIPPED` 后，legacy mirror 仍可能把
