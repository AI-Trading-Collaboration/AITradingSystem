...
- 2026-06-07: 从 VALIDATING 改为 DONE。原因：`aits etf forward validate`
  复验仍为 PASS，输出 `production_effect=none`、`broker_action=none`、
  `manual_review_required=true`；本任务的 dashboard / weekly review / Reader
  Brief / watchlist / validation baseline 已满足验收，后续真实 forward 样本积累由
  影子观察任务继续承接。
