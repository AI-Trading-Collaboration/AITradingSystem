...
- 2026-05-05：从 BASELINE_DONE 改为 IN_PROGRESS。原因：owner 要求继续开发下一批五个任务，当前实现范围覆盖 PIT 字段强校验、challenger shadow runner、shadow 样本成熟度、rule card promotion/retirement 和 production vs challenger 周/月复盘。
- 2026-05-05：从 IN_PROGRESS 改为 BASELINE_DONE。原因：五个任务均已完成基础版实现：字段级 source availability 检查、`feedback run-shadow`、`feedback shadow-maturity`、`promote-rule-card` / `retire-rule-card`、以及周/月复盘和 feedback loop review 的 production vs challenger 接入；完整 DONE 仍依赖真实 shadow 样本、owner approval 和后续新增数据源持续补充字段级检查。
