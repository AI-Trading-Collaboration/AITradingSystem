...
6. 输出 coverage sufficiency：`FULL_WEEK_REVIEW`、`PARTIAL_ARTIFACT_WINDOW_REVIEW`、
   `RECOVERY_MODE_REVIEW` 或 `INSUFFICIENT_REVIEW`。
7. 明确 safety metadata：`production_effect=none`、`broker_effect=none`、
...
- 2026-06-15：实现完成并转 DONE；新增 `paper-shadow-weekly-review build/report`
  ...
  no production。
- 2026-06-16：TRADING-353A 扩展 coverage sufficiency schema；weekly review 现在显式披露
  selected window、expected/covered/missing market days、coverage ratio、coverage classification、
  coverage safe flag、coverage policy 和 manual override reason。一日 recovery artifact 不再被
  downstream 误读为 full weekly review；真实 recovery artifact
  `paper-shadow-weekly-review_67b1b8ae09e18fab` classified as `RECOVERY_MODE_REVIEW` /
