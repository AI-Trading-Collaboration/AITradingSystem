...
- 2026-07-12：ARCH-004G2.4BA对Replay-to-Forward Bridge执行source-derived hardening并迁移CLI。Run在output前要求Diagnosis/Comparison/Calibration三源技术校验PASS、完整lineage/time ordering，冻结三源bundle和reviewed bridge policy。AZ的`require_more_forward_data` evidence action不再被丢失或包装为proposal，无pending reason不再制造unknown；10-event/windows/focus/weekly questions进入policy。Validator重算全部views并检测source/snapshot/policy/output tamper；只写observation/review guidance，无policy/config/portfolio/production/broker effect。
