...
- 2026-07-12：ARCH-004G2.4AH接口迁移与TRADING-208 same-chain/as-of hardening完成。Weekly callbacks已有独立canonical owner；week-ending cutoff、owner-chain anchor、same-chain paper、owner inventory/source checksum、五态chain status及selection/count/summary/Markdown/Reader Brief重算均已落地。Focused 107、architecture-fitness 217通过；不同链、未来source和tampering均fail closed，Reader Brief不再以其他global latest补缺。
- 2026-07-12：ARCH-004G2.4AH开始迁移TRADING-208并修复weekly independent-latest拼接风险。Weekly source selection必须以`week_ending`为截止，优先选择截止时点最新owner review及其dry-run，并只匹配相同owner/dry-run的paper action；无owner时才使用最新dry-run。输出新增冻结source evidence、owner decision inventory与chain completeness status，validator必须复算source selection、counts、summary和rendered reports；不得用不同链或周末之后artifact形成周报。
