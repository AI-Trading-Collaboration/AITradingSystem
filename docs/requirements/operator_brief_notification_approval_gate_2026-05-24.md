...
- 2026-05-24：实现完成并进入 `VALIDATING`。新增核心 approval gate builder、CLI、
  ...
  未混入无关格式化 diff。
- 2026-06-09：从 `VALIDATING` 改为 `BASELINE_DONE`。本轮先读取 operations runbook 并运行
  `aits validate-data`，状态为 `PASS_WITH_WARNINGS` / 错误数 0；临时真实形态 fixture smoke
  覆盖 valid marker `APPROVED`、无 marker `APPROVAL_REQUIRED` 和缺 preview `BLOCKED`，
  均正常写出 JSON/Markdown/latest/run.log 审计 artifact；valid marker 场景确认
  `allowed_to_enter_dispatch=true` 仅作为后续任务审批证据，所有 email/Gmail/webhook/broker/replay/trading
  flags 为 false；目标 approval gate + dashboard pytest 37 passed，Ruff 和 scoped Black 通过。
  当前缺真实 `TRADING-032` dispatch preview artifacts 和真实人工 approval marker，剩余观察已登记为
  `TRADING-033A`。
