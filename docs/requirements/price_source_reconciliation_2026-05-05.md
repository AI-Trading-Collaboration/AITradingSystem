...
- 价格一致性默认只覆盖项目当前主要研究窗口 `unified_primary_2021`，即 `2021-02-22`
...
- 报告显示价格一致性和宏观变化检查窗口起点；当前默认起点为 `2021-02-22`。
- Marketstack 自身价格异常显示来源为“第二行情源 Marketstack”。
- 可证明的指数 volume 不适用、已知拆股复权跳变、Marketstack 自身坏点和 raw close 已核验的 adjusted close 分红复权口径差异默认显示为“信息”，并保留 row count、样例、来源、规则和证据。
- `outputs/reports/data_quality_YYYY-MM-DD_marketstack_reconciliation.csv` 逐行记录 Marketstack reconciliation 的 ticker/date、主/二源数值、分类、规则和证据。
...
- 2026-07-23：按项目统一 primary research window policy 修正当前口径。运行配置
  `config/data_quality.yaml` 已使用 `2021-02-22`；本需求的 active 决策和验收同步改为
  `unified_primary_2021`。下方 2026-05-05 的 `2022-12-01` 记录仅保留为当时 baseline
  的历史事实，不再供应当前默认值、required comparator 或 minimum allowed start。
...
- 2026-05-06：DATA-012 达到 `BASELINE_DONE`。新增
  `aits data-sources yahoo-price-diagnostic`，复用 `validate-data` 质量报告定位
  Marketstack 第二源 self-check 异常 `ticker/date`，只读拉取 Yahoo raw OHLC/close，
  输出 provider、endpoint、request parameters、row count、checksum、
  FMP/Marketstack/Yahoo 对比和 `diagnostic only / production_effect=none` 声明。
  该命令不写主价格缓存或第二源缓存，不进入评分、仓位闸门或回测真值；Yahoo 失败、
  空结果或与 FMP 不一致只进入调查报告。
- 2026-05-10：DATA-016 进入验证。`validate-data` 新增 INFO severity 和
  Marketstack reconciliation companion CSV；当前真实缓存 `validate-data --as-of 2026-05-10`
  为 PASS，错误 0、警告 0、信息 11。`^VIX` 缺 volume 按指数 volume 不适用记录；
  NVDA/SMH/SOXX 复权比例跳变匹配已配置拆股事件；Marketstack SPY/INTC/MSFT/TSM
  自身坏点进入 `marketstack.secondary_self_check.v1`；4771 条 adjusted close 差异
  因 raw close 对齐进入 `marketstack.adjusted_close_basis.v1`，未改写任何价格缓存。
- 2026-06-07：DATA-016 继续保持验证。最新真实
  `aits ops daily-run --as-of 2026-06-05 --run-id codex_20260605_20260607103901`
  的 `validate-data` 为 PASS、错误 0、警告 0；但本轮 companion reconciliation CSV
  只覆盖 QQQ/SMH/SOXX/SPY，未直接复现 NOW/full-universe
  `known_split_raw_close_basis_difference` 证据，因此不作为 DATA-016 归档样本。
- 2026-06-09：DATA-016 达到 DONE。复跑 2026-06-05 full-universe
  `download-data` / `validate-data` 后，正式 `data/raw` cache 覆盖 ASX，
  `outputs/reports/data_quality_2026-06-05.md` 为 PASS、错误 0、警告 0、信息
  12；companion CSV 直接记录 NOW 2025-12-17
  `known_split_raw_close_basis_difference`，并按 INFO 分类保留 ASX/Marketstack
  self-check 坏点和 adjusted close 分红复权口径差异。`validate-data`
  同步修正 download manifest provenance 判定，只对当前文件 checksum 的最新
  reconstructed 记录输出 warning，避免历史重建 manifest 行覆盖当前真实下载记录。
