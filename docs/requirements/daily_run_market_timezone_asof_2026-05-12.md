...
- 2026-05-12：实现完成，进入 `VALIDATING`。新增
  ...
  测试环境残余风险。
- 2026-06-09：继续保持 `VALIDATING`。默认 `daily-plan` smoke：
  `aits ops daily-plan --fail-on-missing-env --output-path
  outputs/tmp/ops_011_daily_plan_default_20260609.md` 生成 `READY` 计划，
  报告生成时间 `2026-06-08T17:04:32.336059+00:00`，默认评估日期为
  `2026-06-05`，市场日状态 `TRADING_DAY`，上一交易日 `2026-06-04`；
  计划内下载、验证和日报命令均使用 `--end 2026-06-05` / `--as-of
  2026-06-05`。该结果覆盖美股 2026-06-08 常规交易日 16:30 ET 前的默认
  解析语义，即选择上一完整交易日。尚未执行默认 `daily-run` 全链路 smoke，
  因此不归档为 `DONE`。
- 2026-06-09：从 `VALIDATING` 改为 `DONE`。无 `--as-of` 执行
  `aits ops daily-run --run-id ops-011-default-smoke-20260609`，返回 `PASS`，
  32/32 steps 通过；run bundle 为
  `outputs/runs/daily/20260608T174358Z/as_of_2026-06-05__ops-011-default-smoke-20260609`，
  `manifest.json` 与 metadata 均为 `PASS`、`failed_step=None`。执行报告显示
  默认评估日期 `2026-06-05`、市场日状态 `TRADING_DAY`、上一交易日
  `2026-06-04`，命令链使用 `--end 2026-06-05` / `--as-of 2026-06-05`。
  这覆盖了默认 `daily-run` 在美股 2026-06-08 常规交易日 16:30 ET 前选择
  最近完整交易日的生产 as-of 语义；所有步骤仍按 daily-run runbook 记录
  production effect、env presence、artifact checksum 和 no broker / no production
  mutation 边界。
