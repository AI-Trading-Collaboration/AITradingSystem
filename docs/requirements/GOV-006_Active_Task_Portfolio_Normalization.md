...
- priority：`P0`
- status：`IN_PROGRESS`
- current phase：`N1_FORMAL_COMPLETE_PUSHED_WAVE14_S0_ACTIVE`
...
8. **风险等级与执行排序分离。** `P0/P1/P2`继续表达长期风险；实际队列使用
   `critical-path gate -> actionability/status -> correctness/DQ risk -> unblock value -> waiting condition
   -> family rank`。等待自然样本、owner或外部输入的高风险任务保持可见，但不占用当前两个domain worker。
...
### N1：高置信 terminal reconciliation（FORMAL_COMPLETE）
...
- 形成“任务族 + 可执行状态 + 当前critical path”视图，避免把当前241个P0误当作同一调度队列。
...
- 2026-07-23：N1 coordinator review 已基于 refreshed manifest
  ...
  两份legacy Markdown且shadow replay byte-identical。application commit=
  `58ee6a80b5a04ff68a97a96d36b575ae8391f657`；commit-bound
  `gov_006_portfolio_normalization_applied_closeout.v1` 已生成并通过独立 validator，closeout id=
  `gov_006_applied_closeout_bdbfd433a72d5349ead9`，严格证明before/after Git lineage、30个原物理行空槽、
  非目标任务partition/priority/status不变及`production_effect=none`。append-only compatibility、
  initial focused=`124 passed`、architecture=`527 passed`、contract=`266 passed`、
  reproducibility=`23 passed`均PASS；final tracked-state focused=`194 passed`。自然Full保留
  `6843 passed / 1 failed / 3 skipped`父证据，修复归档任务仍被旧测试要求留在
  active register的问题后，failure-fix Full=`6844 passed / 3 skipped / 643 warnings / 1340.11s` PASS。
  N1因此转`FORMAL_COMPLETE`，但GOV-006 program仍为`IN_PROGRESS`。本次closeout提交推送后还必须从
  最终HEAD生成新的Wave14 exact manifests、ownership与readiness；只有Wave14 S0 contract/readiness
  PASS才允许dispatch D0B2+bounded G3，旧G2.5 rehearsal不可复用为授权。
- 2026-07-23：N1最终closeout commit
