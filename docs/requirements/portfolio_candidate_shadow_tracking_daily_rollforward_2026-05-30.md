...
- 2026-06-09: 从 VALIDATING 改为 DONE，原因：latest
  `portfolio_candidate_tracking/2026-06-08` 已进入 `active_tracking`，
  `portfolio validate-tracking --latest` 和 `reports portfolio-candidate-tracking
  --latest` 均可读取，安全字段仍为 `production_effect=none`、
  `manual_review_required=true`、`auto_promotion=false`，shadow backtest dry-run
  仍为 `promotion_status=rejected`；后续 performance review 已由 TRADING-058
