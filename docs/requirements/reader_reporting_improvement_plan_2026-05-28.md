...
|阶段|任务|状态|依赖|验收重点|
|---|---|---|---|---|
|1|`REPORT-048` Calculation Explainer Registry|DONE|`docs/calculation_logic.md`、现有 score / snapshot / trace 字段|关键 metric 有 formula、inputs、source artifacts、PIT policy、limitations；缺失解释可被测试捕获。|
...
- 2026-06-09：`REPORT-048` 从 `VALIDATING` 改为 `DONE`。latest 真实 artifact 复核通过：`aits reports calculation-explainers --as-of 2026-06-05` 生成 `calculation_explainers_2026-06-05.json`，状态 `PASS`、metrics=13、warnings=0、`production_effect=none`；字段级复核确认所有 metric 均有 formula、input fields、source artifacts、PIT policy、limitations 字段、common misread 和 metric-level `production_effect=none`，10 个当日指标为 `AVAILABLE`，`baseline_coverage`、`max_drawdown`、`rank_ic` 按设计为 `DEFINITION_ONLY`。Reader Brief source input 中 calculation explainers 为 `AVAILABLE` / `exists=True` / `production_effect=none`，HTML 保留 calculation explainer source 与 `<details>` 审计展开；focused tests 61 passed，Reader Brief quality、documentation contract 和 docs freshness 均通过。
