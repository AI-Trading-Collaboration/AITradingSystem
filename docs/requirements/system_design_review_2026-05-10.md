...
|6. CLI 分包路线|IN_PROGRESS|登记 `ARCH-001` 分阶段路线，并逐步迁移低耦合 Typer 子命令组|已迁移 `docs`、`security`、`scenarios`、`catalysts`、`execution`、`watchlist`、`industry-chain`、`trace`、`evidence`、`data-sources`、`thesis`、`valuation`、`pit-snapshots`、`llm`、ETF compatibility alias、`signals`、`fundamentals`、`portfolio`、`parameters`、主系统 `data`、`risk-events`、`ops`、`reports`、`feedback`、根级 `download-data` / `validate-data`、根级 utility `explain` / `score-example` 和根级 `review-trades` 命令；保持 `ai_trading_system.cli:app` 入口兼容，命令名、参数和退出码不变；后续继续评估其他低耦合命令组。|
...
- 2026-06-07：`ARCH-001` 第十九批低耦合命令组迁移完成，`aits risk-events list/validate/list-occurrences/record-review-attestation/import-occurrences-csv/fetch-official-sources/triage-official-candidates/precheck-triaged-official-candidates/apply-llm-formal-assessment/precheck-openai/import-prereview-csv/validate-occurrences` 迁入 `src/ai_trading_system/cli_commands/risk_events.py`；共享风险事件 profile/path helper 抽入 `src/ai_trading_system/cli_commands/risk_event_artifacts.py` 并供命令模块和 `score-daily` 共用；官方来源抓取、OpenAI 预审、LLM formal、评分、回测和日报投资解释语义保持兼容。
- 2026-06-07：`ARCH-001` 第二十批低耦合命令组迁移完成，已先读取 `docs/operations/operations_runbook.md`；`aits ops health/daily-plan/daily-run/replay-day/replay-window` 迁入 `src/ai_trading_system/cli_commands/ops.py`，`cli_direct` 的 `ops health` 改为调用新模块；每日 scheduler 统一入口、cadence、调度顺序、质量门禁、canonical run bundle、历史 replay 和 production effect 边界保持兼容。
- 2026-06-07：`ARCH-001` 第二十一批低耦合命令组迁移完成，`aits reports` 命令组迁入 `src/ai_trading_system/cli_commands/reports.py`，`cli_direct` 的报告 alias 改为调用 `reports_cli`；验证通过 reports help smoke、direct/scheduler/ops focused tests、Reader Brief/dashboard/report focused tests 和 alias writer focused tests；报告生成逻辑、Reader Brief、dashboard、daily task dashboard、报告 registry、评分、回测和数据质量语义保持兼容。
- 2026-06-07：`ARCH-001` 第二十二批低耦合命令组迁移完成，`aits feedback` 命令组迁入 `src/ai_trading_system/cli_commands/feedback.py`，`cli_direct` 的 feedback 调用改为调用 `feedback_cli`；验证通过 feedback help smoke、direct feedback focused test、calibration/outcome focused tests 27 passed、shadow/causal/learning focused tests 30 passed、governance/parameter/market feedback focused tests 36 passed；校准、shadow、learning queue、rule governance、benchmark policy、parameter governance、market feedback、评分、回测和数据质量语义保持兼容。
- 2026-06-07：`ARCH-001` 第二十三批低耦合命令迁移完成，根级 `aits download-data` / `aits validate-data` 迁入 `src/ai_trading_system/cli_commands/data_cache.py`，`cli_direct` 的根级数据缓存调用改为调用 `data_cache_cli`；验证通过 root / download-data / validate-data help smoke、direct/scheduler/ops focused tests 54 passed、data download/quality focused tests 30 passed 和 Yahoo price diagnostic focused tests 3 passed；数据下载 provider/cache/diagnostic、数据质量门禁、报告输出、daily-run、ETF alias `data ingest/validate`、主系统 `aits data ...` 子命令、评分、回测和日报投资解释语义保持兼容。
- 2026-06-07：`ARCH-001` 第二十四批低耦合命令迁移完成，根级只读解释入口 `aits explain` 和示例仓位建议 `aits score-example` 迁入 `src/ai_trading_system/cli_commands/root_utils.py`；验证通过 explain / score-example help smoke、score-example command smoke 和 explain focused tests 5 passed；字段/产物/gate 解释、示例 scoring render、数据质量门禁、生产评分、回测和日报投资解释语义保持兼容。
- 2026-06-07：`ARCH-001` 第二十五批低耦合命令迁移完成，根级 `aits review-trades` 迁入 `src/ai_trading_system/cli_commands/trade_review.py`；验证通过 review-trades help smoke 和 trade review focused tests 6 passed；交易记录校验、数据质量门禁、benchmark attribution、报告输出、评分、回测和日报投资解释语义保持兼容。
