...
|9B. Shadow weight profile 长期观察|DONE|维护若干套隔离测试权重参数，初始值参考生产 `weight_profile_current.yaml`，每日从 production decision snapshot 计算 shadow score / model band / gated band，并可选写入隔离 prediction ledger 进入 outcome 观察；不得替换生产权重。|
|9C. 当前样本 validation 门槛|DONE|把 diagnostic/validation 门槛放宽到当前已积累 outcome 样本可启动后续验证，但 production promotion floor 不降低；shadow maturity 区分 validation review 和 promotion/governance review，低样本 validation 只能进入 `READY_FOR_VALIDATION_REVIEW`。|
...
- 2026-06-09：`CALIBRATION-006` 从 `VALIDATING` 归档为 `DONE`。复核确认当前默认路径 `outputs/current_context.json` 与 `outputs/current_effective_weights.json` 已由 2026-06-08 `score_daily` 输出写入；current context 披露 `market_regime_id=ai_after_chatgpt`、`data_quality_status=PASS`、`feature_status=PASS`，current effective weights 披露 `weight_profile_version=2026-05-06`、`matched_overlays=[]`、`No approved overlays matched`，且 base/effective weights 相等。该任务完成的是审计产物接线，不批准 overlay、不改变 production scoring、position gate、日报结论或回测仓位。验证通过 `tests/test_weight_calibration.py`、`tests/test_daily_scoring.py` 和 `tests/test_market_feedback_optimization.py` 共 44 passed，以及 Ruff、repo-wide Black check 和 `compileall`。
...
- 2026-06-09：`CALIBRATION-007` 从 `VALIDATING` 归档为 `DONE`。复核确认 `parameter_candidates_flow_validation_2026-05-14.md` 为 `PASS_WITH_LIMITATIONS` / `production_effect=none`，`evaluation_mode=flow_validation`，16 个 candidate 以 `flow_validation_override` 进入 `READY_FOR_FORWARD_SHADOW` 但严格 veto reasons 仍保留；`parameter_governance_flow_validation_2026-05-14.md` 只输出 `PREPARE_FORWARD_SHADOW` / `COLLECT_MORE_EVIDENCE`，并明确不可进入 owner approval 或 production；`prediction_ledger_flow_validation.csv`、`prediction_outcomes_flow_validation.csv` 和 `shadow_maturity_flow_validation_2026-05-14.md` 跑通 validation-only shadow 接线。生产 scoring、position gate、approved overlay、正式 `prediction_ledger.csv`、日报结论和回测仓位未改变；阶段 8 的 broader promotion 连接仍保持 `VALIDATING`。验证通过 `tests/test_parameter_candidates.py`、`tests/test_parameter_governance.py`、`tests/test_prediction_ledger.py` 和 `tests/test_market_feedback_optimization.py` 共 22 passed，以及 Ruff、repo-wide Black check 和 `compileall`。
...
- 2026-06-09：`CALIBRATION-008` 从 `VALIDATING` 归档为 `DONE`。复核确认 `config/weights/shadow_weight_profiles.yaml` 仍声明 `production_effect=none` 和 `isolation=shadow_only`，三套 profile 来源任务均为 `CALIBRATION-008`；`shadow_weight_profiles_2026-05-14.md` 为 PASS / `production_effect=none`，报告披露主线评分、shadow score delta、model band、gated band、observation ledger 和“不修改生产 `weight_profile_current.yaml`、approved overlay、正式 prediction ledger、日报结论或仓位 gate”的边界；`shadow_weight_profile_observations.csv` 已记录 profile、score delta、模型/最终仓位、gate cap source 和 source snapshot。后续表现优选、gate profile、搜索、maturity validation 和 promotion contract 已由后续任务承接，因此长期观察框架不再阻塞 9B 完成。验证通过 `tests/test_shadow_weight_profiles.py` 15 passed、Ruff、repo-wide Black check 和 `compileall`。
...
- 2026-06-09：`CALIBRATION-009` 从 `VALIDATING` 归档为 `DONE`。复核确认 `feedback_sample_policy_v2` 仍把 decision/prediction diagnostic floor 设为 14/13，promotion floor 保持 60/30；`shadow_weight_profile_maturity_validation_2026-05-14.md` 为 PASS / `production_effect=none`，所有 profile/horizon 行只进入 `READY_FOR_VALIDATION_REVIEW`，没有写成 `READY_FOR_GOV_REVIEW`、owner approval 或 production promotion。生产 prediction ledger、approved overlay、日报结论和仓位 gate 未改变；后续真实滚动样本观察由长期 shadow/promotion 治理任务承接。验证通过 `tests/test_prediction_ledger.py`、`tests/test_market_feedback_optimization.py`、`tests/test_decision_outcomes.py` 和 `tests/test_feedback_loop_review.py` 共 13 passed，以及 Ruff、repo-wide Black check 和 `compileall`。
...
- 2026-06-09：`CALIBRATION-011` 从 `VALIDATING` 归档为 `DONE`。当前
  `config/weights/shadow_position_gate_profiles.yaml` 保留三套 validation-only gate
  profile，均声明 `production_effect=none` 与 `isolation=shadow_only`；真实
  `shadow_weight_performance_2026-05-14.md/csv` 仍为 `PASS` / `production_effect=none`，
  报告披露 observation ledger、profile 组合、收益、回撤、换手、beat rate 和“不修改生产
  `weight_profile_current.yaml`、approved overlay、正式 prediction ledger、日报结论或仓位
  gate”的边界。Observation ledger 记录 `gate_profile_id`、`gate_cap_overrides_json`、
  `gate_cap_sources`、production/shadow model target position 和 gated target position；
  return-leading profile 仍为 `shadow_alpha_tilt_v1__shadow_gate_relaxed_valuation_v1`，
  available=8、pending=1、missing=2，shadow total return 8.31%、production total
