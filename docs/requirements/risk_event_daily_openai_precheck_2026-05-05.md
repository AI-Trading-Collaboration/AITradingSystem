...
- 2026-05-05：基础版实现完成。`score-daily` 会在风险事件发生记录校验前抓取官方来源并调用 OpenAI metadata-only 风险事件预审；输出仅写 `llm_extracted / pending_review` 队列和报告，`irrelevant` 结果不增加人工队列；默认最多预审 20 条官方候选，可通过 `--risk-event-openai-precheck-max-candidates` 调整。验证：`ruff check src tests` 通过，`pytest -q` 362 passed。
...
- OpenAI 预审不会自动生成有效复核声明；要让 `policy_geopolitics` 从 `insufficient_data` 变为 `manual_input`，仍需要 owner 真实复核后运行 `aits risk-events record-review-attestation`，或导入人工确认后的 occurrence CSV。
