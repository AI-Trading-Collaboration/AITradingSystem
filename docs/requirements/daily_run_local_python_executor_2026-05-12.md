...
- 执行报告继续展示用户理解的逻辑命令 `aits ...`，主报告和 metadata 不记录 secret
  或未脱敏 stdout/stderr 原文。
- 子命令失败时写入单独的脱敏 stdout/stderr 诊断 artifact，并在执行报告和 metadata
  暴露路径，避免只剩 return code 和行数而无法排查 traceback。
...
- `aits ops daily-run` 默认最新完整交易日能越过 PIT 和 SEC companyfacts；若后续步骤失败，
  报告新的真实门禁原因。
- 子命令失败时，执行报告的阻断步骤列出诊断 artifact 路径；metadata 的对应
  `step_results` 记录同一路径；诊断 artifact 包含命令、退出码、stdout/stderr 行数和
  脱敏输出，不包含 API key、token、Authorization header 或未脱敏 secret 值。
...
- 2026-05-17：owner 明确要求继续推进后续 Python 版本切换工作。当前决策是把项目本地
  主运行环境标准化为 Python 3.11，与 GitHub Actions、Ruff/Black/Mypy target version
  对齐；裸 `python` 可保留其他版本，但项目命令、验证和 IBKR Paper read-only snapshot
  应优先通过 `.venv` 的 Python 3.11 运行。下一步补齐 `.venv` 依赖并用 3.11 跑
  trading_engine、全量 pytest、ruff、black 和 IBKR snapshot 验证。
- 2026-05-17：本地 Python 3.11 环境切换完成并进入验证。新增 `.python-version=3.11`，
  `pyproject.toml` 新增 `brokers` optional dependency group（`ib-insync`），README
  改为 `py -3.11 -m venv .venv` 和 `.[dev,data,dashboard,brokers]` 安装路径；
  本机 `.venv` 已补齐依赖，确认 `.venv\Scripts\python.exe` 为 Python 3.11.4。
  验证通过 `.venv\Scripts\python.exe scripts\run_ibkr_paper_readonly_snapshot.py --date
  2026-05-17 --config config\ibkr_paper_readonly.local.yaml`（CONNECTED/PASS）、
  `.venv\Scripts\python.exe -m pytest tests\trading_engine`、全量 pytest、ruff 和
  black check。真实 daily-run 仍作为后续生产链路观察项。
- 2026-05-28：真实 daily-run 连续两天在 `valuation_snapshots` 出现短暂
  `return_code=1`，但旧执行器只记录 stderr 行数，原始 traceback 不可恢复。同一
  valuation 子命令随后可通过，说明需要保留失败时的脱敏 stdout/stderr 诊断 artifact，
  以便区分供应商/API、数据 schema、Windows 子进程运行时或工作区状态问题。
- 2026-06-07：收口为 `DONE`。原因：已先按 periodic operations runbook 确认生产边界，
  随后执行最新完整美股交易日 `aits ops daily-plan --as-of 2026-06-05
  --fail-on-missing-env` 和真实 `aits ops daily-run --as-of 2026-06-05 --run-id
  codex_20260605_20260607103901`；daily-run 31/31 steps PASS，执行报告披露项目
  `.venv` direct dispatcher、隔离 pycache 和脱敏诊断边界，所有子命令无 stderr、无
  Windows native crash。验证还包括同一轮 `.venv\Scripts\python.exe -m pytest tests -q`
  全量测试通过（2197 passed, 329 warnings）。
