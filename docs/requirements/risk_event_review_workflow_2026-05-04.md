...
- 2026-05-10：owner 重新确认 paid vendor 文本可在个人研究、非商用目的下进入 OpenAI 预审；该确认不等于允许未知 provider 或未登记来源直接发送全文。实际运行仍必须在 `config/data_sources.yaml` 或请求 envelope 中记录 provider、授权范围、`external_llm_allowed=true`、`content_sent_level`、`cache_allowed`、`redistribution_allowed`、`approval_ref` 和 `reviewed_at`，未知授权继续 fail closed。
