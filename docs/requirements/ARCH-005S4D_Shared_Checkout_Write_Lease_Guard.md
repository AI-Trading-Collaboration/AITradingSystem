...
本需求只登记风险和建议顺序，不在 Wave14 中扩大实现范围。Wave14 S2 formal exit 已完成；窄版S0/S1
仍须 owner 明确从 `PROPOSED` 授权为可执行，并置于 Wave15 两个domain worker启动之前；
ARCH-005 S5仍需独立owner授权。

2026-07-24：dependency evidence 已闭合：Wave14 C7 replacement Full=`7007 passed / 4 skipped`
且post-Full evidence-only gates PASS。该结果只把依赖从“等待Wave14”改为“等待owner授权”；
`status=PROPOSED`、`next_slice_unblocked=false`、`production_effect=none`保持不变。
