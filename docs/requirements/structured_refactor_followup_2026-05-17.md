...
- 2026-06-09：`ARCH-003` 从 `VALIDATING` 归档为 `DONE`。复核确认五个基础切片仍保持兼容：`core/workflow.py` / `ops_daily.py` 承载 daily-run workflow contract 适配，`daily_task_dashboard.py` 保留 view-model/render 边界，`cli_commands/docs.py` 承载 `aits docs` 命令组，`shadow/lineage.py` 被 shadow/search/trading-engine helper 复用，`core/production_effect.py` 与低风险调用点统一 production effect 表达。验证通过 `tests/test_core_contracts.py`、`tests/test_ops_daily.py`、`tests/test_daily_task_dashboard.py`、`tests/test_cli_direct.py`、`tests/test_docs_freshness.py`、`tests/test_documentation_contract.py`、`tests/test_shadow_weight_profiles.py` 和 `tests/test_prediction_ledger.py` 共 107 passed，Ruff、repo-wide Black check、`compileall` 和目标 mypy 均通过；未改变 production scoring、position gate、approved overlay、正式 prediction ledger、backtest 语义或 shadow promotion 结论。
