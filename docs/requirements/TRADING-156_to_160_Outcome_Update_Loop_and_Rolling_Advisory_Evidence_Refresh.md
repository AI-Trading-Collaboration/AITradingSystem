...
6. weekly forward outcome decision pack 是人工下一步建议包；它只消费cutoff-bound、validated且lineage一致的Outcome Update→Rolling Refresh→Evidence Trend chain，并冻结full source/policy snapshot。缺source与invalid必须分离，missing metrics保持null；样本、confidence、risk或trend未通过reviewed policy时只能路由`continue_tracking` / `wait_for_more_outcomes` / `manual_review` / `do_not_change_policy`，即使READY也只允许`review_rule_calibration_evidence`且`do_not_auto_apply_policy`。
...
- 2026-07-12：ARCH-004G2.4BK 对 TRADING-160 Forward Outcome Decision 完成实现与验证。3 callback迁canonical；建立generated/week cutoff，支持显式id或semantic-latest zero-or-one选择，selected Update/Refresh/Trend必须full PASS并冻结bundle/validation，且Update→Refresh→Trend lineage一致。Missing与invalid分离，full-Dashboard metrics保持null；readiness、trend/risk routing、recommendation和cadence由reviewed policy治理。Validator重验live selection/source/policy并重算全部views；focused 365、architecture 246、contract 203 PASS。只生成owner support，不运行上游、不改policy/config/portfolio/production/broker。
- 2026-07-12：ARCH-004G2.4BK 对 TRADING-160 Forward Outcome Decision 完成contract freeze并进入实现：以generated/week cutoff选择zero-or-one validated Outcome Update、Rolling Refresh与Evidence Trend，冻结full bundles/validation/policy并校验update→refresh→trend lineage；missing与invalid分离，counts/metrics保持null/MISSING。Readiness、trend/risk manual review、recommended action precedence和next due cadence由reviewed policy治理；validator重验live source/policy并重算全部views。只生成owner decision support，不运行上游、不改policy/config/portfolio/production/broker。
