...
- 最终只有临时隔离 5/9、5/10 valuation YAML，并把 PIT manifest 限制到 5/8
  可见窗口后，缓存回放才通过。
- 2026-05-10 后续真实最近交易日回放又暴露一个剩余缺口：`replay-day`
  已传入隔离后的 valuation 路径，但没有传入隔离后的 `risk_event_occurrences`
  路径，导致 `score-daily` 仍读取生产目录中 5/10 的 LLM formal occurrence
  和 attestation，并触发未来日期错误。
- 风险事件路径修复后，2026-05-08 回放继续推进到 `pipeline_health`，
  又暴露 production `visibility_cutoff` 的边界：该 5/8 production metadata
  是 5/10 补跑生成，若直接用 5/10 cutoff 过滤 PIT，会纳入 5/10
  `available_time`，并被 health 按 5/8 as-of 正确判为未来输入。
- 将 PIT filtering cutoff 收紧到 as-of 日末后，当前生产
  `fmp_forward_pit_2026-05-08.csv` 又暴露出被 5/10 补跑覆盖的问题：
  该 CSV 内所有 normalized 行的 `available_time` 都是 5/10，但 filtered
  PIT manifest 仍引用 5/8 可见 raw payload。因此 replay 需要能从过滤后的
  raw manifest 重建隔离 normalized CSV。
...
|7. 风险事件 occurrence PIT 冻结修复|DONE|补齐 `risk_event_occurrences` 的 replay-scoped input view，并让 `score-daily` 读取隔离路径|2026-05-08 cache-only replay 不再读取生产 5/10 occurrence/attestation；manifest 记录纳入/排除数量和 checksum；生产 occurrence YAML 不被改写。|
|8. PIT 输入 cutoff as-of 收紧|DONE|当 production metadata cutoff 晚于 as-of 时，PIT manifest 和 normalized CSV 仍按 as-of 日末过滤；生产 normalized CSV 仅有未来 `available_time` 时，从 filtered raw manifest 重建 replay normalized|2026-05-08 cache-only replay 的 `ops health` 不再因 5/10 `available_time` 失败；manifest 记录 PIT 行排除数量；重建不调用外部服务、不读取旧 replay 输出。|
...
- 风险事件 occurrence store 必须与 valuation snapshots 一样写入隔离 input view；
  不能通过移动或删除生产 `data/external/risk_event_occurrences` 来完成历史回放。
- PIT 输入的 filtering cutoff 必须不晚于 as-of 日末；production metadata cutoff
  可用于记录回放可见性来源和普通 artifact 审计，但不能让 PIT `available_time`
  超过被回放交易日。
- 当生产 normalized CSV 已被后续补跑覆盖、无法提供 as-of 可见行时，
  replay 可从 filtered PIT manifest 引用的本地 raw payload 重新 normalize；
  这不是数据回填，因为 raw payload 的 `available_time` 仍来自原始 manifest。
...
- 2026-05-10：剩余重点任务完成。`daily_ops_run_metadata_YYYY-MM-DD.json`
  ...
  live OpenAI。
- 2026-05-10：登记 `OPS-009` 并开始修复。原因：真实最近交易日
  `replay-day --as-of 2026-05-08 --openai-replay-policy cache-only`
  验证发现 `score-daily` 仍读取生产 `risk_event_occurrences`，被 2026-05-10
  LLM formal occurrence/attestation 的未来日期门禁阻断；修复范围限定为
  replay input freeze、score command path override、测试和系统流图更新。
- 2026-05-10：登记 `OPS-010` 并开始修复。原因：`OPS-009` 修复后
  2026-05-08 replay 的 `score_daily` 已通过，但 `pipeline_health` 被
  5/10 `available_time` 的 PIT manifest/normalized 行阻断；production
  metadata cutoff 晚于 as-of 时，PIT 输入窗口必须进一步收紧到 as-of 日末。
- 2026-05-10：扩大 `OPS-010` 修复范围。原因：PIT 输入窗口收紧后，
  当前生产 normalized CSV 已无 as-of 可见行，但 filtered manifest 仍有
  5/8 raw payload；实现应从 filtered raw manifest 重建隔离 normalized CSV，
  而不是复用旧 replay 产物或移动生产缓存。
- 2026-05-10：`OPS-009/OPS-010` 完成。`replay-day` 现在会过滤并隔离
  `risk_event_occurrences`，把 `score-daily --risk-event-occurrences-path`
  指向 replay bundle；PIT manifest/normalized 使用不晚于 as-of 日末的
  effective input cutoff；当生产 normalized CSV 已被补跑覆盖为未来
  `available_time` 时，从 filtered raw manifest 按 ticker 选择 as-of 当天
  最新 raw payload 重建 replay normalized。真实验证
  `replay-day --as-of 2026-05-08 --mode cache-only --compare-to-production
  --openai-replay-policy cache-only` 通过，`score_daily`、`ops health`、
  `secret_hygiene` 均 PASS。
