...
- metadata声明为`COMPLETE`但文件只能证明`PARTIAL`时，validation必须FAIL且report显示observed `PARTIAL`。
- metadata同时伪造`detail_level=complete`和空`missing_fields`时，若逐行pre/post constraint/rescue权重不存在或不可解析，必须降为`INCOMPLETE`并FAIL。
- daily weights必须通过日期、唯一键、candidate id、数值边界和逐日归一校验；metadata count/start/end/symbol必须与CSV一致。
- evaluation id命中多个目录、event JSON schema/id错配或turnover为负时必须fail closed。
...
- 2026-07-11：ARCH-004G2.4Q实现shared只读evidence inspection：逐文件重新推导observed completeness，校验唯一evaluation目录、daily schema/date/id/duplicate/finite/bounds/per-date sum、metadata count/range/symbol/flags parity、event JSON schema/id及turnover schema/id/nonnegative；minimal有效path保持`PARTIAL`且可PASS，core无效降为`INCOMPLETE`，伪造`COMPLETE`和declared/observed不一致均FAIL。`weight-path validate/report`两callback迁canonical owner；仍不补造缺失中间权重、不运行候选、不promotion、不产生production/broker effect。
- 2026-07-11：ARCH-004G2.4Q复核发现TRADING-112 exporter虽诚实输出minimal/`PARTIAL`，但`validate-weight-path`和`weight-path report`仍过度信任metadata声明，不能证明row count、date range、symbol count、candidate/evaluation id、唯一键、权重边界/归一、event及turnover内容一致。TRADING-112继续保持`VALIDATING`；补充验收为shared read-only content inspection重新推导observed completeness，metadata声明不等于证据，content invalid必须`INCOMPLETE`/validation FAIL，minimal valid path保持`PARTIAL`，只有完整detail且无missing fields才允许`COMPLETE`。
