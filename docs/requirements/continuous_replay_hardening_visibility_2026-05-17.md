...
- 2026-06-09：复核归档。顺序运行两种 replay smoke，`daily-independent`
  输出 `portfolio_carry_forward=false`、`continuous_metrics_available=false`
  且没有 `equity_curve` / `daily_equity`；`continuous-portfolio` 输出
  `portfolio_carry_forward=true`、`continuous_metrics_available=true`、
  `final_equity=100000.0`、`final_cash=100000.0`、17 条 equity/cash/exposure
  序列、17 条 portfolio snapshot、`order_expiration_policy`、`unsupported_order_policy`、
  `max_drawdown_pct=0`、`exposure_peak=0` 和 `position_concentration_peak=0`。
  Markdown 明确 paper-only、不是实盘交易、不是真实账户收益、不是真实 broker
  成交、不读取 broker API key，并区分 `daily-independent` 与
  `continuous-portfolio`。当前 latest dashboard `2026-06-05` 因 run-bundle
  内没有 replay JSON 仍按设计显示 `daily_independent` / `LIMITED`，未补造
  continuous 结论；dashboard continuous replay 可见性由
  `tests/test_daily_task_dashboard.py` fixture 覆盖。paper signal quality 使用
  当前 continuous replay smoke 作为输入时只保留 `PAPER_ONLY_SIMULATION` warning，
  不出现 `DAILY_INDEPENDENT_ONLY` 或 live/trade/promotion 语义。验证通过
  replay/dashboard/continuous replay 目标测试 33 passed、paper signal quality
  目标测试 3 passed、`tests/trading_engine` 939 passed、scoped Ruff/Black、
  两种 replay smoke 和 paper signal quality smoke；GitHub Actions run
  `25986442870` 对 commit `75cba8fe37621c73e2c0e32334e3a0f78c3ad475`
  公开状态为 `Success`。任务从 `VALIDATING` 归档为 `DONE`；后续真实
  daily flow 持续观察由 TRADING-008 等后续任务承接。
