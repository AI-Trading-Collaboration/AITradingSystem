...
|runtime contract|2438K 输出 `runtime_executable=true` readiness claim 和通用 engine/adapter/smoke-check shell；2438B entrypoint 是 control-plane builder，缺少 compute-plane role、runner interface、runtime execution support 和 output schema；M artifact 中 candidate-specific executor id/version/input contract version 也全部为 null|BLOCKED；readiness shell 不能替代 callable engine contract 或 executor binding|
...
首个 required failed stage 是 `RUNTIME_CONTRACT_RESOLVED`。2438K 的
`runtime_executable=true` 只保留为 source claim；通用 engine contract、candidate-specific
executor binding 和 hydrated parameters 必须分层验证。当前不能从 candidate 名称或
rationale 推断策略参数，也不能自行设定投资阈值；否则会违反 heuristic governance、
PIT 审计和 no-silent-workaround 要求。其余独立 blocker 是参数化 runtime input 与
governed threshold spec 缺失。
...
- `CANDIDATE_RUNTIME_ENGINE_CONTRACT_UNRESOLVED`：source runtime-executable claim
  或通用 engine id/entrypoint/status contract 未就绪时使用；不得与 candidate executor
  mapping 缺失混为同一 blocker。下一责任方：系统在 M2 修复 compute-plane contract。
- `CANDIDATE_RUNTIME_EXECUTOR_MAPPING_MISSING`：三个 candidate 没有与 engine id
  对齐的 executor id、executor version 和 input contract version；2438K readiness claim
  与 2438B 通用 blocker-closure entrypoint 不能作为实际 binding。下一责任方：owner
  先在 M1 审批 mapping，随后系统在 `TRADING-2438M2` 验证并绑定真实 compute-plane。
- `CANDIDATE_RUNTIME_INPUT_CONTRACT_MISSING`：三个 candidate 没有参数化 executable
  ...
  研究 owner 定义/批准候选参数与 source policy。
...
- source `runtime_executable=true` 不得单独让 runtime contract stage PASS；必须有与
  engine id 对齐的 candidate executor id/version/input contract binding。
...
- 2026-07-10: completion audit 修正 `RUNTIME_CONTRACT_RESOLVED` 对 2438K
  `runtime_executable=true` readiness claim 的过度信任。新增
  `CANDIDATE_RUNTIME_EXECUTOR_MAPPING_MISSING`、source claim/engine contract/executor
  binding 分层字段、M2-only route 和 4 项 anti-shortcut regression，focused suite 更新为
  34 passed。真实 DQ=`PASS_WITH_WARNINGS`（0/2/12）；strict M run status 保持 BLOCKED，
  unresolved blockers 从 33 精确化为 39，新增 engine-contract/executor blocker=`3/3`，三个 candidate 的
  first failed stage 均修正为 `RUNTIME_CONTRACT_RESOLVED`，runtime source claim/engine
  contract/binding/executable=`true/false/false/false`，next route 仍为 M1，因为 owner
  parameters 与 threshold policy 也同时缺失。本修正未填入 candidate parameters 或
  threshold values，未启用 paper-shadow/production/broker/weight mutation。
