...
- 2026-06-07：从 `VALIDATING` 改为 `DONE`。原因：官方政策来源 parser
  fail-closed 修复已有目标测试与真实 daily-run 23/23 PASS 证据，后续最新生产
  daily-run 也稳定越过 `score_daily` 官方来源抓取链路，未再出现 OFAC XML row
  count 裸 traceback。
