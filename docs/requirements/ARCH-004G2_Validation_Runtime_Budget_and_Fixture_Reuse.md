...
### Wave 5 W5E1：Paper-shadow drift immutable upstream reuse

2026-07-21：Wave 5从`main@1fa742d3`启动。先复核Wave 4 Full profile首位候选
`tests/test_dynamic_v3_system_target_smoothed_operations_hardening.py`：Full file worker-s=`260.80s`，
无负载isolated=`1 passed / 84.21s`，约3.1倍差异表明主要风险是Full并发负载放大；在没有更强本地
重复证据前不修改该文件。随后筛选`tests/test_paper_shadow_drift_monitor.py`，Wave 4 Full file
worker-s=`155.80s / 3 nodes`，同机无负载isolated baseline=`3 passed / 66.57s`，call分别为
`17.51/18.08/25.40s`。

三个node各自调用同一helper，重复构造formal research method contract、paper-shadow protocol、signal-input
completeness和daily observation完整前缀；clean report与CLI节点的source语义完全相同，missing-input节点只在
market-panel artifact是否存在上不同。W5E1 owned scope仅为该test file：允许在module/worker内构造一次
immutable contract/protocol/signal-completeness shared prefix，并从该prefix在互斥variant目录各构造clean与
missing daily observation；两个clean consumer只读共享同一observation。每个test的drift output与CLI output
继续落入独立`tmp_path`，不得共享可写output、减少真实validator调用、缓存FAIL或修改production/helper/CLI/
policy。monkeypatch与artifact-validation session必须覆盖module fixture完整生命周期，并在teardown恢复。

退出条件预先冻结：保留3个nodeid；clean report、missing market-panel blocking finding、CLI report/read/
validate及research-safety断言全部保留；两次同命令after中较慢一次必须`<=56.57s`，即相对baseline至少
降低15%且绝对节省不少于10秒。任一source被测试写入、variant隔离丢失、validator/CLI语义弱化或收益未达标
即byte-exact撤回。通过后再刷新下一版Full duration seed/source-bound contract、module/test manifests、
compatibility/deprecation/source hashes并执行focused、architecture、contract与单次自然Full；单一Wave 5
样本不得升级为stable Full improvement，`production_effect=none`。

实现后两次同机同命令after=`3 passed / 29.16s`与`3 passed / 28.58s`；较慢值相对baseline降低
`37.41s / 56.20%`，通过双门槛。较慢一次中module setup=`8.17s`，clean/missing/CLI calls分别约
`3.50/3.31/8.80s`。3个nodeid、clean与missing daily observation的互斥目录、两个clean consumer只读
共享、per-test drift/CLI outputs、真实report/validator及CLI report/read/validate均保持。随后从Wave 4
唯一自然Full `full_20260720T175548Z`同步生成v19 advisory seed：`1085 files / 6500 nodes`，source profile
SHA-256=`99cecaf6...00e96f`，manifest SHA-256=`75216672...02d24`；source-bound test在同一integration
scope更新。formal integration仍待执行，`stable_full_improvement_claimed=false`、`production_effect=none`。

Wave 5正式集成完成：focused=`36 passed / 32.33s`；architecture=`446 passed / 36.73s`、artifact=
`architecture-fitness_20260720T185022Z`；contract=`265 passed / 108.65s`、artifact=
`contract-validation_20260720T185059Z`；唯一自然Full=`6498 passed / 2 skipped / 643 warnings`，pytest=
`928.81s`、runner=`929.82s`、artifact=`full_20260720T185256Z`。Full scheduler使用v19，匹配
`1085 files / 6500 nodes`，applied=true、fallback=false，profile/telemetry/performance/provenance均PASS；
tail idle max=`0.0080s`。W5E1目标文件Full worker-s=`155.8019 -> 59.6908`，减少
`96.1111s / 61.69%`；共同1085文件duration median ratio=`1.0039`，说明环境并未整体变快，文件级收益
具有针对性。整体file worker-s=`14868.10 -> 14704.52`（`-1.10%`），runner相对Wave 4下降
`10.65s / 1.13%`。本轮多出的1条warning来自既有IBKR测试触发第三方`eventkit` deprecation，W5E1没有
...
### Wave 10 W10E1：Execution semantics function-scoped validation session pilot

2026-07-22：W9E1闭合后从最近8份complete Full profile筛选下一独立候选。`tests/
test_execution_semantics.py`固定3个nodes，file worker-s依次约`176.71/175.46/186.72/195.40/
191.83/187.77/249.91/240.35s`；当前重节点在同一真实builder链内连续生成并验证多个依赖相同PASS
artifact的报告。W10E1 owned scope仅限该test file，允许接入既有function-scoped
`artifact_validation_session`，只在单个test生命周期复用内容指纹一致的PASS validation；不得跨test
共享、缓存FAIL/exception、修改production/helper/CLI/config、改变3个nodeid、断言、fixture数据规模、
producer/validator/CLI调用或DQ/PIT/投资语义，也不得重引入S3G per-node context cache。

先在无其他pytest负载下取得原文件baseline，再冻结绝对数值；两次同命令after较慢值必须至少降低20%，
且相对baseline绝对节省不少于25秒。任一node outcome、真实validation/CLI覆盖、安全边界、内存释放或
收益门槛不满足即byte-exact撤回。局部门槛通过后刷新manifests/compatibility/deprecation/source hashes，
并在本wave唯一自然integration-boundary Full同时验证v21调度画像；不足连续3份qualifying Full仍不得
声明stable improvement。`strategy_logic_changed=false`、`cached_data_mutated=false`、
`production_effect=none`。

无其他pytest负载baseline=`3 passed / 169.67s`，三个call=`0.04/160.24/4.13s`；据此在任何实现
改动前冻结两次after较慢值必须`<=135.74s`，并同时满足绝对节省`>=25s`。该数值不得按实现结果放宽。

W10E1最终状态=`CLOSED_REVERTED_EXIT_GATE_NOT_MET`。首轮after=`3 passed / 171.79s`，builder
call=`162.33s`，相对baseline反而增加`2.12s / 1.25%`，已足以拒绝方案，故不为凑样本继续第二轮。
test file的session import/fixture已byte-exact撤回，production与测试语义均未改变，也不运行formal Full。
该负结果说明Execution Semantics长尾不来自validation-session可消除的重复校验；后续不得以同机制重试，
应先做独立只读调用级归因。`production_effect=none`。

### Wave 11 W11E1：Simple-baseline CLI command-tree reuse pilot

2026-07-22：按W10退出结论先对`tests/test_simple_baseline_portfolio_control.py::
test_simple_baseline_cli_smoke_and_report_registry`做显式serial cProfile诊断；该运行只用于归因，不作为正式
PASS或性能baseline。单节点真实执行50次`CliRunner.invoke`，累计约`156.40s`耗在每次从同一个Typer
`app`重复构建完整Click command tree，真正50次Click command执行合计约`46.46s`，其中DQ约`25.12s`。

W11E1 owned scope仅限该test file：允许在该test开始时用Typer官方`get_command(app)`构造一次同一root
command，并让现有50次`runner.invoke`复用它。50组args、callback执行、参数解析、exit/output断言、全部
producer/validator/DQ与report-registry断言必须保留；不得mock callback、跳过CLI、修改production/app/
config、跨test缓存、改变nodeid或投资语义。先取得同一并行命令的无负载baseline，再冻结绝对门槛；两次
after较慢值必须至少降低50%且绝对节省不少于60秒，否则撤回。局部门槛通过后按本wave唯一自然Full同时
验证v21；serial profiler例外不得冒充parallel formal gate。`production_effect=none`。

无其他pytest负载的正式parallel baseline=`1 passed / 98.11s`，call=`92.61s`；据此在实现前冻结
两次after较慢值必须`<=38.11s`，同时满足相对降低至少50%与绝对节省至少60秒。数值不得事后放宽。

首个实现用Typer `CliRunner`直接接收预构建`TyperGroup`，在进入任何业务callback前因该runner会再次
调用`get_command`而以`AttributeError`正确失败；该run不是有效after，也未被降级为PASS。修正采用配套的
Click `CliRunner`执行Typer官方`get_command(app)`返回的Click root，这是预构建command的正式类型路径，
不是跳过CLI的workaround。两次有效after=`1 passed / 25.80s`与`1 passed / 25.84s`，较慢值相对
baseline节省`72.27s`、降低`73.66%`，通过`<=38.11s`及绝对节省`>=60s`双门槛；call从
`92.61s`降至较慢`20.69s`。50组args、真实callback、DQ/producer/validator、exit/output与registry
断言保持。W11E1进入完整文件与formal integration验证，`stable_full_improvement_claimed=false`。

W11E1完整文件回归=`7 passed / 102.92s`，正式pre-gates为architecture=`446 passed /
37.92s`、contract=`265 passed / 121.00s`。首个自然Full `full_20260722T015025Z`在
`6547 passed / 2 skipped / 6 failed`后以runner=`1094.01s`明确FAIL；W11目标业务节点及其余
6546个通过节点没有回归，但该run不得计为PASS或stable performance样本。两项失败来自v21 seed
刷新后`tests/test_validation_runtime_profile.py`仍绑定v20，且其traversal替换哨兵仍指向W9已拆除的
旧聚合文件名；另外四项失败发生在Full期间另一路工作把OPS-062状态写入共享`task_register`，令此前
生成的task shadow及compatibility source hashes即时过期。修复边界冻结为：source-bound测试绑定
当前v21并从实际profile选择traversal哨兵；保留并集成OPS-062共享文档增量，重新生成shadow/manifests/
compatibility hashes，focused门禁通过后以显式integration-retry原因重跑Full。不得撤销并行lane成果、
把首轮FAIL重写成PASS，或用串行结果替代；`production_effect=none`。

修复后focused=`28 passed / 14.77s`，registry/devex validate均PASS，正式architecture=`446 passed /
44.15s`、contract=`265 passed / 125.82s`。以`failure_fix_rerun`和首轮summary强绑定的Full
`full_20260722T022254Z`最终=`6553 passed / 2 skipped / 642 warnings`、pytest=`1088.18s`、
runner=`1089.21s`，parent明确记录首轮`PYTEST_FAIL`，W11E1状态转
`COMPLETE_RUNTIME_TASK_CONTINUES`。相对W9 Full，W11目标node=`144.60s -> 32.53s`
（`-77.50%`），目标file=`240.84s -> 134.71s`（`-44.07%`）；profile elapsed=
`1227.03s -> 1087.85s`（`-11.34%`），tail idle max=`190.893s -> 0.0146s`、total=
`2575.464s -> 0.149s`。但1095个共同文件duration ratio中位数=`1.0133`，worker busy中位数
反而约`+4.92%`，说明本轮全局墙钟收益主要来自v21消除队尾不均衡，而非所有测试普遍变快；两次
高度相关运行也不构成连续独立证据，故`stable_full_improvement_claimed=false`。

PASS profile已机械刷新为`arch_004g2_wave11_full_duration_partial_seed` v22，exact collection=
`6555 nodes / 1095 files`，tracked seed SHA-256=`c4b7fa4480d231f0165ccbcfc2c5109e835f27c8e322d26fea944e581185e83f`。
v22只作为下一自然Full的advisory scheduling输入；本批只更新source-bound test并跑post-full focused/
