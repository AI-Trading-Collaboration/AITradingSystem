...
关联任务：`DATA-001`、`VALUATION-002`、`BTINPUT-001`
...
owner 决策：先实现低成本 baseline，提高当前日报的估值/预期覆盖；完整生产级 `DONE` 仍需要真实 point-in-time estimates/archive，不能用采集日回填数据伪造历史可见性。
...
- 完整 `DONE` 条件仍保留：只有真实 PIT estimates/archive 或等价供应商快照才能支持严格历史回测中的 EPS 修正结论。
