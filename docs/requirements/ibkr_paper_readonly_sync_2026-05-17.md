...
- 2026-05-17：`TRADING-009A` 实现完成并进入验证。已新增本机验证 runbook、
  ...
  tests/trading_engine`、`python -m pytest`、`python -m ruff check scripts src
  tests` 和 `python -m black --check scripts src tests`。
- 2026-06-09：`TRADING-009` / `TRADING-009A` 从 VALIDATING 归档为 DONE。
  当前本机仍保留 ignored 真实 snapshot
  `outputs/reports/ibkr_paper_account_snapshot_2026-05-17.json/md`，字段级复核确认
  `snapshot_status=PASS`、`connection_status=CONNECTED`、`reconciliation_status=PASS`、
  `production_effect=none`、`readonly=true`、account id masked、account assessment PASS、
  account summary 30 rows、positions/open orders/recent executions 均为空列表，且完整账号、
  credential 和 order mutation 关键字匹配数为 0。默认配置 smoke 使用
  `python scripts/run_ibkr_paper_readonly_snapshot.py --date 2026-05-17 --output-dir
  outputs/reports/trading009_check` 输出 `DISABLED/BLOCK`，证明无 local 配置时不会连接
  真实 IBKR；sanitized fixture 与 smoke 输出均未匹配完整 `DUP` / `U` 账号、凭证或下单/取消
  关键字。验证通过 `python -m pytest tests/trading_engine/test_ibkr_paper_readonly.py -q`
  （15 passed）、`python -m pytest tests/trading_engine -q`（939 passed）、
  `python -m pytest -q`（2270 passed）和 `python -m ruff check scripts src tests`。
  TRADING-009 触达文件
  `scripts/run_ibkr_paper_readonly_snapshot.py`、
  `src/ai_trading_system/trading_engine/brokers/ibkr_readonly.py` 和
  `tests/trading_engine/test_ibkr_paper_readonly.py` 的 Black check 通过；全仓
  `python -m black --check scripts src tests` 当前仍被后续无关 130-file formatter baseline
  阻断，本轮未为归档 TRADING-009 重排无关模块，该后续治理继续由当前任务表中
  TRADING-006 提到的 repo-wide formatter baseline 承接。
