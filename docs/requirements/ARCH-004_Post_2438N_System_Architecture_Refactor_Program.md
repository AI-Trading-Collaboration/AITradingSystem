...
- 2026-07-23：Wave12审计时真实`data/raw/prices_daily.csv` SHA-256未被
  `data/raw/download_manifest.csv`覆盖，strict gate必须输出`DQ_MANIFEST_CURRENT_CHECKSUM_MISSING`；
  该FAIL实例保留为历史证据。Wave13复核时当前prices/rates/Marketstack SHA均已匹配，但非原子
  download/publish/manifest transaction、composite binding、market-calendar freshness、逐ticker
  coverage/internal gap与finite checks仍留在D0B2；不得因现场一致而提前授权consumer。
- 2026-07-23：Wave12 S2 已正式收口。combined focused=`246 passed / 1 skipped`；最终
  ...
  broker 均未自动授权。
- 2026-07-23：Wave13 GOV-006 N1 formal gate 已闭合。30条decision迁移结果为18 `DONE`/12
  `DROPPED`，active/completed=`405/487`；initial focused/architecture/contract/reproducibility=
  `124/527/266/23 passed`，final tracked-state focused=`194 passed`。自然Full=
  `6843 passed / 1 failed / 3 skipped`，唯一失败是已归档任务仍被
  旧测试要求留在active register；修复为terminal任务只在completed register唯一存在后，带parent
