...
- 2026-08-31：v7 独立 authority validation、Ruff 与 5 项代表性历史断言均 PASS；候选 `1aa68cd3778f21005b1eb60308e8b13c2759903d` 的 formal Architecture 为 `879 passed / 1 failed`。唯一失败位于 `TRADING-2458` 历史包含关系：简单的 `DEVX-011 exact paths - stop section recorded paths` 仍错误扣除了 5 个在 stop section 之后、DEVX-011 之前被 DATA-GOV-002-B2 重新声明的共享权威路径。失败证据为 `outputs/validation_runtime/architecture-fitness_20260830T195305Z/test_runtime_summary.json`，v7 已失败释放；v8 将把扣减资格收紧到 DEVX-011 之前的最后有效 owner 早于 stop section 的路径，保留所有 stop 之后的合法 successor ownership。
