...
### EB2 启动授权与 ARCH-005 pre-bootstrap 门禁调整

2026-07-19，project owner 在复核“完整 ARCH-005 前置不会直接加速 G2.4，但可复用的冲突检测
primitives 可能帮助后半程”后明确指令“继续按照这个思路实现”。该指令授权：

1. 从 base=`9c0025e4` 启动 `ARCH-004G2.4-EB2`，先顺序收口 Filtered Backfill / Comparison /
   Gate / Promotion / Owner Roadmap 的 15 个 callback；
2. EB2 integration gate 通过后，允许实现一个非 cutover 的 ARCH-005 pre-bootstrap primitives slice，
   只包含 versioned change manifest、owned/shared-path overlap、base drift、coordinator-only path、
   module/contract conflict、deterministic lane plan 与 validation-evidence binding；
3. pre-bootstrap 不创建 canonical task registry、不迁移 859 个 legacy tasks、不生成替代
   task-register views、不派发任务、不获取真实 lease，也不改变 Markdown source-of-truth；
4. EB3 继续保持当前 signal/filter 语义链顺序；只有 pre-bootstrap validation PASS 后，EB4/EB5/EB7/EB8
   才可选择两个互斥 domain lanes + 一个 integration coordinator 做受控试运行；EB6 继续单独隔离；
5. 整个 G2.4 phase exit、`arch_005_bootstrap_handoff.v1`、正式 ARCH-005 S0/S1 与 G2.5 恢复门禁不变。

EB2 callback freeze 共 15 个：app callbacks=
`callback_a33fa990c918948b9ebc`、`callback_a4cd6ebcc496b116d75f`、
`callback_40dc9ddf1428e6f075dc`、`callback_1968aa37900159f7ee2c`、
`callback_964cd04fa5d1959bc782`、`callback_16006da07acaee874ddb`、
`callback_7a7fca9876c77c238bb1`、`callback_0625ba1667fdf79f151f`、
`callback_1d8a34572cd3ea19688c`、`callback_8a776d6eb5742d8cb974`；validator callbacks=
`callback_4a1f13c3c10114ab1b7c`、`callback_b237ecd12ed1bacf746a`、
`callback_21e2267f900c47c3e547`、`callback_c5e9840ca543b39e79c0`、
`callback_1e7bb148f995a30f1fcc`。不得静默替换或吸收 EB3 callback。

EB2 同步修正 legacy “empty filter 仍生成五个候选、合成 return/drawdown/churn delta、选择 best
variant 并给出 CONTINUE_TESTING roadmap”语义。canonical path 必须绑定 EB1 validated full bundles、
同一 ledger/policy lineage 与 timezone-aware chronology；5 个 v2 snapshots 冻结 exact source files/hashes，
24 个 JSON/JSONL/Markdown views 全部可 byte rebuild。无 validated dated events 或 filter
`evidence_status!=SUFFICIENT` 时，variant specs/performance/signal metrics/comparison matrix/gate results均为空，
best variant、confidence 与 candidate spec 为 null，comparison/gate/promotion/roadmap 明确
`INSUFFICIENT_DATA`；不得用 aggregate proxy、默认分母或公式合成表现。所有 producer 在创建 output 前
验证 source，validator 重验 live source、lineage、policy、chronology 与全 view bytes。

2026-07-19，EB2 canonical implementation 与 legacy subtraction 已完成并进入 `VALIDATING`：15个CLI
callbacks迁至`interfaces/cli/etf_portfolio/dynamic_v3_filtered_candidate_pipeline.py`，15个public入口迁至
同名domain，legacy CLI不再持有decorator/callback，legacy domain仅保留15个lazy compatibility wrappers，
旧render/calculation helpers已删除。五类v2 snapshots、同一EB1 policy/source lineage、pre-output chronology、
24-view byte rebuild、live source/output/cross-lineage/chronology tamper fail-close与legacy/CLI parity已有focused
coverage；空filter/0 dated events的实际链输出empty rows、null winner/candidate/confidence/rates与
`INSUFFICIENT_DATA`。Callback matrix=`745 migrated / 222 pending / 0 unresolved / 0 duplicate`，CLI仍为
`41/291/993/0`且tree hash不变。当前只声明进入正式integration validation，不提前宣告EB2 gate PASS，
也不据此启动pre-bootstrap、EB3、ARCH-005 S0或G2.5。

EB2 integration gate 已闭合为 `COMPLETE_G2_4_CONTINUES`。Canonical/legacy/downstream focused=
`183 passed / 86.69s`，pre-EB2 artifact compatibility回归=`50 passed / 73.18s`，runtime-profile
合同=`91 passed / 14.49s`；pre-final architecture/contract=`374/262 passed`。首轮natural Full的
pytest=`6,329 passed / 2 skipped / 1,143.56s`，但新增test file令旧COMPLETE duration集合漂移，
strict reader正确回退并令该轮不得作为promotion evidence。保留原1,069-file历史行作为
`PARTIAL_SEED`后，唯一parent-bound recovery Full=`6,329 passed / 2 skipped / 1,029.52s`，
profile/telemetry/performance/provenance均PASS、scheduler applied/no fallback；相同完整测试集单次墙钟
约改善`10.0%`，但仍固定`stable_full_improvement_claimed=false`。该真实sidecar已机械生成
`COMPLETE v6`，精确绑定`6,331 nodes / 1,070 files`、source/profile SHA和collection/file/order
hash，离线全集 verifier PASS。Matrix仍=`745/222/0/0`，所以whole G2.4、handoff、EB3、正式
ARCH-005 S0与G2.5均未解锁；仅owner已批准的非cutover pre-bootstrap primitives现在可启动，
`next_phase_or_slice_unblocked=false`、`production_effect=none`。

Owner批准的`ARCH-005-PB1` non-cutover pre-bootstrap primitives随后完成。`change_manifest.v1`与
`validation_evidence.v1`现已提供canonical hash、owned/shared path、module、contract access/version、
base drift、coordinator-only guard、显式capacity deterministic waves和root-contained artifact SHA binding；
所有结果固定`dispatch_allowed=false`、`lease_acquisition_allowed=false`、`task_registry_mutated=false`。
Focused/runtime-profile/architecture/contract=`49/23/395/262 passed`，natural Full=`6,350 passed /
2 skipped / 912.17s`且profile/telemetry/performance/provenance PASS、无fallback；`COMPLETE v8`精确覆盖
`6,352 nodes / 1,071 files`。单次墙钟较EB2 recovery Full约改善`11.4%`，但继续固定
...
- ARCH-005 canonical registry/S0～S6、G2.5/G3～G7、ARCH-004H cutover；EB2 后获批的非 cutover
  pre-bootstrap primitives 不在本延后项内；
