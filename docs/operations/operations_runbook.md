...
|Weekly / Manual|ETF weight calibration dual-track setup|`aits etf weight-calibration search --search etf_initial_weight_search_v1`、`aits etf weight-calibration register-candidates --latest --top N`、`aits etf weight-calibration enroll-forward --latest --top N`|需要重新生成 historical candidate initial weights 或把已登记 safe candidates 显式放入 forward observation 时；当前 TRADING-071E 仅完成 search/register/enroll，TRADING-071F-K 的 evidence aggregator、report 和 validation gate 后续补齐。|Search 必须先通过 ETF price quality gate，并披露 `ai_after_chatgpt` regime、requested date range、baseline/benchmark comparison、robustness metrics 和 safety fields；register/enroll 只写 ignored `data/etf_portfolio/weight_calibration/` runtime state，blocked/rejected/needs_more_data candidates 不得 enroll；固定 `observe_only=true`、`candidate_only=true`、`production_effect=none`、`broker_action=none`、`manual_review_required=true`；不得写 `target_weights.csv`、baseline config、shared experiment shadow registry 或 broker state。|
