...
- TRADING-074A/B/C/D/E/F 的 ETF operations schedule spec、daily / weekly / biweekly / monthly graph、artifact freshness checker 和 failure policy evaluator 只用于规划和校验 ETF portfolio research workflow；weekly / biweekly / monthly graph 会显式披露跨 cadence 上游为 external dependencies，并标记人工复核 checkpoint；monthly graph 保证 bounded historical weight search 只在 slower cadence 计划中出现；freshness checker 只读扫描既有 artifacts，required stale/missing/unknown artifacts 标记为 blocking，optional missing artifacts 标记为 warning，并传播 upstream blocking dependency；failure policy evaluator 把 issue 转成 `info` / `warning` / `error` / `critical` severity、`fail_pipeline` / `block_dependent_steps` / `manual_review_required` 等 policy effect 和稳定 summary；在 TRADING-074H/K dry-run 和 validation gate 完成前，它们不执行命令、不自动 dispatch cadence、不改变 `aits ops daily-run` 的统一入口。
