...
- Owner 已于 2026-07-23 通过“结合我们的并行能力推进这些任务”的新显式指令恢复
  ...
  或自动 merge。
- Wave12 `ARCH-004G4 Operations + DATA-GOV-001 D0B1/S2` 已正式 PASS，G4 转 `VALIDATING`、D0B2
  保持未 dispatch。Wave13 `GOV-006 N1` coordinator-only formal gate 已PASS；30条reviewed decision
  已经应用并由commit-bound artifact证明，shared task register/views在closeout提交推送前继续单写，
  不把空闲domain slot解释成自动派发。N1正式提交推送后先从最终HEAD
  生成新的Wave14 exact manifests、ownership与readiness；Wave14 S0 contract/readiness PASS后才开放
  `D0B2 + bounded G3`。旧G2.5 rehearsal不是dispatch authority；G5继续等待G3共享contract稳定，
  automatic dispatch、consumer cutover或production权限均未开放。
...
- 不因 Wave12 formal PASS 或N1 application commit自动进入 G3/G5、ARCH-004H 或 ARCH-005 S5；
  Wave14 D0B2+G3仍需N1正式validation、commit/push后从最终HEAD生成新manifest/ownership/readiness，
  并通过Wave14 S0 contract/readiness后再dispatch；旧G2.5 rehearsal不可复用为授权；
