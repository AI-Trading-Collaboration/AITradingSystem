...
- Wave12 `ARCH-004G4 Operations + DATA-GOV-001 D0B1/S2` 已正式 PASS，G4 转 `VALIDATING`、D0B2
  保持未 dispatch。当前执行波次是 Wave13 `GOV-006 N1` coordinator-only 高置信terminal reconciliation；
  shared task register/views在该小批期间单写，不把空闲domain slot解释成自动派发。Wave14才开放
  `D0B2 + bounded G3`，G5继续等待G3共享contract稳定；automatic dispatch、consumer cutover或production
  权限均未开放。
- Wave12 final closeout后的第一步调整为Wave13 `GOV-006 N1` coordinator-only高置信terminal
  reconciliation；它不占domain worker，但shared task register/views在该小批期间保持单写。任务池重建并
  验证后，Wave14再并行`DATA-GOV D0B2 + bounded G3`；G4C真实cadence observation异步累积。Wave15只做
...
|Coordinator|`W12_S2_COMPLETE`|拆分`operations_as_of`与`data_quality_as_of`；profile/as-of exact pointer、strict verifier与daily step interval已接入；combined focused=`246 passed / 1 skipped`，formal Full=`6825 passed / 3 skipped / 1147.04s`|Wave13先做GOV-006 N1；不自动开放D0B3/G4B、G3/G5、S5、H、策略研究、promotion、production或broker。|

当前本地真实`data/raw/prices_daily.csv` SHA-256在`data/raw/download_manifest.csv`中无匹配记录，
canonical strict source binding必须产生`DQ_MANIFEST_CURRENT_CHECKSUM_MISSING`。这是D0B2要直接修复的
download/publish/manifest transaction，不是允许临时豁免的S2集成问题。S2退出仍需combined focused、
该 blocker 不影响 Wave12 S2 的 nonexecuting receipt/parity closeout；D0B2 必须在真实 transaction 上直接
修复。Wave12 最终 architecture/contract/reproducibility/integration=`525/266/23/983 passed`，首次 Full
...
- 不因 Wave12 formal PASS 自动进入 G3/G5、ARCH-004H 或 ARCH-005 S5；当前只执行 Wave13 GOV-006
  N1，Wave14 D0B2+G3仍需该治理批次验证后按新manifest dispatch；
