...
状态：`ADOPTED_COORDINATED_INTEGRATION_WITH_DEFAULT_PUSH`（基于 ARCH-005 S4/S4A/S4C；
S5 未授权）
...
2026-07-26 v2流程决定取代v1的remote边界：

- final-tree门禁和local-main fast-forward通过后，coordinator默认fetch并ordinary push
  `origin/main`，随后复核`local main=origin/main=candidate`；
- owner no-push、无remote/upstream、无关提交、remote divergence、non-fast-forward或需要
  merge/rebase/history rewrite/force-push时停止并报告；
- PR、force-push和remote divergence修复仍需单独显式授权，worker仍不得自动push。
...
### S4C 验证通过后的 local main 与 remote main 集成门禁

Owner 已授权 coordinator 在每个 integration batch 的适用门禁全部 PASS 后收口到local main；
DEVX-002 v2进一步授权门禁满足时默认ordinary push remote main。顺序固定：
...
6. 确认`local main=candidate`；
7. fetch `origin/main`，验证它是candidate祖先；若diverged、无upstream或需要非fast-forward/
   history rewrite则停止并报告，不自动修复；
8. 执行ordinary non-force push，复核`local main=origin/main=candidate`，再以该提交作为下一
   batch的exact base。Owner显式no-push时记录skip原因。

任何 dirty/unattributed worktree、stale validation/hash/base、活动lease、分叉或非fast-forward均停止
本地集成并报告；remote push rejection也fail closed并保留已验证local-main状态。禁止用merge commit、
rebase、force-push、删除用户改动或降级验证继续。该集成权限本身不延伸到worker、PR、task status自动变更、
...
- Owner 已于 2026-07-23 通过“结合我们的并行能力推进这些任务”的新显式指令恢复
  ...
  或自动 merge。
- Wave12 `ARCH-004G4 Operations + DATA-GOV-001 D0B1/S2` 与Wave13 `GOV-006 N1`均已正式PASS。
  Wave14 S0 carrier=`39a3ea730`随后通过并由coordinator manual assignment
  `D0B2 + bounded G3`；两个domain现已完成并进入S2 shared integration/formal exit。旧G2.5 rehearsal
  仍不是dispatch authority；G5继续等待G3共享contract稳定，automatic dispatch、consumer cutover或
  production权限均未开放。
- Wave12 final closeout后的第一步调整为Wave13 `GOV-006 N1` coordinator-only高置信terminal
  reconciliation；它不占domain worker，但shared task register/views在该小批期间保持单写。任务池重建、
  验证并推送后，先完成基于最终HEAD的Wave14 S0 exact manifests/ownership/readiness，再并行
...
### Wave 12：G4A + D0B1 -> S2 shared integration（formal PASS）

Base、ownership、contract与两个`change_manifest.v1`已在S0冻结；两个domain worker只交付互不重叠的
实现与focused tests，当前shared path只由coordinator集成。

|Lane|当前状态|Owned result / S2 handoff|停止边界|
|---|---|---|---|
|Data Foundation|`D0B1/S2 PASS`；overall `IN_PROGRESS`|canonical `data_quality_execution_receipt.v1`、single-validator runner、immutable report/receipt、public strict verifier与tamper/drift/chronology tests；S2接入`aits validate-data`、3小时provider-ready默认日期及`daily_default/{data_quality_as_of}` atomic discovery pointer|Pointer只发现exact receipt，不证明PASS；`PASS_WITH_WARNINGS`仍阻断。Wave12 checksum-missing实例保持历史证据；当前现场SHA已匹配，但D0B2仍须关闭非原子publish/manifest、composite binding、calendar/coverage/gap/finite缺口。|
|Operations|`G4A/S2 PASS`；overall `VALIDATING`|fixed 2 daily / 2 weekly / 1 monthly representative matrix，其中`daily_validate_data`是1个receipt producer observation，其余4项是verified receipt consumers；native sidecar只做date/condition、lock、duplicate、retry/resume、typed DQ blocker与lineage parity|Sidecar nonexecuting；G4C cadence异步积累，G4B first consumer等待D0B3；无dispatch、consumer cutover、production或broker。|
|Coordinator|`W12_S2_COMPLETE`|拆分`operations_as_of`与`data_quality_as_of`；profile/as-of exact pointer、strict verifier与daily step interval已接入；combined focused=`246 passed / 1 skipped`，formal Full=`6825 passed / 3 skipped / 1147.04s`|Wave13 GOV-006 N1已formal complete，待closeout commit/push；不自动开放D0B3/G4B、G3/G5、S5、H、策略研究、promotion、production或broker。|

Wave12审计时真实prices SHA无manifest匹配，canonical strict source binding正确要求
`DQ_MANIFEST_CURRENT_CHECKSUM_MISSING`；该历史FAIL实例不可改写。Wave13重新核对后，当前
prices/rates/Marketstack SHA均已有manifest匹配，但这只证明当前快照一致，不能证明现有“先覆盖CSV、
后更新manifest”的流程具备事务性，也不能证明composite row-count/source binding、calendar、
requested-window coverage、internal-gap或finite gate。D0B2必须在真实transaction上直接修复这些
结构缺口。Wave12 最终 architecture/contract/reproducibility/integration=`525/266/23/983 passed`，首次 Full
暴露关闭包旧断言后，冻结 TRADING-2452 package 不改写，failure-fix Full=`6825 passed / 3 skipped /
643 warnings / 1147.04s`。整体继续`production_effect=none`、`broker_action=none`。

### Wave 13：GOV-006 N1 terminal reconciliation（formal complete）

本wave是coordinator-only治理批次，不占domain worker。reviewed dry-run manifest绑定Wave12
HEAD=`02f9492a1975440240cb65f761d2cf544a5a875e`；application commit=
`58ee6a80b5a04ff68a97a96d36b575ae8391f657`已经把30条decision应用为18 `DONE`/12 `DROPPED`，
active/completed=`435/457 -> 405/487`。`gov_006_portfolio_normalization_applied_closeout.v1`
从两个Git commit blob重放before/after registers，证明task-id守恒、非目标partition/priority/status
不变、30个原物理行空槽保留，closeout id=
`gov_006_applied_closeout_bdbfd433a72d5349ead9`。append-only compatibility、initial focused/
architecture/contract/reproducibility=`124/527/266/23 passed`；final tracked-state focused=`194 passed`。
自然Full=`6843 passed / 1 failed / 3 skipped`，
修复已归档task的旧active-register断言后，failure-fix Full=`6844 passed / 3 skipped / 643 warnings /
1340.11s` PASS。PASS profile精确覆盖`6847 nodes / 1108 files / 16 workers`，profile/telemetry/
performance/provenance均PASS；旧v23仅跟踪1097 files且本次tail-idle max=`211.69s`，因此机械刷新
advisory seed为`arch_004g_wave13_full_duration_partial_seed` v24，不声明稳定提速，留待下一自然Full
...
- 不把“默认双线”解释成 S5、worker自主合并或 autonomous task mutation；coordinator 仅按S4C
  final-tree门禁合入local main并在远端门禁满足时ordinary push，不能自动修复remote divergence；
