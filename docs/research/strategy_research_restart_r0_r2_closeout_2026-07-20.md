...
|R1 OOS|R0 PASS、source top-20、2 windows|1-day purge + 1-day embargo；每 fold 真实 evaluator；cost/lag/chronology/false-signal/gate|`reports/etf_portfolio/dynamic_v3_rescue/walk_forward_r1/r1-wf_6447beb5464bad37/`|
|R1 robustness|R0 PASS、source candidate、neighbor grid、daily paths|real/derived neighbor、high-drawdown、fast-recovery、per-regime dynamic-vs-static comparator|`reports/etf_portfolio/dynamic_v3_rescue/robustness_r1/r1-robustness_8c93b0e2615d0ace/`|
...
2026-07-20 ARCH-005 S4 试运行前发现 restart policy 格式化后，旧 R0 artifact 的 source
fingerprint 已过期。没有修改 checksum 或跳过 validator，而是按完整 lineage 重建 R0/R1/R2：
walk-forward=`r1-wf_6447beb5464bad37`（228.2s）、robustness=
`r1-robustness_8c93b0e2615d0ace`（94.9s）、R2=
`r2-decision_c761da11538fc58c`。结果与原结论一致，R2 仍为
`CONTINUE_EVIDENCE_CLOSURE`；这是 source-freshness 修复，不是重新选择候选或改变投资结论。
