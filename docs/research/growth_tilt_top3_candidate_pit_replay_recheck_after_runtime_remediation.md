...
- runtime remediation ready: `True`
- executable handoff ready: `True`
- outcome rechecked: `True`
- pass / fail / blocked: `0` / `0` / `3`
- post-runtime blocker count: `3`
- forward-aging handoff ready: `False`
- next route: `TRADING-2438M_Growth_Tilt_Post_Runtime_Candidate_PIT_Replay_Blocker_Resolution`

TRADING-2438L 在 2438K runtime remediation READY 后独立重判 top-3 candidate 的 PASS / FAIL / BLOCKED。若 runtime executable 但 metric values 或 threshold evaluation 仍缺失，candidate 必须保持 BLOCKED 并进入 2438M post-runtime blocker route；不能误标为 FAIL、NO_PASSING_CANDIDATE 或 paper-shadow candidate。
...
```json
{
  ...
  "schema_version": "growth_tilt_candidate_runtime_recheck_decision_matrix.v1",
  "status": "GROWTH_TILT_TOP3_CANDIDATE_PIT_REPLAY_RECHECK_AFTER_RUNTIME_REMEDIATION_BLOCKED"
}
```
...
```json
{
  ...
  "production_effect": "none",
  "schema_version": "growth_tilt_candidate_forward_aging_after_runtime_remediation_handoff_summary.v1",
  "status": "GROWTH_TILT_TOP3_CANDIDATE_PIT_REPLAY_RECHECK_AFTER_RUNTIME_REMEDIATION_BLOCKED"
}
```
