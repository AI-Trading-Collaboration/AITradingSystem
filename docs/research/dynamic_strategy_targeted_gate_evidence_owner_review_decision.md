...
- 2399 best targeted variant 是 `growth_tilt_guarded_transfer_valid_until_strict_v1`。
- 2399 decision 是 `CONTINUE_TARGETED_IMPROVEMENT`。
- observation preview candidate count 为 `0`。

## Best targeted variant review

- `valid_until_strict` 方向有研究价值，主要来自 signal expiry discipline、stale signal guardrail 和 valid-until strictness。
- 该方向仍未达到 observation preview，不应进入 research-only observation 或 paper-shadow。
...

{
  "OPTION_A_CONTINUE_VALID_UNTIL_STRICT_TARGETED_IMPROVEMENT": {
    "meaning": "continue small valid-until strict improvements, with marginal benefit decay risk",
    "recommended": false
  },
  "OPTION_B_PAUSE_RECOMBINATION_CANDIDATE_LINE_AND_REVIEW_DATA_SIGNAL_QUALITY": {
    "meaning": "pause the current recombination candidate line and review data, PIT, signal quality and regime labeling",
    "recommended": true
  },
  "OPTION_C_BUILD_THRESHOLD_META_DATASET": {
    "meaning": "build threshold calibration meta-dataset to improve gate judgment",
    "recommended": true
  },
  "OPTION_D_RETURN_TO_SIGNAL_CONSTRUCTION_FRAMEWORK": {
    "meaning": "return to indicator -> signal -> weight mapping instead of more strategy-variant combinations",
    "recommended": false
  },
