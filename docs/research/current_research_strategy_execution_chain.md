...
| Fixed-share return与风险 | frozen variant weights、完整price path | `gross_return=Σ w_i(P_i,end/P_i,start-1)`，CASH回报按0；逐日fixed-weight return path用于gross max drawdown/annualized realized volatility，不在窗口内再平衡 | gross return、gross-path drawdown/volatility | 权重或路径不完整不计算部分组合。后续支持versioned cash yield、dividend/tax/corporate-action与daily rebalanced comparator，但必须保留当前schema重放 |
