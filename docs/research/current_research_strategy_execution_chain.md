...
TRADING-2453 的冻结输入重算、constraint-hit 分层归因和 owner review。
...
结果链。后续优化空间只包括：重算 constraint-hit 口径、按 fold/template/policy hash 分解集中度、审计
constraint policy intended effect 与实现是否一致、形成 owner options。若没有发现实现缺陷，继续 KILL/PAUSE；
若要改变 heuristic 或 candidate universe，必须新任务、新 policy version 和新 preregistration，不能在已看
到结果的 TRADING-2452 内改值重跑。
