...
| 冻结证据 | `confirmation_registry_input_snapshot.v2`保存Plan full JSON/Markdown bundle、validation、lineage、registry preimage、预期postimage与路径 |
...
| 冻结证据 | `confirmation_progress_input_snapshot.v2`保存Registry full bundle/validation、selected evidence full bundles/validations、source roots、path/id和selection inventory；validator按同一cutoff重做selection并检测后来插入或source drift |
