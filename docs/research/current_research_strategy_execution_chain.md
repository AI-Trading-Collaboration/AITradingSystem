...
7. `2021-02-22`可在明确标记的长期primary/context研究中另行报告；本链默认AI-cycle conclusion窗口仍自`2022-12-01`，两组结果不得合并headline。
...
3. 45 个 0-sample regime rows 需要 PIT/forward distinct events 或预注册 stress cohort；
   不得跨 regime 借样本或填 0。`2021-02-22`如作为长期 context，必须单独报告，AI-cycle
   primary conclusion 仍从 `2022-12-01` 开始。
...
1. 先补 forward/PIT distinct observations，并分别报告 `2022-12-01` AI-cycle conclusion 与另行声明的 `2021-02-22`长期 context；不得合并 headline 或用 context 样本补 primary floor。
...
1. 先补独立 forward/PIT event observations；默认结论窗口保持 `2022-12-01` 起的 `ai_after_chatgpt`。如使用 `2021-02-22` 起的长期 context，必须单列 regime-comparison 报告，不能混入 primary sample floor 或 headline。
