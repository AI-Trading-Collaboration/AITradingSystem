...
项目级 active market regime：`unified_primary_2021`，起点 `2021-02-22`

当前 QQQ/SGOV/TQQQ primary validated research window：`exact_three_asset_validated`，起点 `2021-02-22`

> 2026-07-21 语义迁移：本文后续出现的 `ai_after_chatgpt=2022-12-01`、
> “当前/默认 AI-cycle 结论窗口”若来自已完成 slice、fixture 或历史运行，只表示当时实际 contract，
> 不再是 future active default。后续新 run 一律使用 `unified_primary_2021=2021-02-22`；
> `ai_after_chatgpt` 只作为显式 historical comparison。不得改写历史 requested/actual range，
> 也不得让它继续提供默认值、下限或 primary evidence。
...
### 1.2 2026-07-21 TRADING-2452 统一窗口正式结果

TRADING-2452 把 future active default 与主结论统一到 `2021-02-22`，新建 versioned
preregistration package，冻结 300 candidates、六个 historical-seen folds、train-only top-20、
cost/lag/purge/embargo 与 `2026-07-22` 后 prospective access=false。有效 run
`trading2452-historical-seen_20260721T053621Z_144f31edee91` 的 package、runtime DQ、source/output
hash、cache commitments 与独立 validator 全 PASS，总耗时 25m45s。

真实研究结果不是“找到 top-20”，而是六 fold 共 1,800 个 `COMPLETE` train evaluations 全部
`gate=reject`、`selection_score=null`：所有行命中 `constraint_hit_rate_exceeds_policy`，第 6 fold
另有 96 行命中 `constraint_hits_delta_exceeds_policy`。因此 selected/test/recent 全部为 0，报告为
`INCOMPLETE_NO_ELIGIBLE_CANDIDATE`。这证明统一窗口下当前候选与 gate 无法形成可进入 test 的选择集；
它不证明所有 dynamic 思路永久无效，也不授权事后改阈值或扩候选。下一策略步骤是
TRADING-2453 的冻结输入重算、constraint-hit 分层归因和 owner review；该诊断结果见下一节。

S4 最终 Full=`6543 passed / 2 skipped / 642 warnings / 1010.39s`，profile、telemetry、
performance evidence 与 provenance PASS；期间暴露的 Windows singleflight lock 初始化竞态已由
OPS-066 以“持锁后初始化”修复并通过完整 cache/coordination focused。该工程修复不改变上述
TRADING-2452 负面研究结果或任何投资阈值。

### 1.3 2026-07-21 TRADING-2453 Constraint-hit 诊断

正式diagnosis=`trading2453-constraint-hit_745d7d6d7b4579929546`，只读绑定TRADING-2452有效
run/package及冻结policy/source hashes，未重跑evaluator。1,800/1,800行hit count↔rate、delta、
candidate parameters、gate与reasons exact match，content-derived validator=`PASS/0`，所以
TRADING-2452拒绝不是serializer、算术或gate执行漂移。`payload.best_candidate`经源码审计确认是四个
`dynamic_v0_3_rescue` templates内部选优的现行设计，不是实现取错candidate。

主要发现是`POLICY_ROLE_MISMATCH_REQUIRES_OWNER_REVIEW`：当前`0.65`来自small-real smoke，原注释
说明其用于打通observe-only downstream且不是promotion gate；TRADING-2452 preregistration却把它冻结
为fold train hard eligibility。结果在该冻结合同下仍然有效，但policy依据与消费角色需要Owner重新
...
### 1.4 2026-07-22 TRADING-2457 无污染选择协议基础

当前策略线没有重开B/C或再次运行TRADING-2452，而是先补齐更上游的通用admission contract。原因是
现有负面结果已经足够说明：如果candidate来源、结果可见时间、数据角色与policy consumer role没有在
结果出现前冻结，再强的evaluator也无法把事后选择变成无偏证据。

TRADING-2457 已完成 reviewed foundation policy、typed protocol与pure validator。它把数据明确拆为
`DISCOVERY_HISTORICAL_KNOWN`、`TRAIN_SELECTION`、`HISTORICAL_SEEN_VALIDATION`、
`PROSPECTIVE_UNTOUCHED`，active primary固定从`2021-02-22`开始；full-period top-N、结果可见后freeze、
角色窗口重叠、prospective访问、observe/reporting policy被当作hard eligibility、缺owner/version/
rationale/review/hash或content tamper均fail closed。valid输出只到
`READY_FOR_OWNER_PROTOCOL_AUTHORING`，且execution/search/holdout/promotion/production/broker仍全部关闭。

该foundation focused 19项全部通过；它不产生收益、排名或新投资结论。下一步只有在Owner另行选择具体
hypothesis/candidate generator，并在结果出现前冻结具体protocol后才可启动新研究；若选择B或C，仍需
各自新任务与显式授权。

最终集成门为 focused/architecture/contract/reproducibility=`28/447/265/23 passed`，唯一自然Full=
`6604 passed/2 skipped/1179.75s`；validation provenance、scheduler、telemetry、performance与全部
safety boundary PASS。该Full只证明foundation与既有系统兼容，不把`READY_FOR_OWNER_PROTOCOL_AUTHORING`
解释为策略效果、研究执行或prospective访问授权。
...
同一项目同时存在 market regime、research window、historical comparison、requested range、actual coverage、effective signal range 和 evaluation range。active regime 与 scoped primary window 现在都从 `2021-02-22` 开始，但 ID、证据角色和覆盖范围仍不能互换。`ResearchEvaluationContext` 因此继续拆分这些概念并做一致性验证。
...
| 概念 | 当前值/用途 | 不能推出 |
|---|---|---|
| `unified_primary_2021` market regime | 2021-02-22 起；后续active策略研究、主回测和主要结论默认 | 不是所有输入的数据可用起点 |
| `exact_three_asset_validated` | 2021-02-22 起；QQQ/SGOV/TQQQ primary decision evidence | 是research-window ID，不是market-regime ID |
| `ai_after_chatgpt` | 2022-12-01 起；历史 ChatGPT-cycle comparison | 不提供active default或primary evidence |
...
`config/market_regimes.yaml` 的 active default 与 QQQ/SGOV/TQQQ primary window 均从
`2021-02-22` 开始；caller 仍必须同时解析 market-regime 与 research-window policy，不能因日期相同而
把两个 ID 合并。旧 backtest/config 中的 `2022-12-01` 只能在明确 historical role 时继续读取。

任何 window-facing artifact 至少应分开披露：

| 字段 | 谁决定 | 含义 | 当前审计能力 |
|---|---|---|---|
| `market_regime_id/start` | `config/market_regimes.yaml` | active=`unified_primary_2021/2021-02-22`；historical comparison=`ai_after_chatgpt/2022-12-01` | policy/context校验，不由实际数据起点反推 |
...
特别注意：新 run 的 `configured_backtest_start` 与 primary `actual_evaluation_start` 应共同为
`2021-02-22`；若历史 artifact 仍显示 configured=`2022-12-01`，必须标为 legacy comparison/old
contract，不得用于新 primary 结论。`research_window_id/role/caveats`仍负责区分 primary、warm-up、
stress、sensitivity 和 historical comparison；Window Audit 的 range PASS 不能替代这种证据角色校验。
...
本段记录的 TRADING-2451 冻结包使用 AI regime anchor=`2022-11-30`、历史结论起点=
`2022-12-01`；该包已被 TRADING-2452 supersede，不能再提供 active default。后续 primary requested
research start=`2021-02-22`；旧包四个test windows均从2023开始，且历史结果统一标为`KNOWN`。
真正outcome-blind evidence从冻结后的prospective holdout `2026-07-22..2027-07-21`开始，本任务
...
#### TRADING-2452 unified-2021 historical-seen evaluator

TRADING-2452 已替代上述旧 S1 包并实际执行。输入是新的 2021 package、同源 cached-data DQ 与冻结
runtime/config hashes；每个 train fold 对 300 candidates 逐一生成 real evaluation，再按预注册 gate/score
选择，不使用 test metric。候选 robustness reports 先按 `(label, policy hash)` 全局去重：每个 train
只有 228 份 unique report；24 workers 通过 phase-transient content-addressed cache 消费，校验后删除大
payload，manifest 只保留 identity/file/report/binding commitments。六个 train 共计算 1,368 份，
cache hit rate 81%，释放 15.3 GB 临时字节；空 test/recent phase 不计算 fixed reports，但仍写完整零计数
telemetry。JSONL 固定为 one-object-per-line，validator 会重读 1,800 行并重算 selection。

当前输出为 1,800 COMPLETE、1,800 reject、0 selected、0 test、0 recent；这是一条 fail-closed 的负面
结果链。TRADING-2453 已完成constraint-hit口径重算、fold/template/policy hash集中度、policy intended
effect/实现审计与owner pack：机械计算exact、template best-row是现行设计，主要问题是smoke来源的0.65
与fold hard eligibility之间的policy-role mismatch。当前继续`KILL_PAUSE`；若要改变heuristic或candidate
universe，必须新任务、新policy version和新preregistration，不能在已看到结果的TRADING-2452内改值重跑。

#### TRADING-2457 generic uncontaminated selection admission

为什么不直接再跑一次：TRADING-2452的evaluator、DQ与内容重算已经工作正常，真正缺口位于运行前的
selection provenance。TRADING-2457因此是“协议准入”，不是第二套evaluator或新的策略模块；它复用
canonical `ResearchEvaluationContext`与`ResearchPreregistration`，只解析tracked policy和typed payload。

|环节|输入|计算与阻断逻辑|输出|后续优化空间|
|---|---|---|---|---|
|Foundation policy|owner/version/status/rationale/review condition；active regime/window；四角色、禁止overlap与禁止origin；safety字段|canonical YAML payload规范化并计算semantic SHA-256；status只接受reviewed集合；active primary必须匹配`unified_primary_2021/exact_three_asset_validated`和`2021-02-22`|reviewed `uncontaminated_selection_protocol_foundation_policy.v1`|未来只能version bump后变更角色或准入规则；不得原地把2022恢复成默认|
|Candidate commitment|universe/generator id+version+SHA、universe SHA、derivation source ids、是否full-period ranked|derivation source必须全部属于discovery；full-period/result-visible top-N origin或rank flag任一出现即BLOCKED|稳定candidate universe commitment|具体candidate generator仍需新protocol authoring；foundation不生成候选|
|Role/visibility timeline|四个exact-once data bindings：date range、source SHA、market outcome visibility、freeze时evaluation visibility、first-visible time、access/input flags|六组角色两两窗口不重叠；train/validation/prospective在freeze时必须`ResultVisibility.NONE`；first-visible早于或等于freeze阻断；prospective必须UNTOUCHED、未访问、无可见时间|可审计data-role matrix与逐check blocker|后续可加append-only single-access ledger；没有Owner授权前不能读prospective|
|Policy consumer role|selection policy id/version/status/owner/rationale/review/source SHA、intended roles与consumed roles|consumed必须是intended子集；observe-only/reporting-only不能消费为hard eligibility；selection rule SHA与canonical preregistration互相绑定|policy-role compatibility checks|若走B，另建reviewed hard-gate policy；不能复用small-real smoke阈值|
|Identity/admission|上述payload、canonical context/preregistration及全部safety|按稳定排序与canonical JSON重算`protocol_id`；嵌套tamper、policy semantic drift、缺字段或unsafe flag均BLOCKED|`FOUNDATION_ONLY + READY_FOR_OWNER_PROTOCOL_AUTHORING`，或明确blocker；`execution_unblocked=false`|下一阶段先由Owner写具体protocol，再单独验证；admission PASS不等于research/OOS/promotion PASS|

当前实现只读取内存contract和tracked foundation policy，不调用market provider、cached DQ、evaluator、
backtest、search、paper-shadow、promotion或broker。`NOT_PREREGISTERED`是typed但不可authoring的阻断状态；
因此不会用“已有一个schema”伪装成“已获准运行”。优化优先级依次为具体protocol authoring模板、结果出现前
的外部签名/冻结证据、prospective append-only access ledger，再到经新授权的runner adapter；在这些完成前
不应增加更多候选或调阈值。
...
### 7.7 Forward Confirmation Plan（TRADING-173 / ARCH-004G2.4BX）

为什么这样设计：Forward Bridge已经冻结reviewed numeric criteria，Proposal Review只确认哪些真实proposal可进入人工观察。旧Plan重新拼接两个mutable view，又补造Bridge不存在的consensus target和10/5 events、0.55/0.50/0阈值，导致“计划”悄悄成为第二套策略policy。本环节因此只做严格、可重放的source projection。

| 项目 | 当前定义 |
|---|---|
| 输入 | 显式Proposal Review与Forward Bridge ids/roots、timezone-aware cutoff；两个content-derived validator均PASS，且`proposal_review.calibration_id == bridge.calibration_pack_id` |
| 冻结证据 | `forward_confirmation_plan_input_snapshot.v2`：两个full bundles/validations/lineage，以及`forward_confirmation_plan_v1.yaml` payload与原始bytes |
| target计算 | 遍历Bridge真实targets；reviewed semantic policy给出proposal-to-target binding；只有Proposal Review中真实存在的proposal命中时才投影该target |
| criteria计算 | `required_forward_events`、`windows`和`success_criteria`从Bridge target逐值复制；`_min`只转换成结构化`>=`，`_max`只转换成`<=`，不改变数值、不提供fallback |
| failure计算 | failure condition/action来自带owner/version/rationale/review condition的semantic policy；policy必须覆盖每个Bridge target，否则写件前fail closed |
| 空证据 | 空proposal或没有proposal命中Bridge target时，targets/triggers/failures均为空且`evidence_status=INSUFFICIENT_DATA`；不补造target、confidence或threshold |
| 输出 | manifest、targets、structured triggers、failure conditions、冻结快照、报告与Reader Brief；全部逐字节可重算，固定manual tracking/no registration/no forward run/no production |
| 当前结果 | focused fixture只有`limited_adjustment_vs_no_trade`被真实proposal解锁，因此只输出1个target；defensive未命中、consensus不在Bridge，均不会被补造。Workflow PASS只证明plan可追溯，不是forward success |
| 优化空间 | 等真实forward outcome ledger成熟后，可在下游预注册evaluation/sample maturity/uncertainty；新增target或proposal binding必须先改Bridge/semantic policy并做holdout验证，不能由Plan renderer发明 |

后续Confirmation Targets registry必须作为独立slice重新验证Plan；本环节不会自动注册、启动progress/evaluation、修改规则、portfolio、official weights、production或broker状态。

### 7.8 Confirmation Targets Registry（TRADING-174 / ARCH-004G2.4BY）

为什么这样设计：Registry是从immutable Plan到可持续tracking state的显式治理写入。若不重验Plan、假设固定三target、用0/MEDIUM补缺字段并直接覆盖全局YAML，旧的伪造target会成为后续progress事实。本环节因此把“注册什么、从哪里来、覆盖前是什么”全部冻结并可重放。

| 项目 | 当前定义 |
|---|---|
| 输入 | 显式`confirmation_plan_id/root`、artifact root、materialized registry path与timezone-aware cutoff；Plan validator必须PASS且manifest status=`AVAILABLE` |
| 冻结证据 | `confirmation_registry_input_snapshot.v2`保存Plan canonical文件的path/size/SHA-256 commitment、计算必需JSON views、validation、lineage、registry preimage、预期postimage与路径；不递归嵌入Plan snapshot |
| target计算 | target id必须非空唯一；只遍历Plan rows，source `TRACKING_REQUIRED`严格映射`active/in_progress`；priority/windows/events/success criteria/source row逐值保留，不提供default |
| failure计算 | 每个注册target必须有Plan source failure condition/action，unknown target、缺condition/action或无对应failure均fail closed |
| 重复与写入 | 当前materialized registry已绑定同一Plan时拒绝重复注册；所有artifact views先计算，artifact YAML和materialized YAML使用canonical atomic writer，latest pointer最后更新 |
| 输出 | manifest、registered targets YAML、input snapshot、report和materialized reviewable YAML；validator重验live Plan并逐字节重算JSON/YAML/report；若materialized仍指向本registry则必须完全一致 |
| 当前结果 | BX fixture只有1个source-backed limited target，因此Registry也只有1 active/0 watch-only；不会恢复历史defensive/consensus集合。PASS仅证明manual tracking state可追溯 |
| 优化空间 | 后续可把materialized supersede写成append-only registry event/decision ledger并加入trusted identity；但必须保持old artifact可验证、同Plan幂等阻断和显式owner mutation，不能自动运行progress |

Progress Tracker必须作为下一独立slice重新验证Registry；Registry PASS不代表样本增长、criteria达标、rule review readiness或production approval。

### 7.9 Confirmation Progress Tracker（TRADING-175 / ARCH-004G2.4BZ）

为什么这样设计：Progress是把静态Registry与随时间到达的forward evidence连接起来的观察层。旧实现读取mutable latest而不验证source，以target id切换分支并给events设10/5 fallback，还把同一event的多个window相加；missing metric和defensive pressure evidence也可能被0或普通regime样本替代。这会把“证据尚未到达”误写成“观察到零效果”，甚至提前解锁evaluation。本环节因此只使用validated、cutoff-bound、frozen evidence，并把event maturity与metric interpretation分开。

| 项目 | 当前定义 |
|---|---|
| 输入 | 显式`registry_id/root`、limited-vs-notrade/consensus-risk roots与timezone-aware cutoff；Registry必须content-derived PASS且不晚于cutoff |
| source选择 | 只为Registry真实targets加载所需source adapter；在cutoff内按`generated_at + artifact id`确定性选择zero-or-one latest，不使用mtime；被选artifact必须content-derived PASS，unknown target/source、duplicate identity、future/invalid source在写件前阻断 |
| 冻结证据 | `confirmation_progress_input_snapshot.v2`保存Registry与selected evidence全部canonical文件的path/size/SHA-256 commitments、计算必需views/validations、source roots、path/id和selection inventory；validator按同一cutoff重做selection并检测后来插入或任一文件byte drift，不递归复制上游snapshot |
| event计算 | `windows`与required event字段逐值继承Registry，不提供fallback；每个window只计`AVAILABLE`且核心metric finite的unique sample；总available用各window最大值作保守unique-event下界，不把同一event跨window重复相加 |
| metric计算 | 逐window保存validated source metrics；跨window success aggregate在尚无reviewed aggregation policy时保持null/`WINDOW_METRICS_ONLY`，不以0或任意平均替代。Defensive target没有真实pressure tag时available=0、metric null并明确`MISSING_PRESSURE_REGIME_TAGS` |
| status计算 | required满足、全部window有证据且无blocker才`READY_FOR_EVALUATION`；有样本为`IN_PROGRESS`，无样本为`INSUFFICIENT_EVENTS`。旧80% `NEAR_READY`阈值未受治理，已从实际判定移除 |
| 输出 | manifest、target JSONL、summary、input snapshot和Markdown；全部从live validated sources逐字节重算。只更新manual progress evidence/latest pointer，不运行Evaluation |
| 当前结果 | 当前Registry只有1个limited target；focused空source fixture得到0 unique event、四个window均0、scalar metrics为null、status=`INSUFFICIENT_EVENTS`，正确保持`ready_for_evaluation_count=0` |
| 优化空间 | source应进一步输出显式cross-window `event_id`、pressure regime governed taxonomy与criteria aggregation policy；达到预注册样本后再校准maturity/uncertainty。任何新增aggregate或near-ready band都必须进入reviewed policy并经holdout/误触发验证 |
...
| 冻结证据 | `confirmation_evaluation_input_snapshot.v2`保存Progress全部canonical文件的path/size/SHA-256 commitments、计算必需JSON/JSONL views、validation、path/id和registry lineage；validator逐文件重算，避免递归复制Progress snapshot |
...
### 7.11 Rule Review Cycle（TRADING-177 / ARCH-004G2.4CB）

为什么这样设计：Rule Review是把已冻结的tracking/evaluation证据整理成人工治理问题，而不是自动给出规则变更。旧实现不重验Registry、Progress、Evaluation，也不校验三者lineage/生成时间/target coverage；缺失target可经空mapping降为继续观察，FAILURE又按特定target id硬编码RENAME或TIGHTEN。报告还把Evaluation SUCCESS数量误标成“ready for evaluation”，混淆了样本成熟度和评估结果。本环节因此先验证并冻结完整三层链路，再只输出通用人工review readiness和source-owned actions。

| 项目 | 当前定义 |
|---|---|
| 输入 | 显式`registry_id/progress_id/evaluation_id`、三个artifact roots、output root与timezone-aware cutoff；三个content-derived validators必须在任何output前PASS |
| lineage gate | Registry/Progress/Evaluation manifest id必须分别匹配请求；Progress必须引用同一Registry，Evaluation必须引用同一Progress/Registry；生成时间满足Registry<=Progress<=Evaluation<=cutoff；三层target id非空唯一且集合完全相等，target/progress状态逐层一致 |
| 冻结证据 | `rule_review_cycle_input_snapshot.v2`保存三个`content_commitment_bundle.v1`、三个validation payload、roots/ids、cutoff和lineage。每个bundle覆盖全部canonical source file的path/size/SHA-256，另存计算必需views；validator逐文件重算，不把上游snapshot解析后递归嵌入 |
| decision计算 | watch-only=`KEEP_REFERENCE_ONLY`；active且NOT_READY=`CONTINUE_TRACKING`；MIXED/REVIEW_REQUIRED=`DEFER`；active SUCCESS或FAILURE都只输出`READY_FOR_OWNER_REVIEW`。规则只依赖source target/progress/evaluation状态，不读取target id进行语义分支，不生成新threshold |
| source action保留 | Matrix逐值携带criteria results、完整failure conditions、triggered conditions/actions与Evaluation recommendation；FAILURE不会被renderer替换成固定RENAME/TIGHTEN动作，owner只能看到source已登记的人工action |
| 报告计算 | 分别统计`progress_status=READY_FOR_EVALUATION`、Evaluation SUCCESS、FAILURE、MIXED、REVIEW_REQUIRED，避免把SUCCESS样本误称为尚待Evaluation；每个target同时披露progress/evaluation/decision/source actions |
| 输出 | manifest、decision matrix、input snapshot、Rule Review Markdown和Reader Brief section；validator重验live三源并逐字节重算三个JSON、两份Markdown和全部安全字段 |
| 当前结果 | source-backed fixture只有1个active limited target，Progress为`INSUFFICIENT_EVENTS`、Evaluation为`NOT_READY`，因此Cycle为`CONTINUE_TRACKING`、0 owner action。通用FAILURE单测使用任意新target id仍只输出`READY_FOR_OWNER_REVIEW`并保留`source_owned_manual_action`，证明不存在target-id硬编码 |
| 资源结果 | 首次实现沿用递归parsed+raw bundle，实测Registry/Progress/Evaluation snapshot约0.89GB/2.37GB/6.50GB并在CB序列化时OOM；改为bounded commitment后约18KB/28KB/14KB，Rule Review约37KB。全文件SHA-256承诺与live validator仍能拒绝三源漂移和五类output tamper，增长从指数降为线性 |
| 验证结果 | Rule Review 8、upstream regression 19、Dashboard/Queue 2均以parallel通过；累计focused 570、architecture-fitness 263、contract-validation 203。Architecture/contract runtime evidence分别为`outputs/validation_runtime/architecture-fitness_20260713T082125Z/test_runtime_summary.json`与`outputs/validation_runtime/contract-validation_20260713T082335Z/test_runtime_summary.json` |
| 优化空间 | Forward Plan之前的旧研究链仍存在约348MB深层snapshot，应在独立架构slice统一迁移为同一commitment envelope；当真实SUCCESS/FAILURE样本出现后，可增加review workload prioritization和uncertainty展示，但不得改变source criteria/action或自动创建Owner Decision |
...
日期窗口边界：命令必须记录实际requested `start/end`。后续
`pressure-outcome-backfill` active default 与全项目 primary research window 均从
`2021-02-22`开始；历史示例若实际请求`2022-12-01`必须原样保留并标成 historical comparison，
但该节点不再是必选 comparator、默认值或主要结论下限。
...
5. 主报告统一使用`2021-02-22` primary window；只有研究问题明确需要时才另列
   `2022-12-01` historical comparison，不能把可选比较节点重新变成主报告门禁；
   只有独立holdout与owner-reviewed change proposal通过后，才允许提出policy调整。
...
日期窗口继续分层解释：项目 active default 与 QQQ/SGOV/TQQQ 的
`exact_three_asset_validated` primary research window 均从`2021-02-22`开始。Deep Dive不自行改写
上游requested range，而是逐值继承Backfill/Compare已经声明的source mode、range和regime；历史产物若
实际从`2022-12-01`开始，只能作为原样可复现的 comparison，不得反向覆盖 active default。
...
5. 在Forward Pressure Capture积累真实样本后，以2021 primary window为主；只有明确研究问题需要时
   才另列2022 comparison并做leave-one-regime-out稳定性，不把该比较设成必选headline；
...
4. 统一按2021 primary research window累计和报告；历史`2022-12-01`产物只在显式比较任务中单列；
...
| Paper Init | reviewed active paper-shadow policy 与cutoff内唯一、validator PASS的Model Target；Target/config/source commitments在写件前验证 | active 起始日固定不早于`2021-02-22` unified primary window；每个tracked method用reviewed static baseline建立独立初始state，Model Target只证明全部method可用且冻结lineage，避免在paper start时偷看未来target weights | `paper_shadow_account_input_snapshot.v2`、immutable initial state、method states和报告。没有validated Target不得初始化，也不读取owner real portfolio；旧v1日期只用于历史artifact replay |
...
1. 用真实forward paper observations扩充共同日期样本，primary 统一从`2021-02-22`报告；
   历史`2022-12-01` fixture只在显式comparison中原样披露，且不得参与threshold调优；
...
| 环节 | 输入与门禁 | 计算逻辑 | 输出与解释边界 |
|---|---|---|---|
| Historical Backfill | reviewed active paper-shadow backfill policy、timezone-aware generated cutoff、cutoff内唯一validated Model Target及其linked policies、price/rates/DQ policy/cache sidecars；正式目录创建前运行统一DQ gate | 从Model Target读取真实method weights，不从Daily/Monitor latest或baseline补造；价格只保留全部required symbols共同finite且无duplicate的日期；按reviewed weekly calendar回放，每次rebalance将`transaction_cost_bps+slippage_bps`计入NAV；active config start默认`2021-02-22` | `paper_shadow_backfill_input_snapshot.v2`、manifest、calendar、method states、trade ledger、DQ JSON/Markdown和report；它是current-definition historical simulation，不是PIT replay；旧v1仍只用于历史artifact replay |
...
6. `2021-02-22`是统一 primary conclusion 起点；`2022-12-01`只在显式历史比较任务中单列，
   不再是必选headline或sample floor。
...
6. Window解释：本链后续 primary conclusion 从`2021-02-22`开始；既有`2022-12-01`证据表
   保持历史actual range，不得供应新run默认值。
...
### 7.20 Limited Adjustment 弱点诊断与方法细化（TRADING-224～228 / ARCH-004G2.4CK）

为什么这样设计：Hardening 的 `REVIEW_REQUIRED/LOW` 只说明现有方法未达到继续固化的证据要求，不能直接推出“应该加 risk cap”或“应该切换方法”。旧实现又允许 producer 读取未验证的 mutable latest、跨 Backfill/跨时间拼接；Risk Attribution 直接读 live cache 且把首日或缺失收益 `fillna(0)`，会把“没有观察”伪造成“零贡献”；Alternative/Proposal 可给未实现方法附带看似可比较的结论；validator 只检查文件存在和安全字段，不能证明输出仍来自当前输入。因此本层把“诊断—归因—修复计划—候选比较—提案”定义为五段 source-bound research chain，不在本 slice 调整策略，只把为什么这么判断、每一步输入输出和后续优化条件固定下来。

| 环节 | 输入与写前门禁 | 计算逻辑 | 输出与解释边界 |
|---|---|---|---|
| Limited Instability | 显式且 validated Backfill、同 Backfill 的 validated Limited Consistency 与 Rolling Evaluation；timezone-aware generated cutoff；Backfill 中 reviewed `method_refinement_v1` policy | 对每个非 `INSUFFICIENT_DATA` rolling window，以 `limited_adjustment` 对 static/no-trade 的 return、drawdown、risk-adjusted rank、turnover和区间内最大单symbol weight jump判断原因；severity、return/drawdown tolerance、turnover/jump thresholds全部来自policy。缺baseline/drawdown/risk-adjusted/jump保留具名unknown reason，不填0；再按regime tag汇总 dominant failure 与 risk-cap/regime-gate建议 | `limited_instability_input_snapshot.v2`、unstable window JSONL、reason summary、failure pattern、manifest和Markdown。它解释哪些窗口失败，不证明任一修复方案有效 |
| Limited Risk Attribution | 显式且 validated Backfill；Backfill `paper_shadow_backfill_input_snapshot.v2` 中的 bounded price rows、expected symbols、actual range、DQ/cache/config commitments；same reviewed policy | 价格pivot后只保留全部required symbols共同finite日期，`pct_change(fill_method=None)`使首日/缺失收益保持missing。Symbol return contribution为逐日 `weight[t,s] * return[t,s]` 的有限观察和，与static作差；drawdown contribution只在candidate实际最大回撤峰谷窗口内同样计算；exposure delta按policy risk/semiconductor symbols和CASH平均权重与static相减。20日risk event分别计算candidate-static drawdown、annualized volatility和turnover delta，阈值来自policy | `limited_risk_attribution_input_snapshot.v2`、return/drawdown contribution、exposure shift、risk events、manifest和Markdown。贡献是current-definition historical decomposition，不是严格因果估计；missing保持null，DQ/cache漂移使validator FAIL |
| Data Warning Repair Plan | 同 Backfill/Selection lineage 的 validated Data Warning Impact；source生成时间不晚于本次cutoff | 只把现有 warning inventory 映射为具名 repair action 和 blocking matrix；`PASS_WITH_WARNINGS`但无结构化明细时保留`pass_with_warnings_detail_unavailable`、`hardening_allowed_after_repair=UNKNOWN`。不读取或改写cache，不自动重跑Backfill | `data_warning_repair_plan_input_snapshot.v2`、actions JSONL、blocking matrix、manifest和Markdown。Plan PASS只表示计划可复算，不表示warning已修复 |
| Alternative Method Review | validated Backfill、同 Backfill validated Risk Attribution 与 Instability，且source时间不晚于本次cutoff；reviewed policy定义 existing/conceptual/primary methods | Existing methods从Backfill states计算 observation count、`V_end/V_start-1`、daily-return path drawdown、annualized volatility和turnover；candidate的stability继承Instability。Conceptual methods只记录hypothesis/expected benefit/cost，`observation_count=0`且所有performance metrics=null、expectations=`UNKNOWN`。推荐只按source-backed risk/instability evidence在policy允许的primary candidates中选择 | `alternative_method_review_input_snapshot.v2`、candidate catalog、scorecard、manifest和Markdown。`recommended_alternative`是下一项研究优先级，不是已实现method排名或promotion |
| Refined Method Proposal | validated Instability、Risk、Repair、Alternative；四源same Backfill，Alternative必须精确引用本次Instability/Risk，Repair必须保留Selection lineage，且满足Instability/Risk→Alternative→Proposal chronology；policy从Alternative snapshot原样继承 | 先尊重warning hard block；否则根据Instability recommendation和Alternative source-backed recommendation映射下一研究动作。Warning repair结果UNKNOWN时confidence固定LOW。只为policy primary methods列出research-only validation计划，不生成weights或方法实现 | `refined_method_proposal_input_snapshot.v2`、decision、proposed methods、owner checklist、report、Reader Brief section和manifest。`IMPLEMENT_*_RESEARCH_METHOD`只表示可另开研究实现任务，不能自动apply、promotion或进入broker/production |

五类snapshot都冻结canonical source path、size、SHA-256、上游validation、重算必需的source bundle、reviewed policy，以及Risk链的DQ/cache commitments。Producer先完成source validator、lineage、chronology、policy和有限值检查，再创建正式目录与latest pointer。每个validator重新读取live source/policy/cache/DQ、重跑source validator并逐byte重建snapshot之外的全部JSON、JSONL、Markdown和Reader Brief；source或policy drift、cache替换、snapshot/output tamper、future/naive time、cross-lineage、conceptual metric被补值或missing被升级都会FAIL。15个CLI callback的canonical owner为`interfaces/cli/etf_portfolio/dynamic_v3_system_target_refinement.py`，领域实现为`etf_portfolio/dynamic_v3_system_target_refinement.py`；legacy domain只保留lazy compatibility wrappers。

当前source-backed contract fixture（实际历史区间`2022-12-01..2024-02-29`）得到49个unstable windows；48个存在`drawdown_worse`，12个`turnover_high`，5个`weight_jump_linked`，2个`risk_adjusted_worse`，rolling status=`MIXED`，recommendation=`consider_risk_cap`。Return相对static的前三个正贡献为SOXX、QQQ、SMH；最大回撤窗口为`2023-02-03..2023-02-24`，前三个负贡献为QQQ、SMH、SOXX。Candidate平均risk/semiconductor/cash exposure分别为`0.756158/0.245984/0.158064`，相对static为`+0.055631/+0.045658/-0.041546`，故risk source=`mixed`；20日risk events共268个（153 volatility-higher、15 turnover-higher、100 mixed）。Data Warning仍为`REVIEW_REQUIRED`且repair后是否可hardening=`UNKNOWN`。Existing `limited_adjustment`有326个observations、total return=`0.372159`、max drawdown=`-0.035025`；四个conceptual methods的所有performance指标均为null。最终Proposal=`IMPLEMENT_RISK_CAPPED_RESEARCH_METHOD`、confidence=`LOW`，并把risk-capped/regime-gated列为research-only后续。以上数值只证明输入、计算和解释边界可复算，不是当前投资建议，也不证明risk cap或regime gate有效。

后续优化空间及进入条件：

1. 先补齐结构化DQ warning的id/symbol/date/severity/metric impact，并做repair前后paired rerun；`UNKNOWN`未解除前不得提高Proposal confidence。
2. 当前symbol contribution是权重乘当日收益的可审计decomposition，不是因果归因；只有加入time-aligned rebalance decision、cost/slippage、counterfactual weight path和tail attribution后，才可讨论“风险恶化由某symbol导致”。
3. 20日rolling risk events高度重叠；只有forward/PIT distinct events达到预注册floor后，才加入event clustering、block bootstrap、multiple-testing control和confidence interval，不能把268个重叠窗口当268个独立样本。
4. `method_refinement_v1`阈值目前是保持legacy语义的reviewed pilot baseline；任何调整必须另建ChangeProposal、冻结旧policy、预注册candidate/holdout、做parameter neighborhood、cost/risk/regime和untouched holdout验证，不能依据本fixture事后调参。
5. Conceptual methods只有实现独立versioned research method、通过historical/rolling/pressure/forward验证后才可填充metrics；不得用expected benefit替代实测结果。
6. 验证性能可用`source commitments + policy hash + validator version + cutoff`做可信缓存；命中仍需核对live commitments，不能恢复mtime/latest优先。
7. 本链后续默认 primary conclusion 自`2021-02-22`；旧`2022-12-01`结果只按historical
   comparison解释，不得成为必选比较或新样本下限。
...
7. 本链后续 primary/stress 研究统一从`2021-02-22`开始；旧artifact若实际从
   `2022-12-01`开始，只能作为historical comparison且不能覆盖新headline。
...
### 7.22 Weight Optimization Experiment Factory（TRADING-239～245 / ARCH-004G2.4CM）

为什么这样设计：本层不是再增加一个可以直接交易的方法，而是把“发现问题—提出假设—
低成本筛选—解释证据—决定是否值得正式实现”变成可复算的研究漏斗。旧实现有四个会让
研究结论失真的缺口：配置 FAIL 后仍可能创建正式 artifact；Matrix/Batch/Triage/Plan 没有
逐段证明 source、cutoff 和 exact lineage；Batch 以 `pct_change().fillna(0)` 把首日未知收益
伪造为 0，声明的 candidate subset 也未真正执行；Triage 会把零样本指标转成 0，并允许
“transform 实际一次都没改变 target”的 no-op variant 借相似收益进入 promotion；
Interpretation 又把 hypothesis 中的 expected benefit/cost 写成仿佛已观察到的结果。因此
G2.4CM 先固定证据语义和可审计边界，再陈述当前结果，不根据本次结果回调阈值。

| 环节 | 输入与写前门禁 | 计算逻辑 | 输出与解释边界 |
|---|---|---|---|
| Hypothesis Backlog | reviewed `weight_optimization_hypothesis_v1.yaml`；path、bytes、parsed payload、policy metadata 和 config validation 必须在写件前 PASS | 规范化 failure-mode taxonomy、hypothesis family、target failure modes、expected benefit/cost、complexity/priority；只对预先声明的 failure mode 做 coverage | `experiment_hypothesis_backlog_input_snapshot.v2`、manifest、taxonomy、hypotheses、priority summary、报告。它只说明“为什么值得测”，不是结果 |
| Variant Transform | reviewed `weight_variant_transform_v1.yaml`；transform vocabulary 必须与 allow-list 精确一致，每类 required fields/allowed modes 完整 | 形成可执行 transform catalog；拒绝 unknown type、missing field、invalid mode/range，不用运行时 fallback 猜测 | `experiment_variant_transform_input_snapshot.v2`、normalized YAML、catalog、manifest、报告。它定义操作语义，不新增 formal method |
| Experiment Matrix | Matrix、Hypothesis、Transform 三份 config 都要 PASS 并冻结；source backfill id、AI regime、candidate-selection/triage policy、safety 字段完整 | 将 15 个 hypothesis 与 transform 实例一一校验；实际冻结 method preference、cluster representatives、all-available set、`top_n`、score weights/bounds/labels/hard rejects/top cap | `experiment_matrix_input_snapshot.v2`、variant specs、transform specs、manifest、报告。Matrix 完整不代表任一 variant 有效 |
| Batch Experiment | validated Matrix 与其声明的同一 canonical Paper Backfill；Backfill DQ、config、price/rate cache commitments、generated chronology 和 requested range 必须一致 | 从 Backfill snapshot 取所有 required symbols 的共同 finite、无重复 price rows；收益严格为 `r_t=P_t/P_(t-1)-1` 并丢弃首日，不填 0。每个 variant 用上一日权重算当日组合收益，随后 drift，只有 source rebalance 日才从 reviewed candidate subset 取 target 并执行 transform；输出 return/drawdown/volatility/turnover、rolling/regime/stability，以及 transform effective rebalance count/max weight delta。regime 无样本时 metrics 为 null/`INSUFFICIENT_DATA` | `batch_experiment_input_snapshot.v2`、weight paths、performance/regime/stability JSONL、manifest、报告。它是 current-definition historical screening，不是 PIT 或 production backtest |
| Experiment Triage | validated Batch 与其精确引用的 Matrix；live source bundle、上游 validator、chronology、policy snapshot 必须一致 | return/drawdown/rolling/regime/turnover/simplicity 权重分别为 `0.25/0.25/0.20/0.15/0.10/0.05`，连续指标按 reviewed bounds 映射到 `[0,1]`，labels 按 config score。任一 required metric 缺失，或 `transform_effective_rebalance_count=0`，都加入 `insufficient_required_metrics` 并按 `missing_metric_decision=DEFER`；hard reject 不能被 composite score 覆盖；promotion candidates 最多 2 个 | `experiment_triage_input_snapshot.v2`、scorecard、promotion/rejected lists、summary、报告。`PROMOTE` 只表示可请求 owner 批准另开 formal-method 任务；`DEFER` 不是失败值 0 |
| Top Variant Interpretation | validated Triage 与它引用的同一 Matrix，source 不晚于 cutoff | 只解释 `PROMOTE`/`KEEP` rows；`expected_benefit_hypothesis`、`expected_cost_hypothesis` 原样保留为事前假设，`observed_screening_evidence`、`observed_screening_costs` 只从 scorecard 实测 delta/effective count 生成；failure-mode coverage 区分 `OBSERVED_SUPPORT`、`HYPOTHESIS_COVERAGE_ONLY`、`MISSING` | `top_variant_interpretation_input_snapshot.v2`、explanations、coverage、报告、Reader Brief section。不得把 expected 文字改写成 observed claim |
| Method Promotion Plan | validated Triage 与 Interpretation；Interpretation 必须精确引用同一个 triage id，且 candidate 必须被 interpretation 标记 recommended | 仅把 promotion candidate 转成 owner-controlled research-only method spec，列出后续 Paper Backfill、rolling、regime、stability、forward confirmation；没有 candidate 时输出 `DEFER` 和 `run_more_experiments_before_promotion` | `method_promotion_plan_input_snapshot.v2`、manifest、specs、implementation plan、owner checklist、Reader Brief。Plan 不实现 method、不修改任何策略或仓位 |

七类 snapshot 都保存 source/config bundle、content-derived validation、path/size/SHA-256、
timezone cutoff 和 safety boundary。Producer 必须在正式目录与 latest pointer 创建前完成门禁；
validator 重验 live config/source、重跑上游 validator，并逐 byte 重建 snapshot、JSON、JSONL、
YAML、Markdown 和 Reader Brief。Config drift、future source、cross-lineage、cache/source tamper、
输出篡改或 missing-to-zero 都会 FAIL。21 个 CLI callback 的 canonical owner 为
`interfaces/cli/etf_portfolio/dynamic_v3_system_target_experiment_factory.py`，领域实现为
`etf_portfolio/dynamic_v3_system_target_experiment_factory.py`；legacy domain 只保留 lazy
compatibility wrappers。

当前 source-backed contract fixture 的 requested AI-cycle window 为
`2022-12-01..2024-02-29`；因为 `2022-12-01` 只用于建立第一期收益的前值，实际可计算
return window 为 `2022-12-02..2024-02-29`，这不是把结论窗口偷偷改成 2021，也不是把首日
补成 0。DQ=`PASS_WITH_WARNINGS`，15 个 variants 全部完成；90 个 variant-regime rows 中
45 个为 0 样本并保持 null。Triage 结果为 `0 PROMOTE / 3 KEEP / 7 REJECT / 5 DEFER`。
当前最高为 `sideways_choppy_signal_persistence_3d`，score=`0.656744`，低于 reviewed
promotion floor `0.70`，因此仅 `KEEP_FOR_MORE_TESTING`：相对 limited return delta=
`-0.019496`、drawdown delta=`+0.001951`、turnover delta=`-0.153148`、rolling=
`IMPROVED`、effective rebalances=`53`。`risk_off_only_allow_risk_reduction` 与
`cluster_representative_consensus` 也只进入 KEEP。`tech_drawdown_block_risk_increase`、
3d/5d smoothing、semiconductor block 和 cash buffer 在本 fixture 的 source rebalance targets
上 0 次生效，故全部 DEFER；它们之前出现的微小路径差异不能证明 transform 有效。Promotion
Plan 因此为 `DEFER`、methods=`[]`，这是诚实的研究结果，不应通过放宽阈值制造候选。

后续优化空间及进入条件：

1. 先解释 5 个 no-effect variants：区分 fixture target 本就满足约束、目标序列不变化、
   regime 未出现和 transform 实现语义问题；修复必须新增 versioned hypothesis/config 和
   pre-registered activation coverage，不得复用当前 holdout 调到“刚好生效”。
2. 当前 top KEEP 以约 1.95% return cost 换取 drawdown/turnover/rolling 改善；正式研究前需要
   cost/slippage、non-overlapping windows、event clustering、block bootstrap 和 untouched
   holdout，不能只看 composite score。
3. 45 个 0-sample regime rows 需要 PIT/forward distinct events 或预注册 stress cohort；
   不得跨 regime 借样本或填 0。后续 primary conclusion 统一从`2021-02-22`开始；旧
   `2022-12-01`窗口只保留其历史actual-range语义。
4. Candidate preference 目前是 reviewed pilot baseline，不是由数据证明的最优 ranking；只有
   新证据和 owner-approved ChangeProposal 才能修改 method set、aggregation 或 `top_n`。
5. Score bounds、label scores、hard-reject thresholds 和 `0.70/0.55` floors 必须做 frozen-old
   vs proposed-new 并行 replay、敏感性与 expiry review；本结果不得作为事后调阈值依据。
6. DQ warning 未消除、current-definition/not-PIT 和 forward confirmation 未完成前，任何
   `KEEP`/未来 `PROMOTE` 都不能提升为策略优越、投资建议或 production evidence。

本 slice 固定 manual/current-definition historical、`not_pit_safe=true`、
`not_official_target_weights=true`、`auto_apply=false`、`broker_action_allowed=false`、
`production_effect=none`。定期复盘可以提出新的 Observation/ChangeProposal，但不能直接改
policy、method、official weights、paper/real portfolio、production、order 或 broker。

### 7.23 Smoothed Limited Adjustment（TRADING-246～250 / ARCH-004G2.4CN）

为什么这样设计：本链验证“将 `limited_adjustment` 权重路径做 3d/5d 有限平滑，是否能以可接受的收益、回撤和 lag 代价降低 turnover / weight jump”，不是预先选出 3d 或 5d 胜者。旧 baseline 会读取未冻结的 Model Target/config，Smoothed、Baseline、Risk 可以来自不同 Backfill，重复 method rows 会改变样本权重，missing/zero-sample 又可能经数值默认值变成 0；Review 还固定把 3d/5d 写成 recommended/secondary，并用散落在代码中的阈值判断。这些问题使“产物生成成功”可能被误写成“方法更优”。G2.4CN 因而把五段都改为写前验证、same-lineage、null-preserving、policy-governed、content-derived 链路。

| 环节 | 输入与写前门禁 | 计算逻辑 | 输出与解释边界 |
|---|---|---|---|
| Smoothed Config | reviewed `smoothed_limited_adjustment_v1.yaml` 与其引用的 Model Target policy；policy owner/version/status、method set、alpha/change caps、context overrides、lag diagnostics、`evaluation_policy` 和 safety fields 完整 | 规范化 3d/5d smoothing policy；验证 candidate priority 只包含已启用方法，return/drawdown/turnover/jump/lag/sample floors 均为有限、合法的 reviewed pilot policy | `smoothed_config_input_snapshot.v2`、manifest、normalized YAML、report。只定义研究方法和判断口径，不生成权重或结论 |
| Smoothed Target | 显式且 validated Model Target、validated Smoothed Config；target/config/model policy 均不晚于 timezone-aware cutoff，source path/size/SHA-256 与 id 精确冻结 | 对每一 source target 计算 `w_t=w_(t-1)+alpha*(base_t-w_(t-1))`，再应用 single-symbol / total-change cap 和既有硬约束；3d/5d 的差异来自 policy，不改变信号或候选选择 | `smoothed_target_input_snapshot.v2`、smoothed weights、smoothing/lag events、jump summary、manifest/report。不是 official target weights，不自动选择方法 |
| Smoothed Backfill | reviewed Paper Backfill config、price/rates/DQ cache、cutoff内 Model Target/config；允许显式编排一次 canonical Paper Backfill，并在任何 Smoothed/Risk 正式输出前完成其 validator | 从同一个 canonical Paper Backfill 派生 smoothed states，同时生成同 lineage 的 risk-capped projection；每个 method/date 唯一，状态日期区间与 baseline 完全一致。Smoothed 只改变 target path，NAV/turnover沿用 validated historical paper semantics | `smoothed_backfill_input_snapshot.v2`、method states、trade ledger、summary、report，并返回 `paired_risk_capped_backfill_id`。current-definition historical、not-PIT；不是 forward 或 production evidence |
| Smoothed Comparison | validated Smoothed、Baseline、Risk artifacts；三者必须引用同一个 baseline Backfill id、相同 requested range、合法 chronology，method/date 不重复 | 只在同 method pair 的共同 finite return dates计算 `total/annualized return`、drawdown、volatility、turnover与jump delta；regime不足保持null/`INSUFFICIENT_DATA`；rolling用真实window inventory，存在窗口但rank频率差为0时为`MIXED`；lag只在真实lag样本上计算 | `smoothed_comparison_input_snapshot.v2`、overall/regime/rolling/stability/lag JSON、manifest/report。任一“better”只是当前 pilot metric组合，不是显著性或promotion结论 |
| Smoothed Review | validated Comparison与它精确引用的 Smoothed Backfill；同一 frozen `evaluation_policy`，source chronology与lineage完整 | 对每个候选独立检查 minimum path/regime/rolling samples、return preservation、drawdown、turnover、jump、rolling、stability与lag。只有完整且全部 hard criteria 通过才可进入 promotion eligibility；排序仅在 eligible methods 中按 reviewed priority打破并列。没有 eligible method时 recommended/secondary 都为null | `smoothed_review_input_snapshot.v2`、candidate evidence、decision、owner checklist、report/Reader Brief。只能继续观察/缺证据，不能自动改policy、切换method或写production |

五类 snapshot 都保存 source/config/cache bundle、path/size/SHA-256、timezone cutoff、上游 validation、exact lineage 与重算所需有限 views。Producer 在正式目录和 latest pointer 前完成门禁；validator 重新读取 live source/config/cache、重跑上游 validator，并从 snapshot 逐 byte 重建 JSON、JSONL、YAML、Markdown 与 Reader Brief。Source/policy/cache drift、future或naive时间、cross-lineage、duplicate method/date、non-finite、missing-to-zero、snapshot/output tamper 都会 FAIL。15 个 CLI callback 的 canonical owner 为 `interfaces/cli/etf_portfolio/dynamic_v3_system_target_smoothed_method.py`，领域实现为 `etf_portfolio/dynamic_v3_system_target_smoothed_method.py`；legacy domain 只保留 lazy compatibility wrappers。

当前 source-backed contract fixture 的 AI-cycle requested/state range 为 `2022-12-01..2024-02-29`，不是 2021 context；63 次 rebalance，3d/5d 各 315 个 smoothing events 与 11 个 lag events，共同 return observations=326，DQ=`PASS_WITH_WARNINGS`。3d 相对 limited 的 return/drawdown/turnover/jump delta 为 `-0.001224/-0.000065/-0.096224/+1`，5d 为 `-0.001779/-0.000017/-0.115170/-1`。两者 rolling 均为 `MIXED`；stability 为 3d=`MIXED`、5d=`IMPROVED`，lag 均为 `LOW`。在当前 reviewed policy 下，两者 drawdown 都未达到 non-worsening floor，3d 还增加一次 large jump，因此 decision=`CONTINUE_OBSERVATION/LOW`，recommended_method=`null`、secondary_method=`null`。旧 baseline 中固定的 3d/5d method roles 已撤销；workflow PASS 只证明这套计算和证据边界可复算。

后续优化空间及进入条件：

1. 先补 forward/PIT distinct observations，并按`2021-02-22`统一 primary window报告；仅在明确
   需要复现旧结论时单列`2022-12-01`comparison，不得让它参与新 primary floor。
2. DQ 仍为 `PASS_WITH_WARNINGS`；先补结构化 warning、repair-before/after paired replay、corporate-action 与 session-calendar coverage，未消除前不得提高 confidence。
3. 对 3d/5d 做 versioned cost/slippage、turnover budget、tail loss、non-overlapping window、event clustering、block bootstrap、leave-one-regime-out 与 untouched holdout；不能把 49 个重叠 rolling windows当独立样本。
4. 当前 drawdown 变差幅度很小但为负；只有独立证据达到预注册 floor 后，才可提出带 rationale/expected effect/expiry 的 ChangeProposal 调整 non-worsening floor，不能按本 fixture 事后放宽为可推荐。
5. Candidate priority 只用于 eligible methods 并列，不是性能权重。若未来增加 hybrid/regime-gated method，必须新建 versioned hypothesis/config、activation coverage、ablation 与 comparison，不能在现有 id 下暗改。
6. 深链 validator 可演进为以 `source commitment + validator version + cutoff` 为键的可信缓存，但命中仍必须核对 live commitment，不能删除 fail-closed gate。

本 slice 固定 manual/current-definition historical、`not_pit_safe=true`、`requires_forward_confirmation=true`、`not_official_target_weights=true`、`auto_apply=false`、`broker_action_allowed=false`、`production_effect=none`。定期复盘只能形成 Observation / ChangeProposal，不能自动修改 policy、method、weights、portfolio、production、order 或 broker。

### 7.24 Smoothed Evidence Drilldown 与 Forward Confirmation（TRADING-251～255 / ARCH-004G2.4CO）

为什么这样设计：G2.4CN 已证明 3d/5d 两种方法都能降低 turnover，但没有任一方法同时通过 drawdown、jump、rolling、stability 与 lag 的全部 promotion criteria。旧 TRADING-251～255 实现仍把 3d 当隐含主方法：Benefit/Lag 只用固定字段描述、Confirmation 无条件登记四个 3d targets、Watch 再把这些角色写回摘要；同时五段产物只依赖磁盘路径，缺少完整 source commitments、cross-lineage 拒绝、live replay 和 policy drift 检查。这样会把“某个局部 tradeoff 看起来不错”偷换成“3d 已值得 forward confirmation”，也无法解释产物是否来自同一 Review/Backfill。G2.4CO 因而把链路改为逐方法归因、唯一 eligible candidate 驱动、无候选零 targets、全链 content-derived replay。

| 环节 | 输入与写前门禁 | 核心计算逻辑 | 输出与可解释边界 |
|---|---|---|---|
| Review Attribution | validated Review、它引用的 Comparison 与 Smoothed Backfill；Comparison→Smoothed→Baseline id、requested range、generated chronology、config policy 均一致 | 直接读取 Review 的 candidate evidence，对每个 method 保留 return/drawdown/turnover/jump/rolling/stability/lag 状态，并附对应 raw delta；supporting/blocking/why-not reasons 只能从 frozen decision 和 evidence 推导 | `smoothed_attribution_input_snapshot.v2`、reason breakdown、per-method support matrix、report。只解释既有 decision，不重选方法 |
| Benefit / Lag | validated Comparison 与其 exact Smoothed/Baseline full bundles；`minimum_path_observations` 与 benefit floor 来自 reviewed `evidence_policy` | `jump_reduction=-large_jump_delta`、`turnover_reduction=-turnover_delta`、`churn_reduction=-weight_flip_delta`；按三类 reduction 与 rolling status 得到 `STRONG/MODERATE/WEAK`，再与 lag `LOW/MEDIUM/HIGH` 组合为 tradeoff。缺任一 metric、non-finite 或样本不足时保持 null/`INSUFFICIENT_DATA`，不转成 0 | `smoothing_benefit_lag_input_snapshot.v2`、benefit/lag/tradeoff per-method JSON 与 report。`FAVORABLE` 只表示当前 pilot 指标组合，不覆盖 Review eligibility |
| Regime Validation | validated Smoothed Backfill 与其 exact source Paper Backfill；reviewed regime label policy、sample floors、sideways/recovery thresholds | 用同日期 paired states 计算 return/drawdown；sideways 额外计算 turnover、weight flip 与 large-jump delta；recovery 计算 risk-on response delay 和 missed upside。只有样本 floor 满足且所有 required metrics finite 才分类，否则全部相关 metric 保持 null | `smoothed_regime_validation_input_snapshot.v2`、sideways/recovery summaries、report。历史 regime 结果不等于独立 forward evidence |
| Confirmation Registration | validated Review 与同 comparison/backfill lineage 的 Regime Validation；Review decision、candidate evidence 和 policy 重验 | 只有 `recommended_method` 非空、在 candidate evidence 中唯一且 `promotion_eligible=true` 时，才用 reviewed 1/5/10/20 日 windows、required forward/sideways/recovery event floors 为该方法生成 targets；否则输出空数组 | `smoothed_confirmation_input_snapshot.v2`、manifest/targets/report。Workflow artifact 可 PASS，但研究 status 可为 `INSUFFICIENT_EVIDENCE`；不得补造固定 3d targets |
| Weekly Watch | validated Attribution、Benefit/Lag、Regime、Confirmation，四者的 review/comparison/backfill ids 必须完全一致且 chronology 合法 | 以 Confirmation/Review 的实际 candidate 为唯一 join key；有候选才展示它的 tradeoff/regime/forward status，无候选则分别为 `INSUFFICIENT_EVIDENCE` / `NOT_REGISTERED`，recommended action 保持继续观察 | `smoothed_watch_input_snapshot.v2`、summary、owner checklist、report/Reader Brief。不是 owner approval、promotion 或 scheduler 自动调优 |

五类 snapshot 均保存 source/config full bundles、path/size/SHA-256、上游 validator 结果、timezone cutoff 和 `evidence_policy_metadata`。Producer 在创建正式目录前完成 live source validation；validator 重新读取 live sources/config，从 snapshot 逐 byte 重建所有 JSON、Markdown、checklist 与 Reader Brief。Policy/source drift、future/naive time、cross-review/comparison/backfill lineage、missing-to-zero、render tamper 或不同链路拼接均 FAIL。15 个 CLI callback 的 canonical owner 是 `interfaces/cli/etf_portfolio/dynamic_v3_system_target_smoothed_evidence.py`，领域 owner 是 `etf_portfolio/dynamic_v3_system_target_smoothed_evidence.py`；legacy 仅保留 lazy compatibility wrappers。

当前 source-backed fixture 的 AI-cycle requested/state range 仍为 `2022-12-01..2024-02-29`，未改成 2021 primary conclusion window。Benefit/Lag 中 3d 与 5d 都有 326 个共同 return observations：3d 的 jump/turnover/churn reduction 为 `-1/+0.096224/+82`，5d 为 `+1/+0.115170/+98`；两者 benefit=`MODERATE`、lag=`LOW`、tradeoff=`FAVORABLE`。Sideways 有 283 个 paired observations：3d return/drawdown/turnover/churn/jump delta=`+0.000225/0/-0.100010/-130/+1`，status=`MIXED`；5d 为 `+0.000226/0/-0.112649/-160/-1`，status=`IMPROVED`。Recovery 仅 7 个 observations，两者 response delay=2天、missed upside=0、lag=`LOW`。这些局部证据没有改变上游 drawdown hard criterion：Review 仍为 `CONTINUE_OBSERVATION/LOW`，recommended/secondary 均为 null，所以 Confirmation=`INSUFFICIENT_EVIDENCE`、`candidate_method=null`、`targets=[]`，Watch forward=`NOT_REGISTERED`。这正是候选驱动设计应产生的结果，而不是错误缺件。

后续优化空间及进入条件：

1. 先补独立 forward/PIT event observations；默认结论窗口统一为`2021-02-22`起的
   `unified_primary_2021`。`ai_after_chatgpt=2022-12-01`只在显式历史比较时单列，不能反向成为
   primary sample floor或headline门禁。
2. Recovery 当前只有 7 个 observations，未达到 forward recovery floor=5 虽可做历史分类，但重叠/聚类使有效独立样本更少；需 event de-clustering、non-overlapping windows、block bootstrap 与 untouched forward holdout。
3. `FAVORABLE` 来源主要是 turnover/churn reduction，不能替代 drawdown hard block。只有预注册的新证据显示 drawdown non-worsening，才可通过 ChangeProposal 复审 threshold；不得用本 fixture 事后放宽。
4. Sideways 的 3d 因 jump delta `+1` 仍为 `MIXED`，5d 虽为 `IMPROVED` 但 Review drawdown仍不合格；应补 tail loss、cost/slippage sensitivity、regime transition lag、event clustering 与 source-quality repair-before/after paired replay。
5. Confirmation targets 只在唯一 eligible candidate 出现时注册。未来若多个方法 eligible，必须由 reviewed deterministic tie-break policy 给出唯一 recommendation；否则继续零 targets，不能由 Watch 或 CLI 临时选一个。
6. Weekly Watch 只允许进入统一 periodic orchestration 的 date/condition gate，不新增独立 scheduler；定期复盘只生成 Observation/ReviewDecision/ChangeProposal，owner approval 后才进入下一验证阶段。
7. 深链 live replay 可在后续架构重构中引入以 commitment+validator version+cutoff 为键的可信缓存，但缓存命中仍要验证当前 path/size/SHA-256 和 policy version，不能放松 fail-closed 语义。

本 slice 固定 manual/current-definition historical、`not_pit_safe=true`、`requires_forward_confirmation=true`、`research_target_only=true`、`not_official_target_weights=true`、`auto_apply=false`、`broker_action_allowed=false`、`production_effect=none`。局部 tradeoff、workflow PASS 或 Watch cadence 都不能自动修改 method/policy/weights/portfolio/production、生成 order 或触发 broker。

### 7.25 Smoothed Evidence Completeness 与 Promotion Readiness（TRADING-256～260 / ARCH-004G2.4CP）

为什么这样设计：G2.4CO 已把真实上游结论收口为 `candidate_method=null`、
Confirmation 0 targets、Watch forward=`NOT_REGISTERED`。旧 CP 链却固定 3d 为
primary、5d 为 secondary，把 missing metric 转成 0，并给 `INSUFFICIENT_DATA`、
`NOT_REGISTERED` 正分；即使没有候选也会从两方法中选出 recommended/secondary，甚至
输出 `PROMOTE_FOR_REVIEW`。此外 Churn 可拼接不同 baseline 的 Smoothed/Risk artifacts，
缺 deltas 时用 `turnover*2` 静默补值；Sideways、Scorecard、Owner 也不验证共同
Review/Comparison/Backfill lineage。G2.4CP 因而把“诊断分数”和“候选授权”彻底分开：
诊断逐方法计算，但 candidate 的唯一 authority 是 validated Confirmation；分数不能反向
创造候选。

| 环节 | 输入与写前门禁 | 核心计算逻辑 | 输出与解释边界 |
|---|---|---|---|
| Evidence Gap | validated Benefit/Lag、Regime、Watch；三者 Smoothed/Baseline 相同，Benefit/Watch Comparison 相同，cutoff/chronology 合法 | 对每个方法分别检查 weight-jump reduction、signal-churn reduction、sideways samples、recovery lag；finite 且 source status 有效才为 `AVAILABLE`。若 Watch candidate=null，另列 `eligible_candidate` 与 `forward_confirmation` blocking gap | `smoothed_evidence_gap_input_snapshot.v2`、per-method matrix、reason summary、backfill plan、report。当前 direct churn/jump 已 available，所以 `tradeoff_can_be_resolved_by_backfill=false`；真正 gap 是无候选/无 forward target |
| Churn Backfill | validated Smoothed/Baseline/Risk backfills；三者 `source_paper_shadow_backfill_id`、requested range 一致；method/date 唯一、ordered、finite；reviewed readiness policy | 每个 ledger row 计算 `total_abs_weight_change=sum(abs(deltas))`，并验证 `turnover=0.5*total_abs_weight_change`。只有 turnover=0 时允许空 deltas，来源标记 `explicit_zero_turnover`；非零 turnover 无 deltas 或恒等式不符立即失败。Jump 由 policy threshold 分类；direction flip 只从显式 symbol deltas 的相邻非零方向变化生成；relative metrics 与 limited 同口径相减，missing 保持 null | `smoothed_churn_backfill_input_snapshot.v2`、method metrics、jump/flip events、reduction summary、report。`best_churn_reduction_method` 仅为 diagnostic，不能成为 candidate |
| Sideways Attribution | validated Regime 与 Churn；Smoothed/Baseline ids 一致；Churn snapshot 中 committed states/config 可重算 regime labels | 取 `sideways_choppy` 日期并按月形成窗口；对每方法与 limited 做 exact-date pair。样本达到 `minimum_window_observations` 才计算 compounded return、max drawdown、turnover 与 jump+flip event delta；再用 reviewed return/drawdown/turnover/churn floors 分类 `improved/worse/mixed`，不足时所有指标 null | `sideways_mixed_attribution_input_snapshot.v2`、window outcomes、per-method reason summary/breakdown、report。`preferred_sideways_method=null`，selection 延后给 Confirmation candidate |
| Readiness Scorecard | validated Attribution、Benefit、Churn、Sideways、Confirmation；Attribution/Confirmation Review 相同，Attribution/Benefit/Confirmation Comparison 相同，五者 Smoothed/Baseline 相同，Sideways→Churn 精确绑定 | return/drawdown 来自 Attribution status；turnover/jump/churn 来自 direct reductions；sideways 来自 per-method status；lag 来自 Benefit/Lag；forward 只看该方法 registered targets。所有 mapping/weights/hard blocks/0.75/0.45 boundaries 来自 reviewed policy。`INSUFFICIENT_*` 与 `NOT_REGISTERED` 无 score；任一 component missing 时 overall=null。Decision 只读取 Confirmation candidate 对应行 | `smoothed_readiness_scorecard_input_snapshot.v2`、per-method diagnostics、candidate-aware decision、report。candidate=null 时 recommended/secondary=null、`CONTINUE_OBSERVATION/INSUFFICIENT_EVIDENCE`，禁止 `PROMOTE_FOR_REVIEW` |
| Owner Review Update | validated Scorecard 与 Watch；Review/Comparison/Smoothed/Baseline 和 candidate 完全一致 | 只有 candidate 非空、readiness=`PROMOTE_FOR_REVIEW`、forward=`PASS`、blocking为空时 promotion option 才可 recommended。无候选时唯一推荐为 `request_additional_evidence`；promotion 保持 false | `smoothed_owner_review_update_input_snapshot.v2`、options、checklist、report、Reader Brief。人工研究输入，不是 owner approval event，更不触发 policy/weights/portfolio/order/broker |

五类 snapshots 都保存完整 source bundle、path/size/SHA-256、上游 validator payload、
timezone cutoff 和 policy binding。Producer 在创建正式目录前完成 live validation；validator
重新读取 live sources/config，重跑 lineage/chronology/finite/identity gates，并从 snapshot
逐 byte 重建全部 JSON、JSONL、Markdown、checklist 和 Reader Brief。Source/config drift、
cross-lineage、duplicate method/date、non-finite、missing-to-zero、turnover identity 伪造、
无候选 promotion 或 output tamper 均 FAIL。15 个 CLI callbacks 的 canonical owner 为
`interfaces/cli/etf_portfolio/dynamic_v3_system_target_smoothed_readiness.py`，领域 owner 为
`etf_portfolio/dynamic_v3_system_target_smoothed_readiness.py`；legacy domain 只保留 lazy
compatibility wrappers。

当前 source-backed fixture 继续使用 `ai_after_chatgpt` 的
`2022-12-01..2024-02-29` requested/state range；2021 历史仅可作为另列的长期 context，
不能取代默认 AI-cycle headline。Direct churn/jump 已可从显式 ledger deltas 重算，Gap
不再声称需用 backfill 解决；但上游仍无 eligible candidate，所以两个方法的 forward
component 都为 missing、overall readiness score 都为 null。最终 Scorecard 为
`CONTINUE_OBSERVATION/INSUFFICIENT_EVIDENCE`、recommended/secondary=null，Owner 推荐
`request_additional_evidence`，promotion option=false。这是当前证据的正确结果，不是流水线
失败。

后续优化空间及进入条件：

1. 先补独立 forward/PIT、DQ repair-before/after、versioned cost/slippage、holdout 与
   event de-clustering；只有 Review 产生唯一 `promotion_eligible` recommendation 才登记
   Confirmation targets，不能直接修改 CP score。
2. Readiness weights/status mappings/thresholds 是 versioned pilot policy。独立样本达到预注册
   floor 后才能通过 ChangeProposal 重新校准；不得依据当前 fixture 事后调权以制造 PASS。
3. Direction flip 目前把 symbol 非零 delta 方向翻转作为透明事件定义；如未来需要 persistence、
   hysteresis 或 materiality floor，必须新增 policy version、ablation 和 sensitivity，不在同一
   artifact id 下暗改。
4. Monthly sideways windows 存在时间重叠和事件相关性；后续应增加 non-overlapping windows、
   block bootstrap、leave-one-regime-out 与 untouched forward holdout，不能把窗口数当独立样本数。
5. 深链 live replay 当前正确但成本较高；可引入以 source commitment、validator version、
   cutoff、policy version 为键的可信 validation cache。Cache 命中仍必须核对 live
   path/size/SHA-256，任何 drift 必须失效，不能为了速度跳过 fail-closed gate。

本 slice 仍为 manual/current-definition/not-PIT research，`research_target_only=true`、
`paper_shadow_only=true`、`not_official_target_weights=true`、`auto_apply=false`、
`broker_action_allowed=false`、`production_effect=none`。Workflow PASS、局部 churn 改善或
...
本链的 active primary conclusion window 统一从 `2021-02-22` 开始。这里保留的
`ai_after_chatgpt=2022-12-01` 仅描述 immutable historical contract fixture 的实际请求范围，
不再是 active default、required comparator 或 minimum start。该历史 fixture 中 candidate=null，
...
### 7.27 Smoothed Forward Operations 与 Owner Renewal（TRADING-266～270 / ARCH-004G2.4CR）

为什么这样设计：G2.4CQ 的当前 source truth 是 candidate=null、Binding=`NOT_REGISTERED`、
targets=[]、Switch proposed candidate=null。旧 CR consumer 却继续固定跟踪 3d candidate、3 个
targets 与 10/5/5 requirements，并允许从目录中扫描未绑定的 outcome/classification；这样会把
历史 fixture 假设重新包装成当前 forward evidence。G2.4CR 因而把 Forward Progress 作为唯一
运行 lineage，Dashboard 与 Monitor 只投影同一 Progress，Recheck 只组合同一 candidate/Progress
的 Dashboard、Monitor 与 CQ Switch，Renewal 只组合该 Recheck 与 CQ Owner journal。没有真实
candidate 时，空 requirements、空 events 与 `NOT_REGISTERED` 是正确业务结果，不是工程失败。

| 环节 | 输入与写前门禁 | 核心计算逻辑 | 输出与解释边界 |
|---|---|---|---|
| Forward Progress | validated CQ Forward Binding；可选 outcome/classification id 必须显式提供、live validation PASS、target/candidate/exact lineage 一致且 generated time 不晚于本次 snapshot | candidate 与 targets 只取 Binding；逐 target 汇总已绑定 evidence commitments、实际 forward event 数和 requirement 完成度；0 targets 时不生成默认 requirement、target row 或 event | `smoothed_forward_progress_input_snapshot.v2`、target progress、evidence commitments、summary、report/Reader Brief。当前 candidate=null、0 targets、`NOT_REGISTERED`、建议 `request_more_forward_data` |
| Weekly Dashboard | validated Progress；source path/hash、progress id、candidate 与 chronology 全部重验 | 只从 Progress 的实际 target rows、status、evidence counts 和 recommendation 生成 dashboard summary/status table；不重跑研究、不扫描其他 artifact | `smoothed_weekly_dashboard_input_snapshot.v2`、summary、status table、report/Reader Brief。当前 candidate=null、`NOT_REGISTERED`、`ready_for_switch_recheck=false` |
| Event Monitor | validated Progress；只读取 Progress 已承诺的 exact classification/outcome sources | 将同一 Progress 中真实绑定的 sideways/recovery event 按 commitment 投影成 inventory 与 accumulation；没有 commitment 时保持空，不跨目录或跨 lineage 搜索 | `smoothed_event_monitor_input_snapshot.v2`、sideways/recovery inventories、accumulation summary、report。当前 inventories=[]、requirements=[]、`NOT_REGISTERED` |
| Switch Readiness Recheck | validated Dashboard、Monitor、CQ Paper-shadow Switch；三者 candidate 与 Progress lineage 必须精确一致，chronology 合法 | 由实际 dashboard readiness、event accumulation、CQ switch eligibility 生成 criteria、blockers 与 decision；candidate=null 时 criteria 为空并显式加入 `no_eligible_candidate`，不伪造 owner gate | `smoothed_switch_readiness_input_snapshot.v2`、decision/criteria/report/Reader Brief。当前 `NO_ELIGIBLE_CANDIDATE`、`owner_decision_required=false`、`can_execute_switch=false` |
| Owner Renewal | validated Recheck 与 CQ Owner Promotion journal；candidate、previous decision、chronology 与 source commitments 重验 | 只对当前可用动作生成 options；promote 的 availability 由真实 candidate/recheck 决定，推荐动作从 blockers 与 previous decision 派生 | `smoothed_owner_renewal_input_snapshot.v2`、options/checklist/report/Reader Brief。当前 promote unavailable、推荐 `request_more_forward_data`，不执行 switch |

五类 snapshot 采用 bounded source contract：保留所有直接 source 文件的 path、size、SHA-256、
schema/id/lineage/cutoff 与 validation payload，只冻结本环节实际消费的业务 view，不把上游
`*_input_snapshot.json` 正文再次嵌入 `bundle.json`。Producer 写出前仍运行完整 live upstream
validator，validator 仍按 snapshot 逐 byte 重建所有 JSON/JSONL/Markdown/Reader Brief；任何
source byte、lineage、chronology 或派生 view drift 都必须 FAIL。

工程效率优化采用进程内 content-fingerprint validation session。Cache key 包含 validator
identity、绝对 artifact path 与该 artifact 直接文件清单的完整内容指纹；只缓存 PASS，返回
deep copy，任一文件 byte 变化自动 miss，FAIL 永不缓存。它只消除同一次深链中对未变化 source
的重复验证，不替代 validation，也不跨进程持久化。基准使用同一
`tests/test_smoothed_forward_progress.py`：优化前 557.27 秒，加入缓存后 122.66 秒；再移除递归
snapshot body 后 13.60 秒，总体缩短 97.56%、约 40.98 倍。最大 source snapshot 从约
633.06 MB 降至约 9.49 MB（约 98.5%），同时 cache mutation/tamper、FAIL-not-cached、五阶段
业务语义与全链 source tamper 测试均通过。

本链的 active primary conclusion window 统一从 `2021-02-22` 开始。这里保留的
`ai_after_chatgpt=2022-12-01` 仅描述 immutable historical contract fixture 的实际请求范围，
不再是 active default、required comparator 或 minimum start。该历史 fixture 没有 eligible
candidate 或 registered targets，Progress/Dashboard/Monitor 保持 `NOT_REGISTERED`，Recheck
保持 `NO_ELIGIBLE_CANDIDATE`，Renewal 推荐继续补 forward data。Workflow artifact PASS 只说明
结构、来源和计算可复核，不代表证据充足、方法晋升或策略有效。

后续优化空间及进入条件：

1. 先由 TRADING-271～275 在 DQ/PIT/chronology gate 下积累真实 forward observations；只有
   CQ Binding 出现真实 target 后，CR 才开始计算非空 requirement/event progress，不能反向在
   CR 补造 candidate 或 targets。
2. 将可信 validation cache 扩展为跨进程持久化前，必须把 validator/source schema version、
   policy/config hash、cutoff 与完整 file commitments 纳入 key，并验证并发、原子写、失效、
   corruption recovery；未完成前保持 session-local。
3. 对其余深 evidence chain 逐一做 source-bundle fan-out profile；只有证明下游未消费的递归
   snapshot body 才能移除，并必须以 live validator、显式 source binding 与 tamper tests 补足。
4. 在 CI/runtime artifact 中持续记录 test duration、snapshot bytes、cache hit/miss 与 slowest
   chain；若性能预算回退则作为 architecture finding，不以跳过 validation 解决。
5. candidate/targets 真实出现后补齐多 target、partial requirement、sideways/recovery coexist、
   owner renewal available/unavailable 双路径和跨 lineage 拒绝测试，再评估 Dashboard/Recheck
   policy；当前 null fixture 不足以校准投资阈值。

本 slice 仍为 manual/current-definition/not-PIT paper-shadow operations，
`not_official_target_weights=true`、`auto_apply=false`、`broker_action_allowed=false`、
...
1. Active primary conclusion window 统一从 `2021-02-22` 开始；`as_of/week_ending` 是实际观察日期，
   `generated_at` 必须不早于所有输入 source。历史 contract fixture 若实际请求
   `ai_after_chatgpt=2022-12-01`，只按 immutable historical/legacy comparison 原样披露；该日期不再是
   active default、required comparator 或 minimum start，也不能覆盖本链的 2021 primary 语义。
