...
|UI-002|仪表盘/每日决策展示|P2|VALIDATING|系统验证|详细拆解见 `docs/requirements/decision_dashboard_v2_2026-05-10.md`；`aits reports dashboard` 已接入 `ops daily-run` 的 `score_daily` 后置只读步骤，并接入 `ops replay-day` 的 cache-only 隔离输出；最近交易日 replay 已生成 dashboard HTML/JSON|Dashboard v2 仍只读取已生成 artifact，不重新计算投资结论；HTML 首页能在一屏内展示今日动作、仓位区间、置信度、Data Gate、最大限制、变化摘要、Top evidence/invalidators/next checks；可选读取 `alerts_YYYY-MM-DD.md` 和 `scores_daily.csv` 生成告警摘要与历史趋势；同步输出 JSON payload 供测试和后续前端演进；页面、JSON 和文档均声明 `production_effect=none`、不替代 Markdown 日报和 trace bundle 审计源；`daily-run` 中 dashboard 生成失败不得改变评分结论或交易动作|2026-05-10: 新增并进入实现，原因：owner 认可静态只读 dashboard 路线，并要求继续推进数据展示层；范围限定为现有 evidence dashboard 的可审计增量，避免过早建设在线 Web App 或交易终端。2026-05-10: 从 IN_PROGRESS 改为 VALIDATING，原因：静态 HTML/JSON、alerts 聚合、history 摘要、CLI 和测试已完成；浏览器插件因 file URL 安全策略未能做内嵌视觉预览，需后续真实使用复核。2026-05-10: 从 VALIDATING 改回 IN_PROGRESS，原因：owner 要求把网页生成接入自动 daily-run 流程并跑最近交易日全流程。2026-05-10: 从 IN_PROGRESS 改为 VALIDATING，原因：自动触发已实现并通过测试；真实 `daily-run --as-of 2026-05-08` 先通过 download/PIT/SEC/valuation，随后阻断在 `score_daily` 的 `risk_event_prereview_request_in_future`，dashboard 后置步骤尚未执行到。2026-05-10: 最近交易日改用 strict replay 入口复测，`replay-day --as-of 2026-05-08 --mode cache-only --openai-replay-policy cache-only --compare-to-production` PASS，并在 replay bundle 生成 `evidence_dashboard_2026-05-08.html/json`。|
|VIS-001|数据可见性/PIT 复现契约|P0|VALIDATING|系统验证|详细拆解见 `docs/requirements/input_visibility_contract_2026-05-10.md`；运行入口级强边界已实现：历史 `as_of` 不再用 `daily-run` 调 live provider/OpenAI 或改写生产路径，严格复现走 `replay-day cache-only` 隔离 bundle；OpenAI prereview cache-only 已改为 row 级可见性过滤；下一步把 `trade_theses`、`trades` 等手工输入也纳入 replay 隔离过滤视图，避免只靠下游校验发现 future manual input|`daily-run` 对历史 `as_of` 在执行任何子命令前 fail closed，并在执行报告/metadata 中说明应改用 `ops replay-day`；`replay-day` 不调用 live provider/OpenAI，按可见窗口过滤 PIT、估值、风险事件和 OpenAI cache-only 输入，并生成 replay-scoped dashboard；所有相关文档说明 `event_time/source_published_at/provider_available_at/ingested_at/available_time/visibility_cutoff` 的分层语义；测试覆盖历史 daily-run 前置阻断、OpenAI cache-only 可见性过滤和最近交易日 cache-only replay 全链路|2026-05-10: 新增并进入实现，原因：最近交易日 `daily-run --as-of 2026-05-08` 在 OpenAI 预审处被 `risk_event_prereview_request_in_future` 阻断，暴露生产调度、历史复现和研究重建的输入可见性边界仍不够显式；owner 要求继续修复并用原日期复测。2026-05-10: 从 IN_PROGRESS 改为 VALIDATING，原因：daily-run 历史 as_of 预检查、metadata input visibility、replay dashboard、文档和测试已完成；原日期 daily-run 现在前置 `BLOCKED_VISIBILITY`，cache-only replay PASS。2026-05-10: 从 VALIDATING 改回 IN_PROGRESS，原因：owner 要求 replay 尽量复用历史 OpenAI 请求结果，但必须按复现时点可见性过滤缓存记录，避免未来 LLM 请求结果进入严格复现。2026-05-10: 从 IN_PROGRESS 改为 VALIDATING，原因：OpenAI cache-only replay 已按 `request_timestamp/cache_created_at/cache file created_at` 过滤；真实 2026-05-08 replay PASS，源 OpenAI 预审队列 5 条均因 2026-05-10 请求时间晚于 cutoff 被排除，dashboard/health/secret 全链路通过。2026-05-10: 调查 dashboard `final AI position=0` 时确认，直接原因是 `trade_theses` 全部创建/更新时间为 2026-05-10，严格回放 2026-05-08 时 thesis 校验报 24 个 future-input 错误并触发 `thesis` position gate 上限 0%；当前行为 fail closed，但后续应在 replay 输入冻结阶段先生成 thesis 隔离视图。|
