...
|CAUSE-001|决策因果链/审计|P1|PROPOSED|系统实现|建立 `decision_causal_chain` ledger，把 evidence、模块变化、gate、decision snapshot、outcome 和 rule candidate 串起来；详细拆解见 `docs/requirements/feedback_loop_governance_2026-05-04.md#cause-001`|每个核心仓位变化或结论降级都能对应至少一个 `chain_id`；因果链能连接 evidence、module score/confidence 变化、gate、decision snapshot、outcome 和后续规则复核；不允许用未来 outcome 改写 signal_date 当时的 causal chain，只能追加后验观察字段；实现时同步更新 `docs/system_flow.md` 并补充因果链和未来函数防护测试|依赖 `REPORT-001`、`EVIDENCE-001` 和 `FEEDBACK-001`；目标是支持判断“当时为什么对/错”，不是引入不可审计因果推断模型|
...
|DATA-002|数据源/质量|P1|BLOCKED_OWNER_INPUT|项目 owner + 系统实现|建立 provider health score 和 cross-provider reconciliation；详细拆解见 `docs/requirements/feedback_loop_governance_2026-05-04.md#data-002`|对价格、估值、财报和宏观数据输出 provider 新鲜度、字段漂移、row count 异常、checksum 变化、ticker alias 风险和跨来源差异；跨供应商冲突进入调查项而不是自动平滑；严重问题能停止下游或降级结论；实现时同步更新 `docs/system_flow.md` 并补充数据源健康测试|需要 owner 选择可长期使用的第二来源、供应商权限和口径策略；没有合格第二来源时只能完成单源健康报告，不能假装完成跨源校验|
