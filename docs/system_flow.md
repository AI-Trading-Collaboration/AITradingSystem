...
flowchart TD
    ...
    subgraph Governance["结构校验"]
        ...
        ODP["aits ops daily-plan / daily-run<br/>每日运行计划、America/New_York 最新已完成交易日默认 as-of、交易日历判断、凭据检查、输入可见性预检查、真实执行和调度顺序<br/>交易日顺序由 config/scheduled_tasks.yaml 约束；daily-run 支持 --run-output-root / --run-id / --legacy-output-mode"]
