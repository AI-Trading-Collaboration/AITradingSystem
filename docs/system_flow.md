...
TRADING-064 的 `experiments.yaml` 是后续 batch runner / comparison / ranking / candidate gate 的 config source-of-truth；当前 registry validation 只确认受控 experiment 定义、安全字段、base config 引用和 override surface，不运行回测、不生成 runtime artifacts、不改变 P0 allocation。
