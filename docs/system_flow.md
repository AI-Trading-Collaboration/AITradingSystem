...
flowchart TD
    ...
    subgraph Cache["本地缓存"]
        ERC["data/raw/external_request_cache/<provider>/<api_family>/<cache_key>/<br/>response.body + metadata.json<br/>外部供应商请求级缓存：HIT 不再请求供应商；MISS 才发送并归档<br/>Cboe VIX 静态 CSV cache key 包含 ticker/start/end 窗口，且缓存体必须覆盖 end<br/>API key / token / Cookie / User-Agent 脱敏"]
        ...
        SECPITOBS["aits sec-pit shadow-observe<br/>只读生成隔离 SEC PIT observe-only score、rank shift、bucket comparison、monitoring plan 和 safety audit"]
    ...
    subgraph Feedback["反馈校准"]
        ...
        RBRIEF["aits reports reader-brief --date/--as-of/--latest<br/>统一每日读者入口；只读汇总 decision summary、dashboard、task dashboard、calculation explainers、score change、research governance、report index、documentation contract、snapshot、trace；缺 report_index 时只读 registry fallback"]
        RBRIEFR["outputs/reports/reader_brief_YYYY-MM-DD.html/json<br/>Narrative Executive Summary、Missing/Limited Artifact Impact、market minimum panel、Score & Decision Funnel details、Task Cadence Calendar、grouped Report Navigation、Data/PIT safety、research governance、documentation contract 和 grouped manual review queue；production_effect=none"]
...
|每日运行执行器|`aits ops daily-run`|复用 `daily-plan` 的步骤顺序真实调用本地 CLI，未显式传入 `--as-of` 时使用同一最新已完成美股交易日 resolver；先写计划报告并执行输入可见性预检查，再执行交易日完整链路，并在 `score-daily` 成功后先自动生成同日 `parameter_governance`、`market_feedback_optimization`、`feedback_loop_review` 和 weekly `investment_review`，再调用 `aits reports dashboard` 生成只读 HTML/JSON 展示层；执行报告和本轮子报告收录进 canonical run bundle 后，额外生成 `daily_task_dashboard_YYYY-MM-DD.html/json` 每日关键结论页、`daily_decision_summary_YYYY-MM-DD.json` 每日决策总线 JSON 和 `order_intent_candidates_YYYY-MM-DD.json` blocked-only 候选方向；若同日已存在 `paper_trading_summary_YYYY-MM-DD.json`，dashboard 只读展示 Paper Trading Summary 的 candidate_count、blocked_candidates、generated_intents、风控/订单/成交和 reconciliation 计数，并可只读聚合最近 7/14/30 日已存在的 paper summary 和 order intent candidate JSON 作为 Paper Trading Trend，展示 `replay_mode=daily_independent`、`portfolio_carry_forward=false`、candidate/intent/fill/PnL、reconciliation 分布、synthetic snapshot 比例、top `blocked_by` 和 top `reason_code`；若同日已存在 `paper_signal_quality_YYYY-MM-DD.json`，dashboard 只显示 Paper Signal Quality 轻量卡片：evaluation_status、主要 `blocked_by`、synthetic_snapshot_ratio、sample_count 和 report link，并标记 observe-only / `production_effect=none`，不展示详细聚合表；若同日已存在 `shadow_parameter_impact_YYYY-MM-DD.json` 或 `weight_adjustment_candidates_YYYY-MM-DD.json`，dashboard 只显示对应轻量卡片，其中 Weight Adjustment Candidate 仅展示 candidate_count、top_candidate_id、gate_status、main blocked_by、report link 和 `production_effect=none`；`daily-run` 不自动生成 weight adjustment candidates，不应用参数、不改变主结论、不触发交易；缺失历史 summary、candidate JSON、signal quality JSON、shadow impact JSON 或 weight adjustment candidate JSON 显示 `LIMITED` / `MISSING` 且不补造结论，不触发 paper、replay 或真实交易；paper trading runner / replay / signal quality / shadow impact / weight adjustment candidate 的 `LIMITED` / `ERROR` 只进入该只读 summary/dashboard 风险，不阻断主报告、不改变 production 仓位建议；`daily-run` 是生产调度入口，不用于历史时点复现，输入可见性预检查同样按最新已完成美股交易日判断，显式未来 `as_of` 或历史 `as_of` 返回 `BLOCKED_VISIBILITY`，不执行任何 download/PIT/SEC/valuation/OpenAI/dashboard 子命令，历史日期提示改用 `aits ops replay-day --mode cache-only`；休市日执行官方政策/地缘来源抓取、PIT、SEC companyfacts、SEC metrics 抽取/校验、FMP valuation snapshots、`ops health --non-trading-day` 和 secret scan，同时显式跳过 `score-daily`、复盘前置报告和 dashboard；默认写入 canonical run bundle，可用 `--run-output-root` 指定根目录、`--run-id` 固定运行标识、`--legacy-output-mode mirror/off` 控制旧 `outputs/reports` 兼容镜像；canonical run bundle 只从 legacy 报告路径收录本轮运行开始后实际产出的同日 artifact，避免 skipped dashboard 等旧文件误入本轮输出；执行器内部优先用项目 `.venv` Python 调用 daily-run direct dispatcher，找不到本地虚拟环境时才回退当前 Python，避免 Windows 上从 `aits.exe` 父进程递归启动 `aits.exe`、PATH 上的全局 `aits` 污染每日子命令环境，以及 Typer 对整棵 CLI 做全局解析时触发的 Windows 原生崩溃；direct dispatcher 是 daily-run 子命令薄适配层，会把 `score-daily --skip-risk-event-openai-precheck` 映射到当前 `score_daily(risk_event_openai_precheck=False)`，并传递 `--llm-request-profile`，候选上限未显式覆盖时由 `score-daily` profile 解析，且支持 dashboard 前置的 feedback/report 子命令，避免主 CLI 签名演进阻断生产调度或把默认请求策略重新硬编码；子命令环境显式设置 `PYTHONMALLOC=malloc`、`PYTHONFAULTHANDLER=1`、`PYTHONDONTWRITEBYTECODE=1`，为每次 `daily-run` 使用独立 `PYTHONPYCACHEPREFIX=outputs/tmp/pycache/daily_run/run_*`，并在启动子命令前清理 `src/**/__pycache__`，降低 Windows 本机长流程子进程原生崩溃和字节码缓存异常风险；缺少阻断性环境变量时返回 `BLOCKED_ENV`；任一执行步骤退出码非 0 或关键 artifact 报告状态非 `PASS*` 时停止，不继续下游步骤；dashboard、每日关键结论页、每日决策总线 JSON、order intent candidate JSON、Paper Trading Summary / Trend、Paper Signal Quality、Shadow Impact、Weight Adjustment Candidate 和复盘前置报告均为只读展示/复核层，失败只进入运行报告或命令退出状态，不改变已生成的评分、仓位、回测、OrderIntent 或执行建议；显式 `SKIPPED` 步骤在计划、执行报告、每日关键结论页、决策总线 JSON 和 order intent candidate JSON 中保留限制声明，不得被解释为生成了投资结论|已实现基础版|
...
|Reader Brief|`aits reports reader-brief --date YYYY-MM-DD` / `--as-of YYYY-MM-DD` / `--latest` + `outputs/reports/reader_brief_YYYY-MM-DD.html` / `.json`|只读读取 `daily_decision_summary`、Markdown 日报、evidence dashboard JSON、daily task dashboard JSON、calculation explainers JSON、score change attribution JSON、research governance summary JSON、report index JSON、documentation contract JSON、decision snapshot 和日报 trace bundle；`--latest` 只从默认 decision snapshot 目录选择最新 signal-date snapshot，不运行上游任务；首屏新增 narrative Executive Summary，说明 today conclusion、why、positive/negative drivers、binding constraint、manual review summary 和 `production_effect_statement`；正文包含 Market Situation minimum panel、Score & Decision Funnel、Missing / Limited Artifact Impact、Score Change Attribution、Report Index Freshness、Task Cadence Calendar、Contribution Summary、Component Explainability、Binding Gate Ladder、Data/PIT Safety、Backtest/Shadow/Governance、Documentation Contract、grouped Manual Review Queue、grouped Report Navigation；关键 funnel 数字用 `<details>` 展开 formula、source artifact/freshness、PIT policy、common misread 和 `production_effect`；缺少 `report_index` 时从 `config/report_registry.yaml` 只读生成 cadence fallback；缺少可选 artifact 时输出 `LIMITED_READER_CONTEXT`、`PASS_WITH_WARNINGS`、`MISSING` 或 `LIMITED` 和 impact level，不得补造投资结论；固定 `production_effect=none`，不重算 score、不修改 weights/gates、不触发 paper trading、SEC PIT、backtest、shadow/weight、docs 上游或 broker|已实现基础版；REPORT-054 UX quality 进入验证|
