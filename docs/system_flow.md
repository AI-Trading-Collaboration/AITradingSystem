...
flowchart TD
    ...
    ETFV3FAIL --> ETFRIDX
    ETFV3FAIL --> ETFREAD
    ETFV3FAIL --> ETFV3SWEEP["aits etf dynamic-v3-rescue data-audit/data-provenance/window-audit/weight-path/sweep-config/sweep/candidate/walk-forward/overfit/governance/research/shadow/artifacts/schedule/promotion<br/>Constrained parameter research with profiles + tiny_fixture_proxy or real_dynamic_v3_rescue evaluator<br/>data provenance + window audit + weight path + candidate attribution + train/test selection + overfit/shadow monitor + latest pointer repair/validation + daily scheduler observe gate<br/>reports/etf_portfolio/dynamic_v3_rescue/data_audit|data_provenance|window_audit|sweeps|candidate_attribution|walk_forward_selection|overfit|governance|index|shadow|shadow_monitor|schedule_observe|promotion|latest/..."]
    ETFV3SWEEP --> ETFRIDX
    ETFV3SWEEP --> ETFREAD
...
flowchart TD
    ...
    subgraph Feedback["反馈校准"]
        ...
        HGOV["aits docs heuristic-audit --as-of<br/>扫描投资解释路径 numeric literal baseline，并校验关键 policy metadata"]
        HGOVR["outputs/reports/heuristic_governance_audit_YYYY-MM-DD.md/json<br/>unregistered numeric literal、baseline、policy metadata、production_effect=none"]
