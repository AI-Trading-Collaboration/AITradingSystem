...
TRADING-074A 新增 `config/etf_portfolio/operations_schedule.yaml` 和 `src/ai_trading_system/etf_portfolio/operations.py` loader/validator，把 ETF portfolio research 的 daily / weekly / biweekly / monthly / manual-review command schedule 从聊天计划转成 source configuration。该配置只描述命令、依赖、expected outputs、freshness age 和 failure policy；它不执行命令、不写 runtime artifacts、不改变 `aits ops daily-run` 统一外部入口。后续 TRADING-074B-K 的 command graph、freshness checker、dry-run、operations report、Reader Brief operations health section 和 `aits etf ops validate` 必须复用该 schedule spec，并保持 `observe_only=true`、`candidate_only=true`、`production_effect=none`、`broker_action=none`、`manual_review_required=true`。
...
flowchart TD
    ...
    T074DAILY --> T074NEXT["future TRADING-074B-K<br/>command graph / freshness checks / dry-run / ops report / Reader Brief ops health / validation gate"]
    T074WEEKLY --> T074NEXT
    T074SLOW --> T074NEXT
    T074MANUAL --> T074NEXT
```
