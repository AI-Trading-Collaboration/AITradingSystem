...
flowchart TD
    ...
    ETFV3PSWEEKLY --> ETFV3EVIDENCESTALE["aits etf dynamic-v3-rescue evidence-staleness-monitor run/report/validate-evidence-staleness-monitor<br/>TRADING-354 / TRADING-354B / TRADING-353A evidence staleness monitor<br/>reports/etf_portfolio/dynamic_v3_rescue/evidence_staleness_monitor/...<br/>price/market freshness uses requested_as_of + latest_complete_market_date -> freshness_reference_date; signal/stress/A-B/owner + paper-shadow daily/drift/weekly freshness use research artifact timestamps; weekly coverage status can block safe_to_continue_shadow<br/>stale_reason + coverage_status + missing_artifacts + safe_to_continue_shadow / read-only / no data refresh / no upstream rerun / broker_action_allowed=false / production_effect=none"]
