...
flowchart TD
    ...
    subgraph Feedback["反馈校准"]
        ...
        PCAND["data/processed/parameter_candidates.json<br/>参数 trial、候选状态、material 标记、多目标 veto 和治理下一步"]
        PCANDR["outputs/reports/parameter_candidates_YYYY-MM-DD.md<br/>参数候选台账、trial registry、OOS/random/data veto 证据"]
...
flowchart TD
    ...
    BS --> Q
    EP1 --> Q
    Q --> DASH["生成 evidence_dashboard_YYYY-MM-DD.html/json<br/>daily-run 在 score_daily 后先生成 market feedback / loop review / weekly investment review，再触发 dashboard；读取日报、trace、decision_snapshot、belief_state、alerts、scores_daily 和同日复盘报告"]
    T --> DASH
    D0 --> DASH
    BS --> DASH
```
...
|参数候选台账|`aits feedback build-parameter-candidates` / `data/processed/parameter_candidates.json` / `outputs/reports/parameter_candidates_YYYY-MM-DD.md`|读取 `parameter_replay_YYYY-MM-DD.json`，把每个参数复测场景登记为 trial，并只将可评估的参数类场景登记为 candidate-only 参数候选；记录 candidate id、linked trial、来源场景、收益差异、回撤变化、换手、material 标记、data quality、data credibility、coverage/placeholder/source veto、有效独立窗口、OOS、same-turnover random、signal-family baseline、score-architecture baseline、bootstrap CI、veto reasons、recommendation status、shadow/governance 状态和下一步；正向 material 变化必须同时通过 data/OOS/random/baseline/drawdown/statistical/sample 多目标门禁，才能进入 `READY_FOR_FORWARD_SHADOW`；OOS 弱化、随机基线未过、数据可信度/coverage 阻断、样本独立性不足、统计 CI 不支持或回撤恶化会进入阻断、as-if replay 或风险复核；不批准参数上线、不生成 approved overlay|已实现|
