...
flowchart TD
    ...
    subgraph Feedback["反馈校准"]
        ...
        DTASKDR["outputs/reports/daily_task_dashboard_YYYY-MM-DD.html/json<br/>key_conclusions、shadow parameter 结果/参数对比表、Paper Trading Summary、return 口径、重要风险、任务状态和可点击子报告链接"]
        ...
        OICAND["outputs/reports/order_intent_candidates_YYYY-MM-DD.json<br/>OrderIntentCandidate schema-compatible 候选方向：来自 daily_decision_summary、decision snapshot、position/gate；daily-run 默认 blocked，不接入 broker"]
...
|Order intent candidate JSON|`outputs/reports/order_intent_candidates_YYYY-MM-DD.json` / canonical run bundle 同名文件|由 `aits reports daily-tasks` 和 `aits ops daily-run` 在 `daily_decision_summary` 写出后生成；读取 `daily_decision_summary`、decision snapshot 的 score snapshot 和 position/gate 结论，输出 `schema_version=1`、`report_type=order_intent_candidates`、`production_effect=none`、`execution_boundary`、`source_inputs`、`source_artifacts`、OrderIntentCandidate schema-compatible candidate、`source_decision`、score snapshot 和 position context；daily-run 生成的 candidate 默认 `blocked=true`，`blocked_by` 至少包含 `trading_engine_not_enabled` 和 `manual_approval_required`；缺少 snapshot 或上游结论时只能标记 missing/blocked reason，不补造订单方向、数量、金额或价格；该 JSON 只表达“如果未来允许交易，系统可能想做什么”，默认不生成 `OrderIntent`、不调用 trading engine、不接入 broker API、paper broker、真实下单接口或账户状态；只有外部审查后显式 `blocked=false` 且 `mode=paper` 的 candidate 才能通过 adapter 转换为 paper `OrderIntent`|已实现基础版|
