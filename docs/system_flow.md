...
ARCH-001 第二十一批后，`aits reports ...` 命令组统一由 `src/ai_trading_system/cli_commands/reports.py` 承载；`src/ai_trading_system/cli_direct.py` 的报告别名也调用同一 `reports_cli` 模块。该迁移只改变 CLI 模块边界，不改变报告 payload、data quality gate、Reader Brief、daily task dashboard、report registry、评分、回测或投资解释语义。
