...
flowchart TD
    ...
    subgraph PaperTrading["Paper Trading Engine MVP"]
        ...
        TEWSPRBR["future TRADING-018E3 explicit rollback command<br/>读取 rollback snapshot；要求 rollback approval 和 danger flag；原子恢复 target profile；TRADING-018E2 不实现 rollback"]
    ...
    TEWSPAR -. "future rollback input；018E2 不执行 rollback" .-> TEWSPRBR
    IBKRCFG --> TEIBKR
    IBKRCFG --> TEIBKRA
    TEIBKR --> TEIBKRA
    TEIBKRA --> TEIBKRREC
