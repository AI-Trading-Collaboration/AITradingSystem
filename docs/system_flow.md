...
flowchart TD
    ...
    subgraph Feedback["反馈校准"]
        ...
        OICAND["outputs/reports/order_intent_candidates_YYYY-MM-DD.json<br/>blocked-only 候选方向：来自 daily_decision_summary、decision snapshot、position/gate；不生成 OrderIntent、不接入 broker"]
    ...
    subgraph PaperTrading["Paper Trading Engine MVP"]
        ...
        TEOIB["OrderIntentBuilder / order intent candidate adapter<br/>未来只读读取 daily_decision_summary / order_intent_candidates；当前未接入、不触发交易"]
    ...
    OICAND -. "全部 blocked；后续只读 candidate 输入；当前不生成 OrderIntent" .-> TEOIB
