...
flowchart TD
    ...
    subgraph PaperTrading["Paper Trading Engine MVP"]
        ...
        TEWSPA["future TRADING-018E2 explicit apply command<br/>未实现；必须有单独 apply approval、强制写 production CLI 参数、apply 前后 validation 和 rollback snapshot；禁止 scheduler 自动运行"]
    ...
    TEWSIR --> TEWSPF
    TEWSPF --> TEWSPFR
    TEWSPFR -. "future only；018E1 不执行 apply" .-> TEWSPA
    IBKRCFG --> TEIBKR
    IBKRCFG --> TEIBKRA
    TEIBKR --> TEIBKRA
    TEIBKRA --> TEIBKRREC
