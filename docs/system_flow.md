...
flowchart TD
    subgraph Source["数据输入"]
        ...
        IBKROCFG["config/ibkr_paper_order.yaml<br/>IBKR Paper order lifecycle 配置；默认 disabled；只允许 paper、DUP account、production_effect=none；不保存凭证"]
...
|IBKR Paper order lifecycle 配置|`config/ibkr_paper_order.yaml`|登记 IBKR Paper submit/status/cancel/report lifecycle 测试配置；默认 `paper_order_lifecycle_enabled=false`，只允许显式 local 配置启用；只允许 `trading_mode=paper`、`production_effect=none`、`DUP` account、股票白名单、`LIMIT`、`DAY`、极小 `quantity` 和远离市场价限价；不保存 password、token、API key、session cookie 或其他敏感凭证|已实现基础版|
