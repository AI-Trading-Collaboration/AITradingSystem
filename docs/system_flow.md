...
flowchart TD
    ...
    subgraph Governance["结构校验"]
        ...
        STCFG["config/scheduled_tasks.yaml<br/>OPS-059 统一登记 daily / weekly / biweekly / monthly / ad hoc 调度链；daily-plan/daily-run 按 daily_trading_day 顺序校验；SEC PIT shadow daily 命令显式传入 {as_of}"]
        ODP["aits ops daily-plan / daily-run<br/>src/ai_trading_system/cli_commands/ops.py<br/>每日运行计划、America/New_York 最新已完成交易日默认 as-of、交易日历判断、凭据检查、输入可见性预检查、真实执行和调度顺序<br/>交易日顺序由 config/scheduled_tasks.yaml 约束；daily-run direct dispatcher 覆盖 daily_trading_day 全链路，包括 market data freshness、freshness recovery、portfolio candidate tracking、tracking review、SEC PIT shadow observe/monitor as_of binding、Reader Brief 和 health/secret scan；daily-run 支持 --run-output-root / --run-id / --legacy-output-mode"]
        ODPR["outputs/runs/daily/<executed_at_utc>/as_of_YYYY-MM-DD__<safe_run_id>/{manifest.json,reports/,traces/,metadata/}<br/>legacy mirror: outputs/reports/daily_ops_* 与 daily_score_*<br/>market session、上一交易日、步骤、环境变量 presence、input visibility status、pre-run input checksum、ArtifactRef artifact checksum、visibility cutoff、git/config/rule hash、执行状态、失败步骤脱敏诊断、跳过声明；PASS 步骤声明的 produced_paths 必须存在或明确失败/降级"]
