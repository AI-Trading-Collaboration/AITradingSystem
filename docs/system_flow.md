...
```mermaid
flowchart TD
    ETFP["data/raw/prices_daily.csv 或 data/etf_portfolio/prices.csv<br/>SPY/QQQ/SMH/SOXX OHLCV<br/>standardize adds deterministic synthetic CASH metadata"] --> ETFDQ["aits etf data validate<br/>ETF price schema + duplicate + non-positive + return threshold gate"]
...
flowchart TD
    ...
    P1P --> T079DIAG["aits etf weight-calibration diagnostics --include-robust-packs --cache <mode> --workers <n/auto><br/>historical_weight_search_diagnostics_*.json/md + stable_shapes/near_shadow CSV<br/>multi-preset stability + near-shadow rescue; no gate relaxation"]
