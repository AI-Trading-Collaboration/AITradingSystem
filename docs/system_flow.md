...
flowchart TD
    ...
    subgraph PaperTrading["Paper Trading Engine MVP"]
        ...
        TEOIB["OrderIntentBuilder / order intent candidate adapter<br/>未来只读读取 daily_decision_summary；当前未接入、不触发交易"]
    ...
    DDBUS -. "后续只读 candidate 输入；当前不生成 OrderIntent" .-> TEOIB
