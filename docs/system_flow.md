...
flowchart TD
    ...
    subgraph Governance["结构校验"]
        ...
        ODR["aits ops replay-day / replay-window<br/>历史交易日 cache-only 归档回放<br/>冻结 as-of 可见输入窗口、手工输入隔离视图、OpenAI disabled/cache-only 可见性过滤、dashboard、可选 production diff、窗口批量回放"]
        ODRR["outputs/replays/YYYY-MM-DD/<run-id>/ 与 outputs/replays/windows/<run-id>/<br/>input freeze manifest、OpenAI cache-only 过滤报告、trade_theses/trades 过滤视图、replay_run、diff_vs_production、replay_window、score/dashboard/health/secret 输出"]
...
|SEC 公司映射|`config/sec_companies.yaml`|记录核心标的 ticker、CIK、taxonomy 预期和统一指标周期覆盖范围；US 公司使用 SEC companyfacts，TSM 季度通过 TSM IR 合并补齐，ASML 等 ADR/foreign issuer 按可披露年度 SEC/IFRS 覆盖进入审计|已实现基础版|
