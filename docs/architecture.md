...
默认策略解释区间是 `unified_primary_2021`，主研究窗口为
`exact_three_asset_validated`，统一从 QQQ/SGOV/TQQQ 共同验证历史起点
`2021-02-22` 开始。`ai_after_chatgpt`（`2022-12-01` 起）只保留为历史比较
regime，不再提供 active default、下限或主要结论边界；更早历史仍只能作为带口径说明的
压力测试或敏感性证据。
...
- 风险情绪：VIX。
- 宏观序列：美国 2 年期、10 年期国债收益率，以及 FRED `DTWEXBGS` 广义美元指数代理。
- 可选：TLT。

财报、估值、政策/地缘发生记录、新闻与事件抽取放到第二期和第三期。其中 SEC 基本面、估值快照和手工审计的风险事件发生记录已经接入基础评分；估值快照已支持 FMP paid vendor API 拉取和结构化 CSV 导入，风险事件发生记录已支持结构化 CSV 导入，以减少 YAML 手工维护；LLM claim 预审已支持 OpenAI Responses API 结构化输出和待复核队列，默认请求策略读取 `config/llm_request_profiles.yaml`，当前 profile 保持 `gpt-5.5` 和单请求失败最多重试 2 次，日报自动风险预审 profile 正在用 `reasoning.effort=medium` 做成本 pilot，并记录审计字段，但仍只生成 `llm_extracted` / `pending_review` 线索，未接入交易评分。
...
- `prices_daily.csv`：标准化后的 FMP 股票/ETF 日线 OHLCV 主价格数据；`^VIX` 后续由 Cboe official historical data 单独接入。
- `rates_daily.csv`：长表格式的 FRED 宏观序列数据，当前包含 DGS2、DGS10 和 `DTWEXBGS`。
- `download_manifest.csv`：追加式下载审计清单，记录 provider、endpoint、请求参数、下载时间、行数、输出路径和 sha256。

数据下载命令为 `aits download-data`。默认使用 FMP 主价格源并要求 `FMP_API_KEY`，同时使用 Marketstack 第二行情源并要求 `MARKETSTACK_API_KEY`；Yahoo 仅可通过 `--price-provider yahoo` 显式用于迁移调查。默认只抓核心观察池，`--full-universe` 才抓完整 AI 产业链配置，避免 MVP 阶段数据面过宽。
...
`config/portfolio.yaml:macro_risk_asset_budget` 在 VIX、DGS10 利率变化或
`DTWEXBGS` 广义美元指数代理走强触发宏观流动性压力时下调。该层只影响总风险资产预算；AI 在风险资产内部的相对权重仍由评分模型和 `position_gate` 单独解释。
