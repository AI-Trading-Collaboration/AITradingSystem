...
| `outputs/reports/daily_task_dashboard_YYYY-MM-DD.html/json` | `aits ops daily-run` / `aits reports daily-tasks` | daily_ops_run metadata、本轮同日子报告、evidence dashboard JSON、最近可用 shadow parameter search bundle、最近同日 `shadow_iteration_YYYY-MM-DD.json`、可选同日 `paper_trading_summary_YYYY-MM-DD.json`、最近 7/14/30 日已存在的 paper trading summary 和 order intent candidate JSON、search window 内 production decision snapshots | `key_conclusions`、投资动作/仓位/置信度/Data Gate、数据可信度、参数治理、反馈复盘、shadow parameter 诊断领先 trial 与收益差距、Shadow Iteration Status 只读卡片、Paper Trading Summary（candidate_count、blocked_candidates、generated_intents、approved/rejected、submitted、filled/open、reconciliation_status）、Paper Trading Trend（7/14/30 日窗口、`replay_mode=daily_independent`、`portfolio_carry_forward=false`、summary 可用/缺失、候选/订单/成交/PnL/reconciliation、synthetic snapshot 使用比例、top `blocked_by`、top `reason_code`）、production/current vs shadow candidate 结果对比表（Total return、Max drawdown、Turnover、Beat rate、样本覆盖和 return 计算口径）、分区参数对比表（Gate cap override 与权重参数，含 production 实际 gate cap 数值/区间）、运行健康、任务状态、子报告 `href` 和可点击子任务下钻入口 | 每日关键结论入口 | 否，`production_effect=none` | 任务状态是审计信息，不是页面首要结论；dashboard 不重新计算 shadow iteration，也不触发 paper trading、paper replay、真实 broker 或参数晋级，只读取 JSON；shadow parameter 诊断领先不等于可上线；Paper Trading Summary / Trend 只展示模拟执行复盘，缺历史 summary 或 candidate JSON 显示 `LIMITED` 且不补造结论，`LIMITED` / `ERROR` 不阻断主报告；趋势不是连续组合收益，不改变 production 仓位建议；各子任务 Markdown/HTML/JSON 仍是审计源，后续可升级为专属网页 |
...
| `outputs/reports/paper_trading_replay_START_END.json/md` | `scripts/run_paper_trading_replay.py --start YYYY-MM-DD --end YYYY-MM-DD --mode daily-independent` | 每日 `order_intent_candidates_YYYY-MM-DD.json`；缺失时由 candidate runner 生成 limited `daily_decision_summary` 和 `order_intent_candidates`；逐日 paper summary / trading daily report / audit log；可选 historical OHLC | `replay_mode=daily_independent`、`portfolio_carry_forward=false`、quality_flags、market_snapshot_source 分布、窗口状态、market regime、`production_effect=none`、paper-only execution boundary、candidate / blocked / intent / risk / order / fill / PnL totals、reconciliation_status 分布、按 symbol / strategy_id / reason_code / blocked_by 聚合、每日结果和 limited upstream 标记 | paper trading daily flow validation、复盘和 dashboard 趋势输入复核 | 否，`production_effect=none` | 当前 replay 是逐日独立模拟，不是连续组合收益；`--mode continuous-portfolio` 本阶段只返回 `NOT_IMPLEMENTED`，不会假装支持持仓/cash/open order 结转；replay 不读取 broker API key、不调用真实 broker；缺候选只生成 limited artifacts，不补造投资结论 |
