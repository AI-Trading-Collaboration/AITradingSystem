...
| `outputs/reports/paper_trading_summary_YYYY-MM-DD.json` | `scripts/run_paper_trading_from_candidates.py` / trading report writer | `order_intent_candidates_YYYY-MM-DD.json`、paper trading report、audit log、ReconciliationResult | status、candidate_count、blocked_candidates、generated_intents、approved/rejected、submitted、filled/open/cancelled、realized/unrealized PnL、reconciliation_status、audit_log_path、trading daily report path、`production_effect=none` | daily task dashboard 的 Paper Trading Summary 区块 | 否，`production_effect=none` | 该 JSON 只把模拟执行复盘带回每日主入口；`LIMITED` / `ERROR` 不阻断主报告，不代表实盘订单、账户状态或 broker 回报，也不会改变 production 仓位建议 |
| `outputs/reports/paper_trading_replay_START_END.json/md` | `scripts/run_paper_trading_replay.py --start YYYY-MM-DD --end YYYY-MM-DD` | 每日 `order_intent_candidates_YYYY-MM-DD.json`；缺失时由 candidate runner 生成 limited `daily_decision_summary` 和 `order_intent_candidates`；逐日 paper summary / trading daily report / audit log | 窗口状态、market regime、`production_effect=none`、paper-only execution boundary、candidate / blocked / intent / risk / order / fill / PnL totals、reconciliation_status 分布、按 symbol / strategy_id / reason_code / blocked_by 聚合、每日结果和 limited upstream 标记 | paper trading daily flow validation、复盘和 dashboard 趋势输入复核 | 否，`production_effect=none` | replay 是 paper-only 连续运行验证，不是实盘收益、真实 broker 回报或 production 仓位建议；它不读取 broker API key、不调用真实 broker；缺候选只生成 limited artifacts，不补造投资结论 |
