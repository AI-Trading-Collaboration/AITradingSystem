...
| `outputs/reports/daily_task_dashboard_YYYY-MM-DD.html/json` | `aits ops daily-run` / `aits reports daily-tasks` | daily_ops_run metadata、本轮同日子报告、evidence dashboard JSON、最近可用 shadow parameter search bundle、最近同日 `shadow_iteration_YYYY-MM-DD.json`、可选同日 `paper_trading_summary_YYYY-MM-DD.json`、search window 内 production decision snapshots | `key_conclusions`、投资动作/仓位/置信度/Data Gate、数据可信度、参数治理、反馈复盘、shadow parameter 诊断领先 trial 与收益差距、Shadow Iteration Status 只读卡片、Paper Trading Summary、production/current vs shadow candidate 结果对比表（Total return、Max drawdown、Turnover、Beat rate、样本覆盖和 return 计算口径）、分区参数对比表（Gate cap override 与权重参数，含 production 实际 gate cap 数值/区间）、运行健康、任务状态、子报告 `href` 和可点击子任务下钻入口 | 每日关键结论入口 | 否，`production_effect=none` | 任务状态是审计信息，不是页面首要结论；dashboard 不重新计算 shadow iteration，也不触发 paper trading，只读取 JSON；shadow parameter 诊断领先不等于可上线；Paper Trading Summary 只展示模拟执行复盘；各子任务 Markdown/HTML/JSON 仍是审计源，后续可升级为专属网页 |
...
| `outputs/reports/order_intent_candidates_YYYY-MM-DD.json` | `aits ops daily-run` / `aits reports daily-tasks` | `daily_decision_summary_YYYY-MM-DD.json`、decision snapshot、position/gate 结论 | `as_of`、`run_id`、`production_effect=none`、`execution_boundary`、`source_inputs`、OrderIntentCandidate schema-compatible `candidates`、`blocked_by`、source decision、score snapshot、position/gate context | 后续 order intent candidate adapter 的只读候选输入；当前只表达“如果未来允许交易，系统可能想做什么” | 否，`production_effect=none` | daily-run 生成的 candidate 必须 `blocked=true`，且至少包含 `trading_engine_not_enabled` 和 `manual_approval_required`；不是 `OrderIntent`，不调用 trading engine，不接入 broker API、paper broker、真实下单接口或账户状态；只有显式审查后 `blocked=false` 且 `mode=paper` 的 candidate 才能由 runner 转为 paper `OrderIntent` |
...
| `outputs/reports/paper_trading_summary_YYYY-MM-DD.json` | `scripts/run_paper_trading_from_candidates.py` / trading report writer | `order_intent_candidates_YYYY-MM-DD.json`、paper trading report、audit log、ReconciliationResult | generated_intents、approved/rejected、submitted、filled/open/cancelled、realized/unrealized PnL、reconciliation_status、audit_log_path、trading daily report path、`production_effect=none` | daily task dashboard 的 Paper Trading Summary 区块 | 否，`production_effect=none` | 该 JSON 只把模拟执行复盘带回每日主入口；不代表实盘订单、账户状态或 broker 回报，也不会触发 trading_engine |
