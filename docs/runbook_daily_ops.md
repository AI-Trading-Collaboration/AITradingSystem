...
交易日 `fmp_valuation` capture 继续把当前 valuation YAML 和 analyst raw 写入同日
`data/external/daily_input_capture/YYYY-MM-DD/`、
`data/raw/daily_input_capture/YYYY-MM-DD/`，但 revision 和 valuation percentile 的历史输入
必须读取 canonical `data/processed/pit_snapshots/` 与 `data/external/` 累计归档。读取器只接受
`fmp_*_valuation_*.yaml`，按 `available_time <= decision_time` 过滤 normalized PIT，并排除
`snapshot.as_of >= as_of` 的 valuation history。历史不足时保留 warning/降级；不要把当前日
capture 目录重新当作唯一 history，也不要为补历史重跑 provider。
