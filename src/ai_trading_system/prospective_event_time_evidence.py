...
POLICY_PATH = "config/research/prospective_event_time_evidence_v1.yaml"
...
POLICY_SHA256 = "364257f8c0959a6f042c646fc0ef86c4e9c8ba7351754cc15a8bd3b0db70b466"
_CALENDAR_PATHS = (
...
# datetime's one-microsecond representation is an invariant, not a clock-drift
# allowance. Monotonic samples lie inside the two UTC samples below.
...
def load_time_evidence_policy(*, source_root: Path) -> TimeEvidencePolicy:
    ...
    if policy_binding.sha256 != POLICY_SHA256:
    ...
    return TimeEvidencePolicy(source, policy_binding, bindings)


def _policy_live(policy: TimeEvidencePolicy) -> None:
    if (
        type(policy) is not TimeEvidencePolicy
        or load_time_evidence_policy(source_root=policy.source_root) != policy
    ):
...
class RecordedTemporalEvidence:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            "schema_version": "prospective_time_evidence.v1",
...
def _semantic_intent(
    ...
    return {
        "schema_version": "prospective_time_intent.v1",
        "plan": plan.to_dict(),
...
def _result_state(
    *,
    ...
    if completed < started or (
        parent is not None and started < parent.payload_durable_completed_at
    ):
        _fail("TEMPORAL_CLOCK_BACKWARD", "event clock precedes start or predecessor")
    if kind == "ACTIVATION":
        first = _next_session(completed.astimezone(US_EQUITY_MARKET_TIMEZONE).date())
    ...
    status = "TIMELY_PAYLOAD" if completed < timing.effective_close_at else "LATE_PAYLOAD"
    ...
    *,
    ...
    with exclusive_store_maintenance(store_root=root) as maintenance:
        ...
        if existing is None:
            ...
            # This timestamp is a payload-completion upper bound, not witness durability.
            elapsed = time.monotonic_ns() - mono_started
            completed = _instant(_utc_now())
    ...
    if existing is not None:
        evidence = verify_time_evidence(
            store_root=root, event=_binding(completion_path, existing), policy=policy
        )
        existing_witness = _object(strict_json_loads(existing))
        existing_intent = _object(
            strict_json_loads(_read(root, EventBinding.from_dict(existing_witness["intent"])))
        )
        _equal(existing_intent["semantic"], semantic, "TEMPORAL_EVENT_CONFLICT")
        return evidence
    ...
    with exclusive_store_maintenance(store_root=root) as maintenance:
        witness_started = _instant(_utc_now())
...
def verify_time_evidence(
    *,
    ...
    if (
        witness["schema_version"] != "prospective_time_completion.v1"
    ...
    elapsed = witness["monotonic_elapsed_ns"]
    if type(elapsed) is not int or elapsed < 0:
        _fail("TEMPORAL_MONOTONIC_CLOCK_INVALID", "nonnegative elapsed time required")
    ...
    return RecordedTemporalEvidence(
        event, plan, kind, feature, previous, started, completed, first, timing, status
    )
    ...
    *,
    ...
    while feature <= end:
        ...
        if content is not None:
            ...
            if evidence.payload_durable_completed_at > review_time:
    ...
    _policy_live(policy)
    return {
        "schema_version": "prospective_session_coverage.v1",
