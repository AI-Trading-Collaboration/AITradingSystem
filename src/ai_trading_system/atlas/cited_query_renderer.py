...
def _render_page_effectiveness(showcase: AtlasCitedQueryShowcase) -> str:
    manifest = showcase.page_effectiveness
    if len(manifest.task_coverage) != 38:
    ...
    successor_rows = "".join(
        ...
        if 2494 <= int(item.task_id.split("-", 1)[1].split("_", 1)[0]) <= 2519
    )
    return f"""
    <section class="page-effectiveness" id="page-effectiveness" aria-labelledby="page-effectiveness-title" data-page-freshness="{escape(manifest.freshness_status.value)}" data-task-coverage-count="{len(manifest.task_coverage)}">
      ...
      <div class="reader-answer-grid" aria-label="读者先回答的六个问题">
        ...
        <article><span>05 · 下一步</span><strong>唯一 2516 run 已因 Option filter casting error 失败且 token 已消费；collector 修正版发布后，须由 Project Owner 独立复核并提供新的 v3 exact token，才允许一次新的 zero-order Cloud run。</strong></article>
      ...
      <div class="effectiveness-boundary">
        ...
        <details class="successor-coverage">
          <summary>查看 TRADING-2494–2519 如何影响当前页面</summary>
      ...
      <details class="effectiveness-audit">
        ...
        <dl>
          ...
          <div><dt>覆盖范围</dt><dd><code>TRADING-2481..2504, 2506..2519</code> · {len(manifest.source_artifacts)} semantic sources</dd></div>
        </dl>
      </details>
    </section>
    """
