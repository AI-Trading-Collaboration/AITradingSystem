...
from ai_trading_system.cli_commands.engineering_reports import (
    register_engineering_report_commands,
)
...
from ai_trading_system.cli_commands.report_index_commands import (
    register_report_index_commands,
    report_index_command,  # noqa: F401 - re-exported for direct CLI compatibility.
...
def _write_evidence_repair_validation(
    ...
    *,
    ...
    if source_report_type == evidence_repair_reports.EVIDENCE_GAP_LEDGER_REPORT_TYPE:
        ...
        source_report_type
        == evidence_repair_reports.SIGNAL_ROBUSTNESS_DRILLDOWN_REPORT_TYPE
    ):
        payload = (
            evidence_repair_reports.validate_signal_robustness_blocker_drilldown_payload(
                source_payload
            )
        )
    elif (
        ...
        source_report_type
        == evidence_repair_reports.COST_BENCHMARK_WEAKNESS_ATTRIBUTION_REPORT_TYPE
    ):
        payload = (
            evidence_repair_reports.validate_cost_benchmark_weakness_attribution_payload(
                source_payload
            )
        )
    ...
    elif source_report_type == evidence_repair_reports.CANDIDATE_V2_SPEC_FREEZE_REPORT_TYPE:
        payload = evidence_repair_reports.validate_candidate_v2_spec_freeze_payload(
            source_payload
        )
    elif (
        source_report_type
        == evidence_repair_reports.CANDIDATE_V2_EXECUTABLE_BINDING_REPORT_TYPE
    ):
        payload = (
            evidence_repair_reports.validate_candidate_v2_executable_binding_update_payload(
                source_payload
            )
        )
    elif source_report_type == evidence_repair_reports.CANDIDATE_V2_MINI_BACKFILL_REPORT_TYPE:
        ...
        source_report_type
        == evidence_repair_reports.CANDIDATE_V2_FULL_BACKFILL_REPORT_TYPE
    ):
        payload = (
            evidence_repair_reports.validate_candidate_v2_full_backfill_if_approved_payload(
                source_payload
            )
        )
    elif source_report_type == evidence_repair_reports.CANDIDATE_V2_RESEARCH_GATE_REPORT_TYPE:
        ...
        source_report_type
        == evidence_repair_reports.CANDIDATE_V2_RESEARCH_CYCLE_SNAPSHOT_REPORT_TYPE
    ):
        payload = (
            evidence_repair_reports.validate_candidate_v2_research_cycle_snapshot_payload(
                source_payload
            )
        )
    else:
...
@reports_app.command("signal-robustness-blocker-drilldown")
def signal_robustness_blocker_drilldown_command(
    as_of: Annotated[
        str | None,
        typer.Option("--as-of", "--date", help="Signal drilldown 日期。"),
    ] = None,
    reports_dir: Annotated[
        Path,
        typer.Option(help="报告 artifact 所在目录。"),
    ...
    json_output_path: Annotated[
        Path | None,
        typer.Option(help="Signal drilldown JSON 输出路径。"),
    ] = None,
    markdown_output_path: Annotated[
        Path | None,
        typer.Option(help="Signal drilldown Markdown 输出路径。"),
    ] = None,
) -> None:
    """TRADING-473：下钻 signal robustness blockers。"""
    report_date = _parse_date(as_of) if as_of else date.today()
    try:
        payload = (
            evidence_repair_reports.build_signal_robustness_blocker_drilldown_payload(
                as_of=report_date,
                reports_dir=reports_dir,
            )
        )
    ...
    summary = payload["summary"]
    console.print(f"[yellow]Signal robustness drilldown：{payload['status']}[/yellow]")
    console.print(f"blocker_count：{summary['blocker_count']}")
    console.print(
        f"repairable_without_rule_relaxation："
        f"{summary['repairable_without_rule_relaxation']}"
    )
    console.print(
        f"candidate_redesign_blocker_count："
        f"{summary['candidate_redesign_blocker_count']}"
    )
    console.print(f"JSON：{json_path}")
    console.print(f"Markdown：{md_path}")
...
@reports_app.command("cost-benchmark-weakness-attribution")
def cost_benchmark_weakness_attribution_command(
    as_of: Annotated[
        str | None,
        typer.Option("--as-of", "--date", help="Cost/benchmark weakness attribution 日期。"),
    ] = None,
    reports_dir: Annotated[
        Path,
        typer.Option(help="报告 artifact 所在目录。"),
    ...
    json_output_path: Annotated[
        Path | None,
        typer.Option(help="Cost/benchmark attribution JSON 输出路径。"),
    ] = None,
    markdown_output_path: Annotated[
        Path | None,
        typer.Option(help="Cost/benchmark attribution Markdown 输出路径。"),
    ] = None,
) -> None:
    """TRADING-476：归因 cost/benchmark weakness。"""
    report_date = _parse_date(as_of) if as_of else date.today()
    try:
        payload = (
            evidence_repair_reports.build_cost_benchmark_weakness_attribution_payload(
                as_of=report_date,
                reports_dir=reports_dir,
            )
        )
    ...
    summary = payload["summary"]
    console.print(
        f"[yellow]Cost/benchmark weakness attribution：{payload['status']}[/yellow]"
    )
    console.print(f"design_judgment：{summary['design_judgment']}")
    console.print(f"cost_weakness_count：{summary['cost_weakness_count']}")
    console.print(f"benchmark_weakness_count：{summary['benchmark_weakness_count']}")
    console.print(
        f"fixable_by_candidate_redesign："
        f"{summary['fixable_by_candidate_redesign']}"
    )
    console.print(f"JSON：{json_path}")
    console.print(f"Markdown：{md_path}")
...
@reports_app.command("candidate-v2-spec-freeze")
def candidate_v2_spec_freeze_command(
    as_of: Annotated[
        str | None,
        typer.Option("--as-of", "--date", help="Candidate v2 spec freeze 日期。"),
    ] = None,
    reports_dir: Annotated[
        Path,
        typer.Option(help="报告 artifact 所在目录。"),
    ...
    json_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 spec freeze JSON 输出路径。"),
    ] = None,
    markdown_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 spec freeze Markdown 输出路径。"),
    ] = None,
) -> None:
    """TRADING-478：冻结 research-only v2 candidate spec。"""
    report_date = _parse_date(as_of) if as_of else date.today()
    try:
        payload = evidence_repair_reports.build_candidate_v2_spec_freeze_payload(
            as_of=report_date,
            reports_dir=reports_dir,
        )
    except (FileNotFoundError, ValueError) as exc:
    ...
    summary = payload["summary"]
    console.print(f"[yellow]Candidate v2 spec freeze：{payload['status']}[/yellow]")
    console.print(f"candidate_id：{summary['candidate_id']}")
    console.print(f"selected_hypothesis_id：{summary['selected_hypothesis_id']}")
    console.print(f"stop_condition_count：{summary['stop_condition_count']}")
    console.print(f"JSON：{json_path}")
    console.print(f"Markdown：{md_path}")


@reports_app.command("candidate-v2-executable-binding-update")
def candidate_v2_executable_binding_update_command(
    as_of: Annotated[
        str | None,
        typer.Option("--as-of", "--date", help="Candidate v2 binding update 日期。"),
    ] = None,
    reports_dir: Annotated[
        Path,
        typer.Option(help="报告 artifact 所在目录。"),
    ...
    feature_path: Annotated[
        Path,
        typer.Option(help="ETF feature cache CSV 路径。"),
    ] = evidence_repair_reports.DEFAULT_V2_FEATURE_PATH,
    weight_policy_path: Annotated[
        Path,
    ...
    json_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 binding update JSON 输出路径。"),
    ] = None,
    markdown_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 binding update Markdown 输出路径。"),
    ] = None,
) -> None:
    """TRADING-479：生成 research-only v2 signal/weight binding update。"""
    report_date = _parse_date(as_of) if as_of else date.today()
    data_quality_gate = _run_next_research_data_quality_gate(
        report_date=report_date,
        reports_dir=reports_dir,
    ...
    try:
        payload = (
            evidence_repair_reports.build_candidate_v2_executable_binding_update_payload(
                as_of=report_date,
                reports_dir=reports_dir,
    ...
    style = (
        "red"
        if payload["status"]
        == evidence_repair_reports.CANDIDATE_V2_EXECUTABLE_BINDING_BLOCKED
        else "yellow"
    )
    console.print(f"[{style}]Candidate v2 binding update：{payload['status']}[/{style}]")
    console.print(f"candidate_id：{summary['candidate_id']}")
    console.print(f"signal_rows：{summary['signal_row_count']}")
    console.print(f"weight_rows：{summary['weight_row_count']}")
    console.print(f"safety_audit_status：{summary['safety_audit_status']}")
    console.print(f"JSON：{json_path}")
    console.print(f"Markdown：{md_path}")


@reports_app.command("candidate-v2-mini-backfill")
def candidate_v2_mini_backfill_command(
    as_of: Annotated[
        str | None,
        typer.Option("--as-of", "--date", help="Candidate v2 mini backfill 日期。"),
    ] = None,
    reports_dir: Annotated[
        Path,
        typer.Option(help="报告 artifact 所在目录。"),
    ...
    json_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 mini backfill JSON 输出路径。"),
    ] = None,
    markdown_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 mini backfill Markdown 输出路径。"),
    ] = None,
) -> None:
    """TRADING-480：运行 research-only compact candidate v2 mini backfill。"""
    report_date = _parse_date(as_of) if as_of else date.today()
    data_quality_gate = _run_next_research_data_quality_gate(
        report_date=report_date,
        reports_dir=reports_dir,
    ...
    if data_quality_gate["passed"] is not True:
        raise typer.Exit(code=1)
    try:
        payload = evidence_repair_reports.build_candidate_v2_mini_backfill_payload(
            as_of=report_date,
            reports_dir=reports_dir,
            prices_path=prices_path,
            data_quality_gate=data_quality_gate,
    ...
    style = (
        "red"
        if payload["status"] == evidence_repair_reports.V2_MINI_BACKFILL_BLOCKED
        else "yellow"
    )
    console.print(f"[{style}]Candidate v2 mini backfill：{payload['status']}[/{style}]")
    console.print(f"candidate_id：{summary['candidate_id']}")
    console.print(
        f"windows：{summary['completed_window_count']}/"
        f"{summary['required_mini_window_count']}"
    )
    console.print(f"aggregate_return_proxy：{summary['aggregate_return_proxy']}")
    console.print(f"aggregate_drawdown_proxy：{summary['aggregate_drawdown_proxy']}")
...
@reports_app.command("candidate-v2-full-backfill-if-approved")
def candidate_v2_full_backfill_if_approved_command(
    as_of: Annotated[
        str | None,
        typer.Option("--as-of", "--date", help="Candidate v2 full backfill 日期。"),
    ] = None,
    reports_dir: Annotated[
        Path,
        typer.Option(help="报告 artifact 所在目录。"),
    ...
    json_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 full backfill JSON 输出路径。"),
    ] = None,
    markdown_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 full backfill Markdown 输出路径。"),
    ] = None,
) -> None:
    """TRADING-482：仅在 mini gate 允许时运行 full backfill，否则输出 blocked。"""
    report_date = _parse_date(as_of) if as_of else date.today()
    try:
        payload = (
            evidence_repair_reports.build_candidate_v2_full_backfill_if_approved_payload(
                as_of=report_date,
                reports_dir=reports_dir,
            )
        )
    ...
    style = (
        "green"
        if payload["status"] == evidence_repair_reports.V2_FULL_BACKFILL_READY
        else "red"
    )
    console.print(f"[{style}]Candidate v2 full backfill：{payload['status']}[/{style}]")
    console.print(f"candidate_id：{summary['candidate_id']}")
    console.print(f"source_mini_gate_decision：{summary['source_mini_gate_decision']}")
    console.print(f"full_backfill_executed：{summary['full_backfill_executed']}")
    console.print(f"blocked_reason：{summary['full_backfill_blocked_reason']}")
    console.print(f"JSON：{json_path}")
    console.print(f"Markdown：{md_path}")


@reports_app.command("candidate-v2-research-gate")
def candidate_v2_research_gate_command(
    as_of: Annotated[
        str | None,
        typer.Option("--as-of", "--date", help="Candidate v2 research gate 日期。"),
    ] = None,
    reports_dir: Annotated[
        Path,
        typer.Option(help="报告 artifact 所在目录。"),
    ...
    json_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 research gate JSON 输出路径。"),
    ] = None,
    markdown_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 research gate Markdown 输出路径。"),
    ] = None,
) -> None:
    """TRADING-483：生成 v2 research gate；不激活 paper-shadow。"""
    report_date = _parse_date(as_of) if as_of else date.today()
    try:
        payload = evidence_repair_reports.build_candidate_v2_research_gate_payload(
            as_of=report_date,
            reports_dir=reports_dir,
        )
    except (FileNotFoundError, ValueError) as exc:
    ...
    style = (
        "green"
        if payload["status"] == evidence_repair_reports.V2_RESEARCH_PROMISING
        else "yellow"
    )
    if payload["status"] == evidence_repair_reports.V2_REJECT_RESEARCH_CANDIDATE:
        style = "red"
    console.print(f"[{style}]Candidate v2 research gate：{payload['status']}[/{style}]")
    console.print(f"candidate_id：{summary['candidate_id']}")
    console.print(f"source_full_backfill_status：{summary['source_full_backfill_status']}")
    console.print(f"full_backfill_executed：{summary['full_backfill_executed']}")
    console.print(f"gate_reason：{summary['gate_reason']}")
    console.print(f"JSON：{json_path}")
    console.print(f"Markdown：{md_path}")
...
@reports_app.command("candidate-v2-research-cycle-snapshot")
def candidate_v2_research_cycle_snapshot_command(
    as_of: Annotated[
        str | None,
        typer.Option("--as-of", "--date", help="Candidate v2 research cycle snapshot 日期。"),
    ] = None,
    reports_dir: Annotated[
        Path,
        typer.Option(help="报告 artifact 所在目录。"),
    ...
    json_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 research cycle snapshot JSON 输出路径。"),
    ] = None,
    markdown_output_path: Annotated[
        Path | None,
        typer.Option(help="Candidate v2 research cycle snapshot Markdown 输出路径。"),
    ] = None,
) -> None:
    """TRADING-485：生成最终 v2 research-cycle snapshot；不 append owner decision。"""
    report_date = _parse_date(as_of) if as_of else date.today()
    try:
        payload = evidence_repair_reports.build_candidate_v2_research_cycle_snapshot_payload(
            as_of=report_date,
            reports_dir=reports_dir,
        )
    except (FileNotFoundError, ValueError) as exc:
    ...
    style = (
        "green"
        if payload["status"] == evidence_repair_reports.V2_RESEARCH_CYCLE_PROMISING
        else "yellow"
    )
    console.print(f"[{style}]Candidate v2 research cycle：{payload['status']}[/{style}]")
    console.print(f"candidate_id：{summary['candidate_id']}")
    console.print(f"source_research_gate_decision：{summary['source_research_gate_decision']}")
    console.print(f"recommended_owner_option：{summary['recommended_owner_option']}")
    console.print(f"validation_fail_count：{summary['validation_fail_count']}")
    console.print(f"owner_decision_appended：{summary['owner_decision_appended']}")
    console.print(f"paper_shadow_activation_allowed：{summary['paper_shadow_activation_allowed']}")
    console.print(f"broker_order_allowed：{summary['broker_order_allowed']}")
    console.print(f"JSON：{json_path}")
...
@reports_app.command("validate-signal-robustness-blocker-drilldown")
def validate_signal_robustness_blocker_drilldown_command(
    latest: Annotated[
        bool,
        typer.Option(help="校验 latest signal robustness blocker drilldown。"),
    ] = False,
    as_of: Annotated[str | None, typer.Option("--as-of", "--date")] = None,
    reports_dir: Annotated[Path, typer.Option(help="报告 artifact 所在目录。")] = PROJECT_ROOT
    / "outputs"
    ...
    report_date = _parse_date(as_of) if as_of else date.today()
    source_path, source_payload = _load_evidence_repair_source_payload(
        report_type=evidence_repair_reports.SIGNAL_ROBUSTNESS_DRILLDOWN_REPORT_TYPE,
        report_date=report_date,
        reports_dir=reports_dir,
        latest=latest,
        source_json_path=source_json_path,
        label="signal robustness blocker drilldown",
    )
    payload, json_path, md_path = _write_evidence_repair_validation(
        source_payload,
        reports_dir=reports_dir,
...
@reports_app.command("validate-cost-benchmark-weakness-attribution")
def validate_cost_benchmark_weakness_attribution_command(
    latest: Annotated[
        bool,
        typer.Option(help="校验 latest cost/benchmark weakness attribution。"),
    ] = False,
    as_of: Annotated[str | None, typer.Option("--as-of", "--date")] = None,
    reports_dir: Annotated[Path, typer.Option(help="报告 artifact 所在目录。")] = PROJECT_ROOT
    / "outputs"
    ...
    source_path, source_payload = _load_evidence_repair_source_payload(
        ...
        latest=latest,
        source_json_path=source_json_path,
        label="cost/benchmark weakness attribution",
    )
    payload, json_path, md_path = _write_evidence_repair_validation(
        source_payload,
        reports_dir=reports_dir,
...
@reports_app.command("validate-candidate-v2-spec-freeze")
def validate_candidate_v2_spec_freeze_command(
    latest: Annotated[
        bool,
        typer.Option(help="校验 latest candidate v2 spec freeze。"),
    ] = False,
    as_of: Annotated[str | None, typer.Option("--as-of", "--date")] = None,
    reports_dir: Annotated[Path, typer.Option(help="报告 artifact 所在目录。")] = PROJECT_ROOT
    / "outputs"
    ...
    report_date = _parse_date(as_of) if as_of else date.today()
    source_path, source_payload = _load_evidence_repair_source_payload(
        report_type=evidence_repair_reports.CANDIDATE_V2_SPEC_FREEZE_REPORT_TYPE,
        report_date=report_date,
        reports_dir=reports_dir,
        latest=latest,
        source_json_path=source_json_path,
        label="candidate v2 spec freeze",
    )
    payload, json_path, md_path = _write_evidence_repair_validation(
        source_payload,
        reports_dir=reports_dir,
    ...
    style = "green" if status == "PASS" else "red"
    summary = payload["summary"]
    console.print(f"[{style}]Candidate v2 spec freeze validation：{status}[/{style}]")
    console.print(f"Source JSON：{source_path}")
    console.print(f"Validation JSON：{json_path}")
    console.print(f"Validation Markdown：{md_path}")
    console.print(
...
@reports_app.command("validate-candidate-v2-executable-binding-update")
def validate_candidate_v2_executable_binding_update_command(
    latest: Annotated[
        bool,
        typer.Option(help="校验 latest candidate v2 executable binding update。"),
    ] = False,
    as_of: Annotated[str | None, typer.Option("--as-of", "--date")] = None,
    reports_dir: Annotated[Path, typer.Option(help="报告 artifact 所在目录。")] = PROJECT_ROOT
    / "outputs"
    ...
    report_date = _parse_date(as_of) if as_of else date.today()
    source_path, source_payload = _load_evidence_repair_source_payload(
        report_type=evidence_repair_reports.CANDIDATE_V2_EXECUTABLE_BINDING_REPORT_TYPE,
        report_date=report_date,
        reports_dir=reports_dir,
        latest=latest,
        source_json_path=source_json_path,
        label="candidate v2 executable binding update",
    )
    payload, json_path, md_path = _write_evidence_repair_validation(
        source_payload,
        reports_dir=reports_dir,
    ...
    style = "green" if status == "PASS" else "red"
    summary = payload["summary"]
    console.print(f"[{style}]Candidate v2 binding validation：{status}[/{style}]")
    console.print(f"Source JSON：{source_path}")
    console.print(f"Validation JSON：{json_path}")
    console.print(f"Validation Markdown：{md_path}")
    console.print(
...
@reports_app.command("validate-candidate-v2-mini-backfill")
def validate_candidate_v2_mini_backfill_command(
    latest: Annotated[
        bool,
        typer.Option(help="校验 latest candidate v2 mini backfill。"),
    ] = False,
    as_of: Annotated[str | None, typer.Option("--as-of", "--date")] = None,
    reports_dir: Annotated[Path, typer.Option(help="报告 artifact 所在目录。")] = PROJECT_ROOT
    / "outputs"
    ...
    report_date = _parse_date(as_of) if as_of else date.today()
    source_path, source_payload = _load_evidence_repair_source_payload(
        report_type=evidence_repair_reports.CANDIDATE_V2_MINI_BACKFILL_REPORT_TYPE,
        report_date=report_date,
        reports_dir=reports_dir,
        latest=latest,
        source_json_path=source_json_path,
        label="candidate v2 mini backfill",
    )
    payload, json_path, md_path = _write_evidence_repair_validation(
        source_payload,
        reports_dir=reports_dir,
...
@reports_app.command("validate-candidate-v2-full-backfill-if-approved")
def validate_candidate_v2_full_backfill_if_approved_command(
    latest: Annotated[
        bool,
        typer.Option(help="校验 latest candidate v2 full backfill if approved。"),
    ] = False,
    as_of: Annotated[str | None, typer.Option("--as-of", "--date")] = None,
    reports_dir: Annotated[Path, typer.Option(help="报告 artifact 所在目录。")] = PROJECT_ROOT
    / "outputs"
    ...
    report_date = _parse_date(as_of) if as_of else date.today()
    source_path, source_payload = _load_evidence_repair_source_payload(
        report_type=evidence_repair_reports.CANDIDATE_V2_FULL_BACKFILL_REPORT_TYPE,
        report_date=report_date,
        reports_dir=reports_dir,
        latest=latest,
        source_json_path=source_json_path,
        label="candidate v2 full backfill if approved",
    )
    payload, json_path, md_path = _write_evidence_repair_validation(
        source_payload,
        reports_dir=reports_dir,
    ...
    style = "green" if status == "PASS" else "red"
    summary = payload["summary"]
    console.print(f"[{style}]Candidate v2 full backfill validation：{status}[/{style}]")
    console.print(f"Source JSON：{source_path}")
    console.print(f"Validation JSON：{json_path}")
    console.print(f"Validation Markdown：{md_path}")
    console.print(
...
@reports_app.command("validate-candidate-v2-research-gate")
def validate_candidate_v2_research_gate_command(
    latest: Annotated[
        bool,
        typer.Option(help="校验 latest candidate v2 research gate。"),
    ] = False,
    as_of: Annotated[str | None, typer.Option("--as-of", "--date")] = None,
    reports_dir: Annotated[Path, typer.Option(help="报告 artifact 所在目录。")] = PROJECT_ROOT
    / "outputs"
    ...
    report_date = _parse_date(as_of) if as_of else date.today()
    source_path, source_payload = _load_evidence_repair_source_payload(
        report_type=evidence_repair_reports.CANDIDATE_V2_RESEARCH_GATE_REPORT_TYPE,
        report_date=report_date,
        reports_dir=reports_dir,
        latest=latest,
        source_json_path=source_json_path,
        label="candidate v2 research gate",
    )
    payload, json_path, md_path = _write_evidence_repair_validation(
        source_payload,
        reports_dir=reports_dir,
    ...
    style = "green" if status == "PASS" else "red"
    summary = payload["summary"]
    console.print(f"[{style}]Candidate v2 research gate validation：{status}[/{style}]")
    console.print(f"Source JSON：{source_path}")
    console.print(f"Validation JSON：{json_path}")
    console.print(f"Validation Markdown：{md_path}")
    console.print(
...
@reports_app.command("validate-candidate-v2-research-cycle-snapshot")
def validate_candidate_v2_research_cycle_snapshot_command(
    latest: Annotated[
        bool,
        typer.Option(help="校验 latest candidate v2 research cycle snapshot。"),
    ] = False,
    as_of: Annotated[str | None, typer.Option("--as-of", "--date")] = None,
    reports_dir: Annotated[Path, typer.Option(help="报告 artifact 所在目录。")] = PROJECT_ROOT
    / "outputs"
    ...
    report_date = _parse_date(as_of) if as_of else date.today()
    source_path, source_payload = _load_evidence_repair_source_payload(
        report_type=evidence_repair_reports.CANDIDATE_V2_RESEARCH_CYCLE_SNAPSHOT_REPORT_TYPE,
        report_date=report_date,
        reports_dir=reports_dir,
        latest=latest,
        source_json_path=source_json_path,
        label="candidate v2 research cycle snapshot",
    )
    payload, json_path, md_path = _write_evidence_repair_validation(
        source_payload,
        reports_dir=reports_dir,
    ...
    style = "green" if status == "PASS" else "red"
    summary = payload["summary"]
    console.print(f"[{style}]Candidate v2 research cycle validation：{status}[/{style}]")
    console.print(f"Source JSON：{source_path}")
    console.print(f"Validation JSON：{json_path}")
    console.print(f"Validation Markdown：{md_path}")
    console.print(
