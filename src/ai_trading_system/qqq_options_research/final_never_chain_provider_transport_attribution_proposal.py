"""Sealed offline proposal for TRADING-2535.
...
DEFAULT_POLICY_PATH = (
    ...
    / "config"
    / "research"
    / "qc_qqq_options_final_never_chain_provider_transport_attribution_proposal_v1.yaml"
)
DEFAULT_PACKAGE_ROOT = (
    ...
    / "research"
    / "qqq_options"
    / "trading_2535_final_never_chain_provider_transport_attribution_proposal_v1"
)
TASK_ID = (
...
_OUTPUT_KEYS = (
    "TRADING2535_TARGET_SESSION_COUNT",
    "TRADING2535_TARGET_SESSION_DATE",
    "TRADING2535_TARGET_EQUITY_SLICE_PRESENT",
    "TRADING2535_TARGET_SUBSCRIBED_CHAIN_EVENT_COUNT",
    "TRADING2535_PROVIDER_PROBE_STATUS",
    "TRADING2535_PROVIDER_CONTRACT_COUNT",
    "TRADING2535_ATTRIBUTION",
    "TRADING2535_IDENTITY",
    "TRADING2535_TERMINAL",
)
_CLASSIFICATIONS = (
...
class AttributionProposalPolicy(_FrozenModel):
    schema_version: Literal[
        "qc_qqq_options_final_never_chain_provider_transport_attribution_proposal_policy.v1"
    ]
    policy_id: Literal[
        "qc_qqq_options_final_never_chain_provider_transport_attribution_proposal_v1"
    ]
    policy_version: Literal["1.0.0"]
    policy_status: Literal["OWNER_REVIEW_REQUIRED_EXACT_PROPOSAL"]
    task_id: Literal[
        "TRADING-2535_QC_QQQ_OPTIONS_FINAL_NEVER_CHAIN_SESSION_EXPORT_SAFE_PROVIDER_TRANSPORT_ATTRIBUTION_PROPOSAL_V1"
    ]
    ...
    provider_probe: Literal["QCALGORITHM_OPTION_CHAIN_CATALOG_COUNT_ONLY"]
...
class AttributionRunScope(_SealedModel):
    schema_version: Literal[
        "qc_qqq_options_final_never_chain_provider_transport_attribution_scope.v1"
    ]
    task_id: str
    requested_start: date
    requested_end: date
...
class AttributionProposal(_SealedModel):
    schema_version: Literal[
        "qc_qqq_options_final_never_chain_provider_transport_attribution_proposal.v1"
    ]
    task_id: str
...
class AttributionPackageManifest(_SealedModel):
    schema_version: Literal[
        "qc_qqq_options_final_never_chain_provider_transport_attribution_package_manifest.v1"
    ]
    task_id: str
...
# TRADING-2535 candidate only. External execution requires a new exact owner token.
...
class QQQOptionsFinalNeverChainProviderTransportAttribution(QCAlgorithm):
    def initialize(self):
    ...
    def _new_state():
        return {
            ...
            "provider_contract_count": None,
        }

    def _probe_provider_catalog(self, state):
        if state["provider_probe_status"] != "NOT_EVALUATED":
            return
        try:
            provider_contract_count = sum(1 for _ in self.option_chain(self._option))
        except Exception:
            state["provider_probe_status"] = "ERROR"
            return
        state["provider_contract_count"] = provider_contract_count
        state["provider_probe_status"] = (
            "AVAILABLE" if provider_contract_count > 0 else "EMPTY"
        )
    ...
    def on_data(self, data: Slice):
        ...
        if chain is not None and len(chain) > 0:
            ...
            return
        self._probe_provider_catalog(state)
    ...
    def on_end_of_algorithm(self):
        ...
        if target_count == 1:
            ...
            provider_status = state["provider_probe_status"]
            if state["provider_contract_count"] is not None:
                provider_count = str(state["provider_contract_count"])
            if provider_status == "EMPTY":
                attribution = "PROVIDER_CATALOG_EMPTY_FOR_TARGET_SESSION"
            elif provider_status == "AVAILABLE" and state["provider_contract_count"] > 0:
                attribution = (
                    "PROVIDER_CATALOG_AVAILABLE_BUT_SUBSCRIBED_SLICE_NEVER_DELIVERED"
                )
            elif provider_status == "ERROR":
                attribution = "PROVIDER_PROBE_ERROR"
        self.set_runtime_statistic("TRADING2535_TARGET_SESSION_COUNT", str(target_count))
        self.set_runtime_statistic("TRADING2535_TARGET_SESSION_DATE", target_date)
        self.set_runtime_statistic(
            "TRADING2535_TARGET_EQUITY_SLICE_PRESENT", equity_present
        )
        self.set_runtime_statistic(
            "TRADING2535_TARGET_SUBSCRIBED_CHAIN_EVENT_COUNT", chain_events
        )
        self.set_runtime_statistic("TRADING2535_PROVIDER_PROBE_STATUS", provider_status)
        self.set_runtime_statistic("TRADING2535_PROVIDER_CONTRACT_COUNT", provider_count)
        self.set_runtime_statistic("TRADING2535_ATTRIBUTION", attribution)
        self.set_runtime_statistic("TRADING2535_IDENTITY", IDENTITY)
        valid = (
...
def build_attribution_proposal_package(
    *, project_root: Path = PROJECT_ROOT
    ...
    run_scope = AttributionRunScope.seal(
        schema_version="qc_qqq_options_final_never_chain_provider_transport_attribution_scope.v1",
        task_id=TASK_ID,
    ...
    identity = (
        "schema=qc_qqq_options_final_never_chain_provider_transport_attribution_runtime.v1"
    ...
    proposal = AttributionProposal.seal(
        schema_version="qc_qqq_options_final_never_chain_provider_transport_attribution_proposal.v1",
        task_id=TASK_ID,
    ...
    manifest = AttributionPackageManifest.seal(
        schema_version="qc_qqq_options_final_never_chain_provider_transport_attribution_package_manifest.v1",
        task_id=TASK_ID,
        artifact_count=4,
        artifacts=artifacts,
