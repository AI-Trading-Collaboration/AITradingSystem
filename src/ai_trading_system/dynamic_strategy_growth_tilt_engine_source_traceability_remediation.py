...
TASK_REGISTER_ID = "TRADING-2413_GROWTH_TILT_ENGINE_SOURCE_TRACEABILITY_REMEDIATION"
REPORT_TYPE = "growth_tilt_engine_source_traceability_remediation"
SCHEMA_VERSION = "growth_tilt_engine_source_traceability_remediation.v1"
READY_STATUS = (
    "GROWTH_TILT_ENGINE_SOURCE_TRACEABILITY_REMEDIATION_READY_WITH_REMAINING_BLOCKERS"
)
BLOCKED_SOURCE_STATUS = "GROWTH_TILT_ENGINE_SOURCE_TRACEABILITY_REMEDIATION_BLOCKED_SOURCE"
SOURCE_TASKS: tuple[str, ...] = ("TRADING-2412", "TRADING-2411", "TRADING-2410")
DATA_QUALITY_GATE_REASON = (
    "NOT_APPLICABLE_SOURCE_TRACEABILITY_REMEDIATION_PRIOR_ARTIFACTS_ONLY_NO_FRESH_MARKET_DATA"
)
...
DEFAULT_GROWTH_TILT_ENGINE_SOURCE_TRACEABILITY_REMEDIATION_OUTPUT_ROOT = (
    PROJECT_ROOT / "outputs" / "research_strategies" / REPORT_TYPE
)
DEFAULT_GROWTH_TILT_ENGINE_SOURCE_TRACEABILITY_REMEDIATION_DOCS_ROOT = (
...
DEFAULT_SOURCE_2412_BEFORE_AFTER_REMEDIATION_PATH = (
    m2412.DEFAULT_GROWTH_TILT_ENGINE_AS_OF_SEMANTICS_REMEDIATION_OUTPUT_ROOT
...
def run_growth_tilt_engine_source_traceability_remediation(
    *,
    source_2412_as_of_remediation_result_path: Path = (
        DEFAULT_SOURCE_2412_AS_OF_REMEDIATION_RESULT_PATH
    ),
    source_2412_before_after_remediation_path: Path = (
        DEFAULT_SOURCE_2412_BEFORE_AFTER_REMEDIATION_PATH
    ),
    ...
    output_root: Path = DEFAULT_GROWTH_TILT_ENGINE_SOURCE_TRACEABILITY_REMEDIATION_OUTPUT_ROOT,
    docs_root: Path = DEFAULT_GROWTH_TILT_ENGINE_SOURCE_TRACEABILITY_REMEDIATION_DOCS_ROOT,
...
def _load_sources(
    *,
    ...
    return {
        ...
        "before_after_remediation_2412": _load_json_document(
            source_2412_before_after_remediation_path
        ),
        ...
        "source_paths": {
            ...
            "before_after_remediation_2412": str(source_2412_before_after_remediation_path),
            "updated_source_feature_mapping_2412": str(
                source_2412_updated_source_feature_mapping_path
            ),
            "remaining_blocker_summary_2412": str(source_2412_remaining_blocker_summary_path),
            ...
            "ordered_remediation_items_2411": str(source_2411_ordered_remediation_items_path),
            "unresolved_blocker_summary_2411": str(source_2411_unresolved_blocker_summary_path),
...
def _validate_sources(sources: Mapping[str, Any]) -> list[str]:
    ...
    for key in ("research_doc_2412", "artifact_catalog"):
        source = _as_mapping(sources.get(key))
        if source.get("_missing") is True:
    ...
    if result_2412.get("input_gap_count") != 7:
        errors.append("2412 input_gap_count must be 7 for TRADING-2413")
    if result_2412.get("as_of_remediated_count") != 2:
        errors.append("2412 must have two remediated as-of items")
    ...
    updated_mapping = _as_mapping(
        _as_mapping(sources.get("updated_source_feature_mapping_2412")).get(
            "updated_source_feature_mapping"
        )
    )
    if not _as_list(updated_mapping.get("mapping_rows")):
        errors.append("2412 updated source feature mapping must contain rows")
    if updated_mapping.get("contract_ready_count") != 0:
        errors.append("2412 updated mapping contract_ready_count must remain 0")
    ...
    if result_2411.get("gap_count") != result_2412.get("input_gap_count"):
        errors.append("2411 gap_count must match 2412 input_gap_count")
    ...
    source_items = [
        ...
        or _as_mapping(item).get("missing_source_traceability") is True
    ]
    if not source_items:
        errors.append("2411 must contain source_traceability_required remediation items")

    remaining_2412 = _as_mapping(
        _as_mapping(sources.get("remaining_blocker_summary_2412")).get(
            "remaining_blocker_summary"
        )
    )
    if remaining_2412.get("growth_tilt_engine_blocker_resolved") is not False:
        errors.append("2412 remaining summary must keep growth_tilt_engine unresolved")
    if remaining_2412.get("valid_until_window_blocker_resolved") is not False:
        errors.append("2412 remaining summary must keep valid_until_window unresolved")

    research_doc = str(_as_mapping(sources.get("research_doc_2412")).get("text", ""))
    if m2412.READY_STATUS not in research_doc:
    ...
    if "growth-tilt-engine-as-of-semantics-remediation" not in artifact_catalog:
        errors.append("artifact catalog missing 2412 as-of remediation command")
...
def _ready_sections(sources: Mapping[str, Any]) -> dict[str, Any]:
    as_of_result = _as_mapping(sources.get("as_of_remediation_result_2412"))
    updated_mapping_doc = _as_mapping(sources.get("updated_source_feature_mapping_2412"))
    section_mapping = _as_mapping(updated_mapping_doc.get("updated_source_feature_mapping"))
    if section_mapping:
        as_of_result = {**as_of_result, "updated_source_feature_mapping": section_mapping}
    remediation = build_growth_tilt_source_traceability_remediation(
        _as_mapping(sources.get("remediation_plan_result_2411")),
        as_of_result,
        project_root=PROJECT_ROOT,
        report_registry=_as_mapping(sources.get("report_registry")),
    )
    validation = _as_mapping(remediation.get("source_traceability_remediation_validation"))
    return {
        "source_remediation_plan_ready": True,
        "as_of_remediation_ready": True,
        "source_traceability_remediation_completed": remediation.get(
            "source_traceability_remediation_completed"
        ),
        "source_traceability_contract_metadata_ready": True,
        "before_after_source_traceability_remediation_ready": True,
        ...
        "source_traceability_gap_count": remediation.get("source_traceability_gap_count"),
        "source_traceability_remediated_count": remediation.get(
            "source_traceability_remediated_count"
        ),
        "remaining_source_traceability_gap_count": remediation.get(
            "remaining_source_traceability_gap_count"
        ),
        ...
        "source_traceability_remediation": remediation,
        "source_traceability_contract_metadata": remediation.get(
            "source_traceability_contract_metadata"
        ),
        "before_after_source_traceability_remediation": remediation.get(
            "before_after_source_traceability_remediation"
        ),
        ...
        "source_traceability_status_unclassified_count": validation.get(
            "source_traceability_status_unclassified_count"
        ),
        ...
        "recommended_next_research_task_reason": (
            "TRADING-2413 only remediates source traceability; TRADING-2414 should "
            "address signal validity dependency while candidate search, observation, "
            "paper-shadow, production, and broker paths remain disabled."
        ),
    }
...
def _blocked_sections() -> dict[str, Any]:
    return {
        ...
        "source_traceability_remediation_completed": False,
        "source_traceability_contract_metadata_ready": False,
        "before_after_source_traceability_remediation_ready": False,
        "updated_source_feature_mapping_ready": False,
        "remaining_blocker_summary_ready": False,
        "input_gap_count": None,
        "source_traceability_gap_count": 0,
        "source_traceability_remediated_count": 0,
        "remaining_source_traceability_gap_count": None,
        ...
        "updated_source_feature_mapping": {},
        "remaining_blocker_summary": {},
        "source_traceability_remediation_validation": {},
        "source_traceability_status_unclassified_count": None,
...
def _write_outputs(payload: dict[str, Any], *, output_root: Path, docs_root: Path) -> None:
    paths = {
        "json_path": str(output_root / "source_traceability_remediation_result.json"),
        "source_traceability_contract_metadata_json": str(
            output_root / "source_traceability_contract_metadata.json"
        ),
        "before_after_source_traceability_remediation_json": str(
            output_root / "before_after_source_traceability_remediation.json"
        ),
        ...
        "markdown_path": str(docs_root / "growth_tilt_engine_source_traceability_remediation.md"),
        "source_traceability_contract_markdown": str(
            docs_root / "growth_tilt_engine_source_traceability_contract_metadata.md"
        ),
        "next_route_markdown": str(docs_root / "dynamic_strategy_2414_route.md"),
    }
    payload["artifact_paths"] = paths
    write_json_artifact(Path(paths["json_path"]), payload)
    write_section_json_artifact(
        paths["source_traceability_contract_metadata_json"],
        "growth_tilt_engine_source_traceability_contract_metadata",
    ...
    write_section_json_artifact(
        paths["before_after_source_traceability_remediation_json"],
        "growth_tilt_engine_source_traceability_before_after_remediation",
    ...
    write_section_json_artifact(
        ...
        "growth_tilt_engine_source_feature_mapping_after_source_traceability",
    ...
    write_section_json_artifact(
        ...
        "growth_tilt_engine_source_traceability_remaining_blocker_summary",
    ...
    write_markdown_artifact(
        Path(paths["source_traceability_contract_markdown"]),
        _source_traceability_contract_markdown(payload),
    )
...
def _main_markdown(payload: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            ...
            (
                "- source traceability gap count："
                f"`{payload.get('source_traceability_gap_count')}`"
            ),
            (
                "- source traceability remediated count："
                f"`{payload.get('source_traceability_remediated_count')}`"
            ),
            (
                "- remaining source traceability gap count："
                f"`{payload.get('remaining_source_traceability_gap_count')}`"
            ),
            (
            ...
            "",
            "2413 只补齐 source traceability metadata。signal validity dependency、"
            "PIT gate 和 valid_until_window 仍未在本任务中修复，因此 "
            "`growth_tilt_engine` blocker 不能解除或降级。",
            "",
            "## Before / After",
            "",
            "```json",
            _json_block(payload.get("before_after_source_traceability_remediation", {})),
            "```",
            "",
            "## Source Traceability Contract Metadata",
            "",
            "```json",
            _json_block(payload.get("source_traceability_contract_metadata", {})),
            "```",
            "",
            "## Remaining Blockers",
            "",
...
def _source_traceability_contract_markdown(payload: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- status：`{payload.get('status')}`",
            (
                "- source traceability remediation completed："
                f"`{payload.get('source_traceability_remediation_completed')}`"
            ),
            (
                "- source traceability status unclassified count："
                f"`{payload.get('source_traceability_status_unclassified_count')}`"
            ),
            f"- next route：`{payload.get('recommended_next_research_task')}`",
            "",
            "```json",
