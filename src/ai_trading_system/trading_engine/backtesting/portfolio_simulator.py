...
def simulate_parameter_portfolio(
    ...
    *,
    ...
    feature_frame = _feature_frame(price_panel, returns)
    tradable_assets = [
    ...
    return PortfolioSimulationResult(metrics=metrics, daily_rows=tuple(rows))
...
def _feature_frame(price_panel: pd.DataFrame, returns: pd.DataFrame) -> dict[str, pd.DataFrame]:
