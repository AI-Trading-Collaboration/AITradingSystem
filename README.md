...
TRADING-485 之后，候选研究已回到 backlog，当前优先级是工程收尾、平台冻结准备和权重研究复盘。新用户先从 [docs/platform_user_guide.md](docs/platform_user_guide.md) 进入，不需要先浏览 task register。

| 角色 | 第一入口 | 下钻材料 |
|---|---|---|
| Researcher | `aits system status --as-of YYYY-MM-DD` | [docs/research/weight_research_turn_2026-06-19.md](docs/research/weight_research_turn_2026-06-19.md)、`aits reports latest --report-id next_candidate_research_cycle_snapshot` |
| Operator | `aits system doctor --as-of YYYY-MM-DD` | [docs/operations/operations_runbook.md](docs/operations/operations_runbook.md)、`aits validate-data`、`python scripts/run_validation_tier.py --list` |
| Owner / Reviewer | `aits system status --as-of YYYY-MM-DD` | Reader Brief、[docs/artifact_catalog.md](docs/artifact_catalog.md)、[docs/system_flow.md](docs/system_flow.md) |

`validation PASS` 只说明某个检查通过，不能写成候选研究有效、paper-shadow eligible、official target weights ready 或 broker/order approval。
...
trading_engine 和跨模块集成；`reproducibility` 覆盖 artifact lineage、run manifest 和
runtime summary 复现契约；`slow-research-regression` 单独承载 Dynamic v3、backtest
simulation 和研究回归。旧命令 alias 仍可用：`fast`、`reader-brief`、`artifact-reproduce`、
`dynamic-v3` 和 `trading-engine` 会分别解析到新的正式 suite。涉及投资解释、data quality、scoring、
...
与判定边界来自`sim_defensive_validation_v1.yaml`。Proposal review只接受四个validated、
same-Outcome source，冻结full bundles/validations与`advisory_proposal_review_v1.yaml`；
只映射真实Calibration proposals，空proposal为`INSUFFICIENT_DATA`，不补造confidence，
并固定 `auto_apply=false`、`owner_approval_required=true`。Forward confirmation plan
...
ARCH-004G2.4CH 将上述17个CLI callback迁至 canonical
`interfaces/cli/etf_portfolio/dynamic_v3_system_target_portfolio.py`，领域实现集中在
`etf_portfolio/dynamic_v3_system_target_portfolio.py`。五个producer分别写入versioned
input snapshot并在输出前验证source/config/cache/DQ与cutoff；缺candidate/consensus不再
补造target，Paper Init必须绑定validated Target，Rebalance以
`paper_shadow_state_after.json`追加post-state而不覆写source state，Performance仅使用
`evaluation_as_of`以内的共同finite交易日且missing保持null，Review严格要求
Target→Paper→Performance exact lineage，并把reviewed observation priority与真实
performance winner分开披露。任一source/output drift都会使validator FAIL。
...
ARCH-004G2.4CL 将上述 15 个 callback 迁至
`interfaces/cli/etf_portfolio/dynamic_v3_system_target_risk_capped.py`，领域实现迁至
`etf_portfolio/dynamic_v3_system_target_risk_capped.py`。Config、Target、Backfill、
Comparison、Review 分别写入 bounded `*.v2` input snapshot；producer 在正式写件前
验证 Model Target / Paper Backfill 与 config policy、timezone cutoff、DQ/cache
commitment 和 exact same-Backfill lineage。Comparison 不再把 baseline 内已有的
risk-capped states 与 dedicated projection 重复拼接，缺 regime 样本保持 null/
`insufficient_data`；return/exposure/drawdown decision thresholds 由 reviewed pilot
policy 治理。Validator 重验 live source/policy 并逐 byte 重建全部 views，任何 drift、
future/cross-lineage、duplicate、missing-to-zero 或 tamper 均 fail closed。该变更只加固
研究证据链，不把 fixture decision 提升为投资、promotion 或 production 结论。
...
ARCH-004G2.4CQ 将 16 个 callback 迁至
`interfaces/cli/etf_portfolio/dynamic_v3_system_target_smoothed_promotion.py`，领域实现迁至
`etf_portfolio/dynamic_v3_system_target_smoothed_promotion.py`；Promotion、Gate、Binding、
Switch、Owner 五段分别冻结 `*.v2` input snapshot，并在写件前重验 live upstream、reviewed
promotion policy、exact candidate lineage 与 chronology。Candidate authority 只来自 validated
Confirmation；当前 source-backed fixture 为 `candidate_method=null`，所以 Promotion 不可进入
owner review、Gate=`CONTINUE_OBSERVATION`、Binding=`targets=[]/NOT_REGISTERED`、Switch 的
proposed candidate 为 null，Owner 推荐 `request_more_forward_data`。系统不会回填固定 3d/5d、
补造 targets 或把 workflow PASS 写成晋升结论。Owner `record` 会原子重建同一 artifact；无
eligible candidate 时记录 promote decision 必须 fail closed，合法 `continue_observation` 仍可
完整验证。Source binding 只冻结下游实际消费的业务 views、hash 与 validator 结果，不递归复制
上游 input snapshot 正文，避免深链 artifact 指数膨胀；live validator 仍覆盖完整上游 artifact。
所有输出继续固定
`research_target_only=true`、`paper_shadow_only=true`、`not_official_target_weights=true`、
`broker_action_allowed=false`、`broker_action_taken=false`、`order_ticket_generated=false`、
...
TRADING-324～325 的 canonical research-direction foundation 由
`research_direction_foundation_v1.yaml` 管理，并冻结 Direction 与 Roadmap 两类
`input_snapshot.v2`。当前 Attribution 的 `DEFER_AND_BUILD_DATED_EVIDENCE` 只能映射为
`DEFER_PARAMETER_SEARCH_AND_BUILD_DATED_EVIDENCE`，unknown shift 必须 fail closed；task
只能是 `PROPOSED_OWNER_REVIEW`，不得自动登记或实施。Roadmap 继承 exact Direction 与
Attribution lineage，保持 parameter search=`DEFER`，并把 TRADING-326～345 明确标为历史
上下文而非当前 dated attribution proof。两类 validator 均重验 live source/policy/chronology
并逐 byte 重建 5 个 views；任何 task-state、method、portfolio、broker 或 production mutation
均被禁止。
...
visibility audit。`config/report_registry.yaml` 的 defaults 现在按 cadence 定义
`freshness_basis`：daily / weekly / biweekly 使用 U.S. equity trading days，避免周末把
Friday artifact 误判为 Monday stale；ad-hoc / monthly 继续用 calendar days。TRADING-487_to_504
Stage C 后，registry entry 可用 `visibility_policy=current|legacy_optional|deprecated_optional|archived_optional`
区分当前证据和历史/可选导航面；非 current 的 missing/stale artifact 在 report index 中显示为
`LEGACY_*` / `DEPRECATED_*` 等审计状态并计入 `non_current_visibility_count`，不再制造当前
missing/stale warning。机器可读 waiver policy 仍位于
`config/report_index_visibility_waivers.yaml`，但当前为 `waivers: []`；未来新增 waiver 仍必须记录
report id、issue status、reason、owner、accepted impact、validation coverage 和 exit condition。
required daily missing 和 production-effect risk 仍不能被 waiver 静默压掉。

TRADING-487_to_504_CLEAN_CLONE_RELEASE_ACCEPTANCE 新增平台冻结前的只读 release
acceptance runner。`python scripts/run_clean_clone_release_acceptance.py --as-of
YYYY-MM-DD` 在 clean worktree 上 local clone 当前 repo、创建 editable venv、生成 minimal
sample project，并运行 CLI help、engineering surface inventory、inventory validation、report
index、latest report lookup、canonical system status、system doctor 和可选
`artifact-reproduce` validation tier；输出
`outputs/reports/clean_clone_release_acceptance_YYYY-MM-DD.json/md`。dirty worktree 默认
fail closed；显式 `--allow-dirty-snapshot` 只能生成
`CLEAN_CLONE_ACCEPTANCE_BLOCKED_UNCOMMITTED_CHANGES` smoke 证据，不能作为 release PASS、
tag 或 platform freeze signoff。该 runner 固定 `production_effect=none`，不刷新数据、
不修改策略逻辑、不生成 official target weights、不触发 broker/order 或 production mutation。

TRADING-499_ENGINEERING_CLOSEOUT_RELEASE_CANDIDATE 新增平台冻结 release candidate
只读聚合报告。`aits reports engineering-closeout-release-candidate --as-of YYYY-MM-DD`
读取 latest canonical status / doctor、report index、Stage B readiness、Reader Brief
consistency、task-register consistency、clean-clone acceptance 和 release-blocking validation
runtime artifacts，输出
`outputs/reports/engineering_closeout_release_candidate_YYYY-MM-DD.json/md`，并记录
release version/tag、changelog、stable CLI/schema、compatibility policy、post-freeze change
admission rules 和 engineering closeout Reader Brief；`aits reports
validate-engineering-closeout-release-candidate --latest` 对 blocking closeout check fail
closed。该 report 不创建 git tag、不运行上游、不刷新数据、不修改 strategy / candidate /
paper-shadow / production state，不写 official target weights，不触发 broker/order。
...
`config/report_index_visibility_waivers.yaml` 中每条 waiver 必须记录 `waiver_id`、
...
active review 的 waiver 不再能把 missing/stale issue 清成 explicit waiver。当前 Stage C
cleanup 已把原 legacy/optional waiver 迁移为 registry `visibility_policy`，默认 waiver policy
为空。新增
...
summary/key result/safety boundary/next action 必须 fail closed；非 daily artifact 缺原生
标准 section 时，由 report index metadata 生成 effective Reader Brief view model，并把源模板
缺口记录为 `native_template_gap` 审计项，不重写历史 artifact。Reader Brief quality gate 也会显式检查
...
TRADING-474_WINDOW_FRAGILITY_ATTRIBUTION 在 signal drilldown 后解释 `WINDOW_FRAGILE`。
运行 `aits reports window-fragility-attribution --as-of YYYY-MM-DD` 和
`aits reports validate-window-fragility-attribution --latest` 会读取 window sensitivity
review、TRADING-473 signal drilldown、TRADING-472 repair plan 和 TRADING-471 ledger，
比较 early、middle、recent、stress-heavy 和 calm-market splits，输出 fragile windows、
stable windows、under-observed windows、failure modes、fragility judgment 和是否可继续研究。
归因字段覆盖 regime dependence、overfit threshold、signal instability、turnover
concentration、drawdown behavior、benchmark-relative weakness 和 cost sensitivity。该
attribution 不重新运行 backfill、不调参隐藏 fragility、不改变 candidate spec、不 append
...
TRADING-476_COST_BENCHMARK_WEAKNESS_ATTRIBUTION 在 stress attribution 后解释 cost /
benchmark review 为什么为 `COST_BENCHMARK_REVIEW_WEAK`。运行
`aits reports cost-benchmark-weakness-attribution --as-of YYYY-MM-DD` 和
`aits reports validate-cost-benchmark-weakness-attribution --latest` 会读取 cost/benchmark
review、linked cost sensitivity source report、benchmark baseline control pack、
TRADING-475 stress attribution、TRADING-474 window attribution、TRADING-472 repair plan
和 TRADING-471 ledger，覆盖 zero/low/medium/high cost scenarios 以及 static allocation、
no-trade、QQQ-only、SPY-only、equal-weight ETF baselines，逐行记录 turnover、cost drag、
gross/net proxy、baseline delta、cost weakness reason、benchmark weakness reason、
defensive/recovery behavior assessment 和 redesign repairability。该 attribution 不重新运行
cost/benchmark/backfill、不调参隐藏 weakness、不改变 candidate spec、不 append owner
decision、不创建 paper-shadow、不批准 extended/live、不写 official target weights、不触发
broker/order、不修改 production。
...
TRADING-479_CANDIDATE_V2_EXECUTABLE_BINDING_UPDATE 在 frozen v2 spec 后生成
research-only v2 binding。运行
`aits reports candidate-v2-executable-binding-update --as-of YYYY-MM-DD` 会先执行同一
cached-data quality gate，再读取 TRADING-478 spec、TRADING-464 backfill windows、ETF
feature cache 和 research weight binding policy，输出 v2 signal rows、hypothetical
research weight rows、turnover-aware guard disclosure 和 executable binding safety audit。
`aits reports validate-candidate-v2-executable-binding-update --latest` 校验 source spec、
非静态 signal rows、hypothetical-only weights、safety audit、data quality status、Reader
Brief 和安全边界。Safety audit failed 时不得进入 TRADING-480 mini backfill；该 artifact
不运行 backfill、不生成 return/drawdown/cost/benchmark metrics、不 append owner decision、
...
TRADING-482_CANDIDATE_V2_FULL_BACKFILL_IF_APPROVED 只在 TRADING-481 mini gate 为
`V2_PROCEED_TO_FULL_BACKFILL` 时允许 full backfill。运行
`aits reports candidate-v2-full-backfill-if-approved --as-of YYYY-MM-DD` 会先读取
mini gate 与 validation；若 gate 未批准，输出 blocked artifact，并逐项披露 return
proxy、drawdown proxy、turnover、rotation count、false risk-off count、cost inputs、
benchmark inputs 和 signal completeness 均未生成。`aits reports
validate-candidate-v2-full-backfill-if-approved --latest` 校验 mini-gate hard stop、
required output not-generated disclosure、no synthetic metrics、Reader Brief 和安全边界。
Blocked path 不运行 full backfill、不补造 metrics、不 append owner decision、不创建
...
aits risk-events quarantine-unknown-occurrence --occurrence-path <file.yaml> --authorization-id <owner-or-change-id> --as-of 2026-05-02
