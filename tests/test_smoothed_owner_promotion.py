...
def test_smoothed_owner_promotion_creation_recording_and_reader_brief(tmp_path) -> None:
    ...
    assert decision["recommended_owner_action"] == "request_more_forward_data"
    ...
    with pytest.raises(
        ...
        system_target.record_smoothed_owner_promotion_decision(
            decision_id=owner["decision_id"],
            decision="promote_to_primary_research_candidate",
            decision_reason="Invalid candidate-less promotion attempt.",
            output_dir=tmp_path / "smoothed_owner_promotion",
            recorded_at=datetime.fromisoformat(owner["manifest"]["generated_at"])
            + timedelta(seconds=1),
        )
    assert (
        system_target.validate_smoothed_owner_promotion_artifact(
            decision_id=owner["decision_id"],
            output_dir=tmp_path / "smoothed_owner_promotion",
        )["status"]
        == "PASS"
    ...
    recorded = system_target.record_smoothed_owner_promotion_decision(
        ...
        decision_reason="No evidence-backed candidate or forward target is registered.",
        output_dir=tmp_path / "smoothed_owner_promotion",
        recorded_at=datetime.fromisoformat(owner["manifest"]["generated_at"])
        + timedelta(seconds=1),
    )
    ...
    assert recorded_decision["recommended_owner_action"] == "request_more_forward_data"
    ...
    report_index = {
        "reports": [
            ...
            },
            {
                ...
                "latest_artifact_path": str(
                    fixture["gate"]["gate_dir"] / "primary_research_candidate_gate_manifest.json"
                ),
            },
            {
                "report_id": "etf_dynamic_v3_smoothed_forward_binding",
                "latest_artifact_path": str(
                    fixture["binding"]["binding_dir"] / "smoothed_forward_binding_manifest.json"
                ),
            },
            {
    ...
    assert (
        summary["smoothed_promotion_review_id"]
        == fixture["promotion_review"]["promotion_review_id"]
    )
    assert summary["primary_research_candidate_gate_decision"] == "CONTINUE_OBSERVATION"
