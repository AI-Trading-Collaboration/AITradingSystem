...
def test_event_detection_detects_composite_high_intensity_rule() -> None:
    rows = build_high_intensity_runtime_event_detection_dry_run_result(
...
def test_event_detection_ignores_false_trigger() -> None:
    rows = build_high_intensity_runtime_event_detection_dry_run_result(
        trigger_source_rows=sample_trigger_source_rows(),
        selected_rule=sample_selected_rule(),
        inputs=_inputs(),
...
def test_event_detection_ignores_inactive_scope() -> None:
...
def test_event_detection_ignores_non_defensive_direction() -> None:
    source = deepcopy(sample_trigger_source_rows())
...
def test_event_detection_blocks_missing_known_at_timestamp() -> None:
    source = deepcopy(sample_trigger_source_rows())
