...
def build_high_intensity_promotion_blocker_matrix_fixture(
    tmp_path: Path,
) -> dict[str, Path]:
    fixture = build_high_intensity_production_broker_hard_blocker_plan_fixture(
        tmp_path
    )
    production_broker_dir = tmp_path / "production_broker_plan"
    production_broker_docs_root = tmp_path / "production_broker_docs"
    run_high_intensity_risk_cap_observe_only_production_broker_hard_blocker_plan(
        ...
        output_dir=production_broker_dir,
        docs_root=production_broker_docs_root,
    )
    return {
        **fixture,
        "production_broker_dir": production_broker_dir,
        "production_broker_docs_root": production_broker_docs_root,
    }
