...
def build_high_intensity_event_append_contract_plan_fixture(
    tmp_path: Path,
) -> dict[str, Path]:
    fixture = build_high_intensity_scheduler_idempotency_replay_contract_plan_fixture(
        tmp_path
    )
    idempotency_replay_dir = tmp_path / "idempotency_replay_plan"
    idempotency_replay_docs_root = tmp_path / "idempotency_replay_docs"
    run_high_intensity_risk_cap_observe_only_scheduler_idempotency_replay_contract_plan(
        ...
        output_dir=idempotency_replay_dir,
        docs_root=idempotency_replay_docs_root,
    )
    return {
        **fixture,
        "idempotency_replay_dir": idempotency_replay_dir,
        "idempotency_replay_docs_root": idempotency_replay_docs_root,
    }
