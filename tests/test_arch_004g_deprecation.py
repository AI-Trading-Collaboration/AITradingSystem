...
def test_g0_inventory_is_deterministic_and_blocks_every_removal() -> None:
    ...
    assert inventory.python_module_count == 919
    assert inventory.python_test_file_count == 1120
    ...
    assert surfaces["etf_portfolio_cli_god_module"].line_count == 16761
    assert surfaces["etf_portfolio_cli_god_module"].top_level_function_count == 452
    assert surfaces["etf_portfolio_cli_god_module"].cli_command_decorator_count == 413
    assert surfaces["dynamic_v3_system_target_god_module"].line_count == 18248
    assert surfaces["dynamic_v3_system_target_god_module"].top_level_function_count == 740
