...
def test_g0_inventory_is_deterministic_and_blocks_every_removal() -> None:
    ...
    assert inventory.python_module_count == 909
    assert inventory.python_test_file_count == 1115
    ...
    assert surfaces["etf_portfolio_cli_god_module"].line_count == 19197
    assert surfaces["etf_portfolio_cli_god_module"].top_level_function_count == 534
    assert surfaces["etf_portfolio_cli_god_module"].cli_command_decorator_count == 495
    assert surfaces["dynamic_v3_system_target_god_module"].line_count == 24598
    assert surfaces["dynamic_v3_system_target_god_module"].top_level_function_count == 802
