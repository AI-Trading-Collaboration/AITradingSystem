...
# S4 adds two modules and two test files; the reviewed current ID changes only for those counts.
WAVE21_CURRENT_INVENTORY_ID = "arch_004g_deprecation_inventory_fbf99781316673c2b4fa"
WAVE21_CURRENT_REPOSITORY_COUNTS = {
    ...
    # TRADING-2556 adds one empirical runner module and one focused test file;
    # TRADING-2557 adds its versioned schema-only executor and focused test file;
    # TRADING-2558 adds one matched-placebo module and one focused test file;
    # TRADING-2559 adds one temporal-influence module and one focused test file;
    # TRADING-2563 adds one bounded catch-up executor test file.
    # TRADING-2564 adds two read-only readiness modules and two test files.
    # Its S2a named immutable contract adds one synthetic regression test file.
    # OPS-079 adds one historical-gap recovery module and one focused test file.
    # DEVX-013 adds two read-only workflow modules and two focused test files.
    # DEVX-014 adds source-preservation and OS-arbiter modules and two test files.
    # TRADING-2564 S2b adds four named-DQ modules and five test/support Python files.
    # S2c2 adds two pure preview modules and two focused test files.
    # S3a adds two event-time modules and two synthetic test files.
    # S3b adds capture DTO/runtime and production DQ dispatch (three modules),
    # plus five contract, dispatch, capture and actual-candidate test files.
    # Its clock correction adds two host-clock modules and two focused test files.
...
def test_g0_inventory_is_deterministic_and_blocks_every_removal() -> None:
    ...
    for surface_id, expected_count in WAVE21_CURRENT_DOCS_CONFIG_REFERENCE_COUNTS.items():
