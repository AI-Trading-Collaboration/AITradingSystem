...
def test_g0_inventory_is_deterministic_and_blocks_every_removal() -> None:
    ...
    assert inventory.python_module_count == 917
    assert inventory.python_test_file_count == 1119
    ...
    assert surfaces["etf_portfolio_cli_god_module"].line_count == 17328
    assert surfaces["etf_portfolio_cli_god_module"].top_level_function_count == 468
    assert surfaces["etf_portfolio_cli_god_module"].cli_command_decorator_count == 429
    assert surfaces["dynamic_v3_system_target_god_module"].line_count == 19856
    assert surfaces["dynamic_v3_system_target_god_module"].top_level_function_count == 760
