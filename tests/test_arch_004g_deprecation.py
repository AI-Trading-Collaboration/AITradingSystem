...
def test_g0_inventory_is_deterministic_and_blocks_every_removal() -> None:
    ...
    assert inventory.python_module_count == 926
    assert inventory.python_test_file_count == 1124
    ...
    assert surfaces["etf_portfolio_cli_god_module"].line_count == 15373
    assert surfaces["etf_portfolio_cli_god_module"].top_level_function_count == 407
    assert surfaces["etf_portfolio_cli_god_module"].cli_command_decorator_count == 368
    assert surfaces["dynamic_v3_system_target_god_module"].line_count == 13978
    assert surfaces["dynamic_v3_system_target_god_module"].top_level_function_count == 679
