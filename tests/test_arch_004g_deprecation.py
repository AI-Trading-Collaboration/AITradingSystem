...
def test_g0_inventory_is_deterministic_and_blocks_every_removal() -> None:
    ...
    assert inventory.python_module_count == 832
    ...
    assert surfaces["etf_portfolio_cli_god_module"].line_count == 30628
    assert surfaces["etf_portfolio_cli_god_module"].top_level_function_count == 928
    assert surfaces["etf_portfolio_cli_god_module"].cli_command_decorator_count == 889
