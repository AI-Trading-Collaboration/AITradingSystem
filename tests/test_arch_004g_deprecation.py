...
def test_g0_inventory_is_deterministic_and_blocks_every_removal() -> None:
    ...
    assert inventory.python_module_count == 878
    ...
    assert surfaces["etf_portfolio_cli_god_module"].line_count == 24581
    assert surfaces["etf_portfolio_cli_god_module"].top_level_function_count == 721
    assert surfaces["etf_portfolio_cli_god_module"].cli_command_decorator_count == 682
