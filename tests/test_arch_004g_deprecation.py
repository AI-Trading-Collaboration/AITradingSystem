...
def test_g0_inventory_is_deterministic_and_blocks_every_removal() -> None:
    ...
    assert inventory.python_test_file_count == 1112
    assert inventory.direct_writer_baseline_count == 894
    assert inventory.direct_writer_current_count == 893
    ...
    assert surfaces["etf_portfolio_cli_god_module"].line_count == 37604
    ...
    assert surfaces["dynamic_strategy_task_wrappers"].line_count == 89805
