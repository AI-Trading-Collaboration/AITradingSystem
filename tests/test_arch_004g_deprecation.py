...
def test_g0_inventory_is_deterministic_and_blocks_every_removal() -> None:
    ...
    assert inventory.python_module_count == 924
    assert inventory.python_test_file_count == 1123
    ...
    assert surfaces["etf_portfolio_cli_god_module"].line_count == 15842
    assert surfaces["etf_portfolio_cli_god_module"].top_level_function_count == 422
    assert surfaces["etf_portfolio_cli_god_module"].cli_command_decorator_count == 383
    assert surfaces["dynamic_v3_system_target_god_module"].line_count == 14895
    assert surfaces["dynamic_v3_system_target_god_module"].top_level_function_count == 678
