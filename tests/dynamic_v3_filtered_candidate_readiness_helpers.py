...
from ai_trading_system.etf_portfolio import (
    dynamic_v3_filtered_candidate_readiness_pipeline as canonical_readiness,
)
from ai_trading_system.etf_portfolio import (
...
def _source_filtered_candidate_fixture(tmp_path: Path, monkeypatch: Any) -> dict[str, Any]:
    ...
    policy_version = "filtered_candidate_pipeline_policy.test.v1"
    ...
    comparison_snapshot = {
        "schema_version": "filtered_vs_original_comparison_input_snapshot.v2",
        "comparison_id": comparison_id,
        "policy_source": policy_source,
        **st.EXPERIMENT_FACTORY_SAFETY,
    }
    st._write_json(
        comparison_dir / "filtered_vs_original_comparison_input_snapshot.json",
        comparison_snapshot,
    )
    ...
    review_snapshot = {
        "schema_version": "filtered_candidate_promotion_review_input_snapshot.v2",
        "filtered_review_id": review_id,
        "policy_source": policy_source,
        **st.EXPERIMENT_FACTORY_SAFETY,
    }
    st._write_json(
        review_dir / "filtered_candidate_promotion_review_input_snapshot.json",
        review_snapshot,
    )
    comparison_payload = {
        **comparison_manifest,
        ...
        "input_snapshot": comparison_snapshot,
    }
    review_payload = {
        **review_manifest,
        "filtered_promotion_decision": decision,
        "filtered_candidate_specs": specs,
        "input_snapshot": review_snapshot,
    }

    def _comparison_reader(artifact_id: str, output_dir: Path) -> dict[str, Any]:
        assert artifact_id == comparison_id
        assert Path(output_dir) == tmp_path / "filtered_vs_original_comparison"
        return comparison_payload

    def _review_reader(artifact_id: str, output_dir: Path) -> dict[str, Any]:
        assert artifact_id == review_id
        assert Path(output_dir) == tmp_path / "filtered_candidate_promotion_review"
        return review_payload

    monkeypatch.setattr(canonical_readiness, "_validated_comparison", _comparison_reader)
    monkeypatch.setattr(canonical_readiness, "_validated_promotion_review", _review_reader)
