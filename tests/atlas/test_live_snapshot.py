...
def test_live_policy_separates_research_evidence_and_page_dates() -> None:
    ...
    assert bundle.status_object_zh == (
        ...
        "S2a指定不可变快照合同和S2b named DQ执行合同已通过工程验证及普通main发布；"
        "current仅作已提交成员资格证明，不自动替换研究输入，"
        "结构PASS不签发DQ或consumer权限。"
        "S2c.1固定equal-risk价格消费scope已完成工程验证及普通main发布；它区分prices特征输入和rates质量守卫，"
        "不改变原DQ、策略算法或信号有效性结论。"
        ...
        "S2c.2固定五候选纯preview与显式只读plan已实现、正在验证；"
        "完整registry与XNYS日期随同一seal封存，S3真实前瞻接入仍待完成；"
