...
def test_policy_freezes_reader_questions_and_thirty_five_task_sources() -> None:
    policy = load_page_effectiveness_policy(repository_root=ROOT)
    assert policy.primary_research_start == "2021-02-22"
    assert len(policy.task_sources) == 35
    assert [item.task_id.split("_", 1)[0] for item in policy.task_sources] == [
        f"TRADING-{number}" for number in (*range(2481, 2505), *range(2506, 2517))
    ]
...
def test_manifest_binds_current_sources_tasks_and_independent_reviews() -> None:
    ...
    assert len(manifest.task_coverage) == 35
    assert [item.task_id.split("_", 1)[0] for item in manifest.task_coverage] == [
        f"TRADING-{number}" for number in (*range(2481, 2505), *range(2506, 2517))
    ]
    assert manifest.task_coverage[-8].coverage == "DISCLOSED_VERSIONED_SUCCESSOR_CONTRACT_ONLY"
    assert (
        manifest.task_coverage[-7].coverage
        == "DISCLOSED_PRIMARY_WINDOW_CALIBRATION_EVIDENCE_NOT_PROVIDED"
    )
    assert (
        manifest.task_coverage[-6].coverage == "DISCLOSED_DERIVED_EVIDENCE_GENERATOR_SOURCE_BLOCKED"
    )
    assert (
