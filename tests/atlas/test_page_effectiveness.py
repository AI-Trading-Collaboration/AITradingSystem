...
def test_policy_freezes_reader_questions_and_suffix_aware_task_sources() -> None:
    ...
    assert len(policy.task_sources) == 89
...
def test_manifest_binds_current_sources_tasks_and_independent_reviews() -> None:
    ...
    assert len(manifest.task_coverage) == 89
    ...
    assert coverage_by_task["TRADING-2542G"] == (
        "DISCLOSED_OPTIONAL_RESULT_BLIND_OVERLAY_NOT_BASELINE_BLOCKER"
    )
    assert coverage_by_task["TRADING-2542H"] == (
        "DISCLOSED_HISTORICAL_SCOPE_CORRECTION_SUPERSEDED_BY_2542I_RESULT_ADMISSION"
    )
    assert coverage_by_task["TRADING-2542I"] == (
    ...
    assert coverage_by_task["TRADING-2557"] == (
        "FOUNDATIONAL_F1_SCHEMA_FIX_VALID_EXECUTION_INSUFFICIENT_HOLD"
    )
    assert coverage_by_task["TRADING-2558"] == (
        "MATCHED_PLACEBO_TIMING_NOT_DISTINGUISHED_RESEARCH_PRIORITY_NOT_RAISED"
    )
    assert coverage_by_task["TRADING-2559"] == (
        "TEMPORAL_INFLUENCE_SINGLE_EPISODE_DEPENDENT_RESEARCH_PRIORITY_NOT_RAISED"
    )
