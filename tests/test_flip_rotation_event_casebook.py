...
from ai_trading_system.etf_portfolio import dynamic_v3_flip_rotation_casebook as casebook


def test_flip_rotation_event_casebook_builds_and_validates(tmp_path: Path) -> None:
    ...
    payload = casebook.flip_rotation_event_casebook_report_payload(
        casebook_run_id=result["casebook_run_id"],
        output_dir=tmp_path / "flip_rotation_event_casebook",
    )
    data = result["flip_rotation_event_casebook"]

    assert validation["status"] == "PASS"
    assert payload["flip_rotation_event_casebook"]["casebook_run_id"] == result[
        "casebook_run_id"
    ]
    ...
    assert "flip_rotation_casebook_event_count" in result[
        "flip_rotation_event_casebook_reader_brief"
    ]


def test_flip_rotation_event_casebook_validation_fails_incomplete_event(
    tmp_path: Path,
) -> None:
    config = yaml.safe_load(
        casebook.DEFAULT_FLIP_ROTATION_EVENT_CASEBOOK_CONFIG_PATH.read_text(
            encoding="utf-8"
        )
    )
    config["events"][0].pop("trigger_signal")
...
def test_flip_rotation_event_casebook_cli_report_and_validate(tmp_path: Path) -> None:
    ...
    validation = CliRunner().invoke(
        ...
        [
            "etf",
            "dynamic-v3-rescue",
            "validate-flip-rotation-event-casebook",
            "--casebook-run-id",
            casebook_run_id,
            "--output-dir",
            str(output_dir),
