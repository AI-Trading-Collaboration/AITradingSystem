...
def test_b3_relative_tilt_signal_outputs_signal_only_fields() -> None:
    config = load_etf_config_bundle()
    signal_policy, _ = load_b3_policies()
    features = _feature_fixture()
    selected = features.loc[pd.to_datetime(features["date"]) >= pd.Timestamp("2023-10-02")]

    signal = build_b3_relative_tilt_signal(selected, config=config, policy=signal_policy)

    assert not signal.empty
    assert {"relative_strength_score", "signal_score", "state", "confidence"} <= set(
        signal.columns
    )
    assert set(signal["symbol"]) == {"SPY", "QQQ", "SMH"}
...
def test_b3_target_mapping_changes_relative_selection_not_total_exposure() -> None:
    config = load_etf_config_bundle()
    signal_policy, target_policy = load_b3_policies()
    ...
    weights = target_path["target_weights_json"].map(json.loads)
    assert any(abs(row["SPY"] / row["QQQ"] - 0.30 / 0.40) > 1e-6 for row in weights)
    for row in weights:
        assert abs(row["CASH"] - 0.15) < 1e-9
        assert abs(sum(row.values()) - 1.0) < 1e-9


def test_b3_target_path_can_run_naive_execution() -> None:
    config = load_etf_config_bundle()
    signal_policy, target_policy = load_b3_policies()
    prices = _price_fixture(periods=230)
    features = build_feature_store(
