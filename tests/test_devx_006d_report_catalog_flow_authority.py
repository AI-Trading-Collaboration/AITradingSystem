...
def test_repository_authority_is_fresh_lossless_and_inactive() -> None:
    ...
    assert result["entry_count"] == 3177
...
def test_compatibility_authority_carries_the_inactive_shadow_contract() -> None:
    ...
    latest_section = "phase_trading_2564_s3b_prospective_capture_execution_v1"
    assert next(reversed(merged)) == latest_section
    assert (
        list(merged).index(latest_section)
        == list(merged).index("phase_trading_2564_s3a_prospective_event_time_v1") + 1
    )
    ...
    assert successor["entry_count"] == 3177
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "artifact_catalog",
            "docs/artifact_catalog.md",
            "4ddb8a02cb1ed8f77c685b92f36253a5d980fc737dffc8709574c523c3878016",
            571,
        ),
        (
            "system_flow",
            "docs/system_flow.md",
            "dc7c10cc72bbe174454f441d0e310806f8cc4df218b2b4a61923a43864ccc8e8",
            1233,
        ),
    ],
)
