...
def test_repository_authority_is_fresh_lossless_and_inactive() -> None:
    ...
    assert result["entry_count"] == 3205
...
def test_compatibility_authority_carries_the_inactive_shadow_contract() -> None:
    ...
    successor = merged["phase_prod_004_pit_cumulative_archive_consumption_v1"][
        "report_catalog_flow_successor"
    ]

    latest_section = "phase_devx_015_task_integration_admission_v1"
    assert next(reversed(merged)) == latest_section
    assert (
        list(merged).index(latest_section)
        == list(merged).index("phase_ops_081_scheduler_business_contract_decoupling_v1") + 1
    )
    assert (
        list(merged).index("phase_ops_081_scheduler_business_contract_decoupling_v1")
    ...
    assert successor["entry_count"] == 3205
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "system_flow",
            "docs/system_flow.md",
            "3acbe1110b434fc40df9cffb38cd0f48086bd379479f8580fa8561140228fb77",
            1250,
        ),
    ],
)
