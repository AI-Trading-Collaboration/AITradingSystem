...
def test_reporting_inventory_preserves_f3_baseline_and_applies_wave14_ratchet() -> None:
    ...
    assert inventory.report_registry_entry_count == 1369
    assert inventory.explicit_production_effect_count == 680
    ...
    assert inventory.legacy_unclassified_entry_count == 1369
...
def test_actual_report_registry_is_fully_covered_as_limited_unclassified() -> None:
    ...
    assert len(assessment.entries) == 1369
    ...
    assert assessment.to_dict()["limited_count"] == 1369
...
def test_report_audit_index_preserves_every_legacy_registry_entry(tmp_path: Path) -> None:
    ...
    assert payload["entry_count"] == 1369
    assert payload["typed_count"] == 0
    assert payload["limited_count"] == 1369
    assert len(payload["entries"]) == 1369
    assert len({item["report_id"] for item in payload["entries"]}) == 1369
