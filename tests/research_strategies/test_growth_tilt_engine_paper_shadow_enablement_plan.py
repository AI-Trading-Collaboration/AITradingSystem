...
from ai_trading_system import (
    dynamic_strategy_growth_tilt_engine_paper_shadow_enablement_plan as impl,
)
...
def test_paper_shadow_enablement_plan_builder_ready() -> None:
    payload = _build_payload(_source_documents())

    assert payload["status"] == enablement.READY_STATUS
    ...
    assert payload["enablement_gap_count"] == 0
    ...
    assert payload["recommended_next_research_task"] == enablement.NEXT_ROUTE_READY


def test_paper_shadow_enablement_plan_blocks_preflight_not_ready() -> None:
    sources = _source_documents()
    sources["preflight_result_2423"]["paper_shadow_preflight_ready"] = False

    payload = _build_payload(sources)

    assert payload["status"] == enablement.BLOCKED_STATUS
    assert payload["enablement_plan_ready"] is False
    assert "paper_shadow_preflight_ready" in payload["enablement_gap_ids"]
    assert payload["recommended_next_research_task"] == enablement.NEXT_ROUTE_BLOCKED


def test_paper_shadow_enablement_plan_blocks_contract_not_ready() -> None:
    sources = _source_documents()
    ...
    assert payload["status"] == enablement.BLOCKED_STATUS
    assert "contract_ready" in payload["enablement_gap_ids"]
    assert payload["paper_shadow_enabled"] is False


def test_paper_shadow_enablement_plan_blocks_pit_gate_not_ready() -> None:
    sources = _source_documents()
    ...
    assert payload["status"] == enablement.BLOCKED_STATUS
    assert "pit_gate_ready" in payload["enablement_gap_ids"]


def test_paper_shadow_enablement_plan_blocks_paper_shadow_enabled() -> None:
    sources = _source_documents()
    sources["preflight_result_2423"]["paper_shadow_enabled"] = True

    payload = _build_payload(sources)

    assert payload["status"] == enablement.BLOCKED_STATUS
    assert "paper_shadow_runtime_disabled" in payload["enablement_gap_ids"]
    assert payload["safety_boundary_gap_count"] > 0


def test_paper_shadow_enablement_plan_blocks_schedule_enabled() -> None:
    sources = _source_documents()
    sources["preflight_result_2423"]["paper_shadow_schedule_enabled"] = True

    payload = _build_payload(sources)

    assert payload["status"] == enablement.BLOCKED_STATUS
    assert "paper_shadow_schedule_disabled" in payload["enablement_gap_ids"]


def test_paper_shadow_enablement_plan_blocks_production_or_broker_enabled() -> None:
    sources = _source_documents()
    sources["preflight_result_2423"]["production_enabled"] = True
    sources["preflight_result_2423"]["broker_enabled"] = True

    payload = _build_payload(sources)

    assert payload["status"] == enablement.BLOCKED_STATUS
    assert "production_disabled" in payload["enablement_gap_ids"]
    assert "broker_disabled" in payload["enablement_gap_ids"]


def test_paper_shadow_enablement_plan_ready_does_not_emit_trading_outputs() -> None:
    payload = _build_payload(_source_documents())
    ...
    assert payload["paper_shadow_enablement_plan"]["allowed_future_actions"] == list(
        enablement.ALLOWED_FUTURE_ACTIONS
    )
    assert payload["paper_shadow_enablement_plan"]["forbidden_actions"] == list(
        enablement.FORBIDDEN_ACTIONS
    )


def test_paper_shadow_enablement_plan_blocks_missing_registration() -> None:
    sources = _source_documents()
    ...
    payload = enablement.build_growth_tilt_engine_paper_shadow_enablement_plan(
        ...
        system_flow_text="",
        research_doc_texts={"source_2423_research_doc": ""},
    )

    assert payload["status"] == enablement.BLOCKED_STATUS
    assert "report_registry_registered" in payload["enablement_gap_ids"]
    assert "artifact_catalog_registered" in payload["enablement_gap_ids"]
    assert "system_flow_registered" in payload["enablement_gap_ids"]
    assert "research_docs_registered" in payload["enablement_gap_ids"]


def test_paper_shadow_enablement_plan_wrapper_writes_outputs(tmp_path: Path) -> None:
    paths = _write_sources(tmp_path)
    output_root = tmp_path / "outputs" / "enablement"
    docs_root = tmp_path / "docs" / "research"

    payload = impl.run_growth_tilt_engine_paper_shadow_enablement_plan(
    ...
    for key in (
        "json_path",
        "enablement_plan_json",
        "runtime_boundary_checklist_json",
        "schedule_boundary_plan_json",
        "manual_review_checklist_json",
        "rollback_stop_condition_summary_json",
        "markdown_path",
        "runtime_boundary_checklist_markdown",
        "schedule_boundary_plan_markdown",
        "manual_review_checklist_markdown",
        "rollback_stop_condition_summary_markdown",
        "next_route_markdown",
    ):
        assert Path(payload["artifact_paths"][key]).exists()


def test_paper_shadow_enablement_plan_cli_deterministic(tmp_path: Path) -> None:
    paths = _write_sources(tmp_path)
    output_root = tmp_path / "outputs" / "enablement_cli"
    docs_root = tmp_path / "docs" / "research"
    runner = CliRunner()
    ...
    assert enablement.READY_STATUS in result.output
    ...
    assert "enablement_gap_count=0" in result.output
    ...
    assert "automatic_execution_allowed=false" in result.output
    assert f"next_route={enablement.NEXT_ROUTE_READY}" in result.output
    assert (output_root / "enablement_plan_result.json").exists()


def test_paper_shadow_enablement_plan_missing_prior_artifact_blocks(
    tmp_path: Path,
) -> None:
    paths = _write_sources(tmp_path)
    paths["preflight_result_2423"].unlink()
    ...
    assert payload["paper_shadow_enablement_plan_started"] is False
    assert payload["enablement_plan_ready"] is False
    assert payload["enablement_gap_count"] == 1
    assert payload["missing_enablement_evidence_count"] == 1
    assert payload["paper_shadow_enabled"] is False
    assert payload["recommended_next_research_task"] == enablement.NEXT_ROUTE_BLOCKED


def test_paper_shadow_enablement_plan_registry_catalog_system_flow() -> None:
    registry = load_report_registry(DEFAULT_REPORT_REGISTRY_PATH)
    entries = {item["report_id"]: item for item in registry["reports"]}
    entry = entries[enablement.REPORT_TYPE]
    ...
    assert any(
        "enablement_plan_result.json" in item for item in entry["artifact_globs"]
    )
    assert any("dynamic_strategy_2425_route.md" in item for item in entry["artifact_globs"])

    catalog = Path("docs/artifact_catalog.md").read_text(encoding="utf-8")
    system_flow = Path("docs/system_flow.md").read_text(encoding="utf-8")
    task_register = Path("docs/task_register.md").read_text(encoding="utf-8")
    assert enablement.REPORT_TYPE in catalog
    assert "growth-tilt-engine-paper-shadow-enablement-plan" in system_flow
    assert enablement.READY_STATUS in system_flow
    assert enablement.NEXT_ROUTE_READY in system_flow
    assert impl.TASK_REGISTER_ID in task_register


def _build_payload(sources: dict[str, dict]) -> dict[str, object]:
    return enablement.build_growth_tilt_engine_paper_shadow_enablement_plan(
        sources["preflight_result_2423"],
        sources["contract_readiness_snapshot_2422"],
        sources["readiness_recheck_2421"],
        sources["remediation_result_2420"],
...
def _write_sources(tmp_path: Path) -> dict[str, Path]:
    ...
    paths = {
        ...
        "preflight_checklist_2423": root / "preflight_checklist_2423.json",
        "preflight_gap_summary_2423": root / "preflight_gap_summary_2423.json",
        "research_doc_2423": root / "research_doc_2423.md",
        "checklist_doc_2423": root / "checklist_doc_2423.md",
        "gap_summary_doc_2423": root / "gap_summary_doc_2423.md",
...
def _source_kwargs(paths: dict[str, Path]) -> dict[str, Path]:
    return {
        ...
        "source_2423_preflight_checklist_path": paths["preflight_checklist_2423"],
        "source_2423_preflight_gap_summary_path": (
            paths["preflight_gap_summary_2423"]
        ),
        "source_2423_research_doc_path": paths["research_doc_2423"],
        "source_2423_checklist_doc_path": paths["checklist_doc_2423"],
        "source_2423_gap_summary_doc_path": paths["gap_summary_doc_2423"],
...
def _source_args(paths: dict[str, Path]) -> list[str]:
    ...
    for option, key in (
        ...
        ("--source-2423-preflight-checklist", "preflight_checklist_2423"),
        ("--source-2423-preflight-gap-summary", "preflight_gap_summary_2423"),
        ("--source-2423-research-doc", "research_doc_2423"),
        ("--source-2423-checklist-doc", "checklist_doc_2423"),
        ("--source-2423-gap-summary-doc", "gap_summary_doc_2423"),
...
def _source_documents() -> dict[str, dict]:
    return {
        ...
        "preflight_checklist_2423": {"preflight_checklist": {"checks": []}},
        "preflight_gap_summary_2423": {"preflight_gap_summary": {"gaps": []}},
...
def _preflight_result_2423() -> dict[str, object]:
    return {
        "status": enablement.PREFLIGHT_READY_STATUS,
        "readiness_status": enablement.PREFLIGHT_READY_STATUS,
        "pit_gate_ready": True,
        ...
        "recommended_next_research_task": enablement.EXPECTED_PRIOR_NEXT_ROUTE,
    }


def _contract_readiness_snapshot_2422() -> dict[str, object]:
    return {
        "status": enablement.CONTRACT_READINESS_SNAPSHOT_READY_STATUS,
        "pit_gate_ready": True,
        "pit_gate_ready_count": 1,
        "remaining_blockers": [],
...
def _readiness_recheck_2421() -> dict[str, object]:
    return {
        "status": enablement.PIT_GATE_AFTER_SOURCE_TRACEABILITY_READY_STATUS,
        "source_traceability_recheck_status": "ACCEPTED",
        "remaining_blockers": [],
        "remaining_blocker_count": 0,
        "pit_gate_ready": True,
...
def _remediation_result_2420() -> dict[str, object]:
    return {
        "status": enablement.SOURCE_TRACEABILITY_REMEDIATION_READY_STATUS,
...
def _research_doc_texts() -> dict[str, str]:
    return {
        "research_doc_2423": enablement.PREFLIGHT_READY_STATUS,
        "checklist_doc_2423": "preflight checklist ready",
        "gap_summary_doc_2423": "preflight gaps []",
        "route_doc_2423": enablement.EXPECTED_PRIOR_NEXT_ROUTE,
        "research_doc_2422": enablement.CONTRACT_READINESS_SNAPSHOT_READY_STATUS,
        "route_doc_2422": "TRADING-2423_Growth_Tilt_Engine_Paper_Shadow_Preflight",
        "research_doc_2421": enablement.PIT_GATE_AFTER_SOURCE_TRACEABILITY_READY_STATUS,
        "route_doc_2421": "TRADING-2422_Growth_Tilt_Engine_Contract_Readiness_Snapshot",
        "research_doc_2420": enablement.SOURCE_TRACEABILITY_REMEDIATION_READY_STATUS,
...
def _report_registry() -> dict[str, list[dict[str, object]]]:
    return {
        "reports": [
            ...
            for report_id in enablement.REQUIRED_REPORT_IDS
        ]
    }
...
def _artifact_catalog_text() -> str:
    return "\n".join(enablement.REQUIRED_CATALOG_REFERENCES)


def _system_flow_text() -> str:
    return "\n".join(enablement.REQUIRED_SYSTEM_FLOW_REFERENCES)


def _command_for_report(report_id: str) -> str:
    return {
        enablement.REPORT_TYPE: (
            "aits research strategies growth-tilt-engine-paper-shadow-enablement-plan"
        ),
