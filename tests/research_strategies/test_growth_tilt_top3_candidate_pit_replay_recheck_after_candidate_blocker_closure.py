...
from ai_trading_system import (
    dynamic_strategy_growth_tilt_top3_candidate_pit_replay_recheck_after_candidate_blocker_closure as impl,  # noqa: E501
)
...
RUN_IMPL = (
    impl.run_growth_tilt_top3_candidate_pit_replay_recheck_after_candidate_blocker_closure
)
BUILD_RECHECK = (
...
def test_after_candidate_blocker_closure_reads_2438f_and_blocks_current_zero_zero_three() -> None:
    payload = _build_payload(_source_documents())

    assert payload["status"] == recheck.BLOCKED_STATUS
    assert payload["source_2438f_blocker_closure_ready"] is True
    ...
    assert payload["candidate_records_recheckable_after_candidate_blocker_closure"] is True
    ...
    assert payload["remaining_candidate_replay_blocker_count"] == 3
...
def test_after_candidate_blocker_closure_blocks_when_2438f_not_ready() -> None:
    sources = _source_documents()
    sources["source_2438f_blocker_closure"]["status"] = (
    ...
    assert payload["status"] == recheck.BLOCKED_STATUS
    assert payload["source_2438f_blocker_closure_ready"] is False
    assert "source_2438f_blocker_closure_ready" in payload["evidence_gap_ids"]
    assert payload["recommended_next_research_task"] == recheck.NEXT_ROUTE_BLOCKED


def test_after_candidate_blocker_closure_blocks_when_handoff_not_ready() -> None:
    sources = _source_documents()
    sources["replayability_handoff_manifest"]["replayability_handoff_manifest"][
...
def test_after_candidate_blocker_closure_blocks_when_record_count_is_not_three() -> None:
...
def test_after_candidate_blocker_closure_zero_zero_three_stays_blocked() -> None:
...
def test_after_candidate_blocker_closure_three_fail_routes_to_no_passing_candidate() -> None:
    payload = _build_payload(
        _source_documents(),
        records=_records("FAIL", "FAIL", "FAIL"),
    )
...
def test_after_candidate_blocker_closure_pass_without_blocked_routes_ready() -> None:
    payload = _build_payload(
        _source_documents(),
        records=_records("PASS", "FAIL", "FAIL"),
    )
...
def test_after_candidate_blocker_closure_blocked_candidate_never_becomes_no_passing() -> None:
    payload = _build_payload(
        _source_documents(),
        records=_records("FAIL", "FAIL", "BLOCKED"),
    )
...
def test_after_candidate_blocker_closure_forward_aging_handoff_contains_only_pass_records() -> None:
    payload = _build_payload(
        _source_documents(),
        records=_records("PASS", "FAIL", "FAIL"),
    )
...
def test_after_candidate_blocker_closure_no_pass_has_no_forward_aging_handoff() -> None:
    payload = _build_payload(
        _source_documents(),
        records=_records("FAIL", "FAIL", "FAIL"),
    )
...
def test_after_candidate_blocker_closure_keeps_safety_boundary_false() -> None:
...
def test_after_candidate_blocker_closure_routes_are_deterministic() -> None:
    blocked = _build_payload(
        _source_documents(),
        records=_records("BLOCKED", "BLOCKED", "BLOCKED"),
    )
    ready = _build_payload(
        _source_documents(),
        records=_records("PASS", "FAIL", "FAIL"),
    )
    no_pass = _build_payload(
        _source_documents(),
        records=_records("FAIL", "FAIL", "FAIL"),
    )
...
def test_after_candidate_blocker_closure_wrapper_writes_outputs(tmp_path: Path) -> None:
    paths = _write_sources(tmp_path)
    output_root = tmp_path / "outputs" / "after_candidate_blocker"
    ...
    for key in (
        ...
        "remaining_candidate_replay_blocker_summary_json",
        ...
        "remaining_candidate_replay_blocker_summary_markdown",
...
def test_after_candidate_blocker_closure_cli_deterministic(tmp_path: Path) -> None:
    paths = _write_sources(tmp_path)
    output_root = tmp_path / "outputs" / "after_candidate_blocker_cli"
    docs_root = tmp_path / "docs" / "research"
    runner = CliRunner()
    ...
    result = runner.invoke(
        ...
        [
            ...
            "strategies",
            (
                "growth-tilt-top3-candidate-pit-replay-recheck-after-candidate-"
                "blocker-closure"
            ),
            *_source_args(paths),
    ...
    assert "source_2438f_blocker_closure_ready=true" in result.output
    ...
    assert "remaining_candidate_replay_blocker_count=3" in result.output
    ...
    assert (output_root / "recheck_after_candidate_blocker_closure_result.json").exists()


def test_after_candidate_blocker_closure_missing_source_artifact_blocks(
    tmp_path: Path,
) -> None:
    paths = _write_sources(tmp_path)
    paths["source_2438f"].unlink()
...
def test_after_candidate_blocker_closure_registry_catalog_system_flow() -> None:
...
def _build_payload(
    ...
    *,
    ...
    source_2438f = _source_2438f_ready(selected_records)
    source_2438f.update(sources["source_2438f_blocker_closure"])
    return BUILD_RECHECK(
        source_2438f,
        sources["replayability_handoff_manifest"],
        {"candidate_replay_output_records": {"records": selected_records}},
        sources["candidate_level_closure_records"],
...
def _source_documents() -> dict[str, Any]:
    records = _records("BLOCKED", "BLOCKED", "BLOCKED")
    return {
        "source_2438f_blocker_closure": _source_2438f_ready(records),
        "replayability_handoff_manifest": {
            "replayability_handoff_manifest": _handoff(records),
        },
        "candidate_replay_output_records": {
            "candidate_replay_output_records": {"records": records},
        },
        "candidate_level_closure_records": {
            "candidate_level_blocker_closure_records": _closure_records(records),
        },
        ...
        "research_doc_texts": {
            "source_2438f_doc": "2438G PASS FAIL BLOCKED forward-aging",
            "replayability_handoff_doc": "2438G PASS FAIL BLOCKED forward-aging",
            "candidate_level_closure_doc": "2438G PASS FAIL BLOCKED forward-aging",
            "candidate_output_records_doc": "2438G PASS FAIL BLOCKED forward-aging",
        },
    }


def _source_2438f_ready(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "status": recheck.EXPECTED_2438F_STATUS,
        "recommended_next_research_task": recheck.EXPECTED_2438F_ROUTE,
...
def _handoff(records: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "status": recheck.EXPECTED_2438F_STATUS,
        "replayability_handoff_ready": True,
        "candidate_replayable_after_closure_count": len(records),
        "handoff_policy": "REPLAYABILITY_ONLY_2438G_DECIDES_PASS_FAIL_BLOCKED",
        "next_route": recheck.EXPECTED_2438F_ROUTE,
        "replayable_candidates": [
            {
                "candidate_id": record["candidate_id"],
                "closure_evidence_ref": (
                    f"TRADING-2438F:replayability_handoff:{record['candidate_id']}"
                ),
...
def _closure_records(records: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "records": [
            {
                ...
                "closure_action_taken": "Close candidate-level blocker for 2438G.",
                "closure_evidence_ref": (
                ...
                "after_state": {
                    ...
                    "replay_status_after_closure": "BLOCKED",
                    "next_recheck_route": recheck.EXPECTED_2438F_ROUTE,
                },
            }
            for record in records
        ],
...
def _status_reason(status: str) -> dict[str, str | None]:
    if status == "PASS":
        return {
            "pass_reason": "Candidate replay passed after candidate blocker closure.",
            "fail_reason": None,
            "blocker_reason": None,
        }
    if status == "FAIL":
...
def _write_sources(tmp_path: Path) -> dict[str, Path]:
    ...
    paths = {
        "source_2438f": root / "blocker_closure_result.json",
        "handoff": root / "replayability_handoff_manifest.json",
        "candidate_records": root / "candidate_replay_output_records.json",
        "candidate_closure_records": root / "candidate_level_closure_records.json",
        "source_2438f_doc": root / "source_2438f.md",
        "handoff_doc": root / "handoff.md",
        "candidate_closure_doc": root / "candidate_closure.md",
    ...
    _write_json(paths["source_2438f"], sources["source_2438f_blocker_closure"])
    _write_json(paths["handoff"], sources["replayability_handoff_manifest"])
    _write_json(paths["candidate_records"], sources["candidate_replay_output_records"])
    _write_json(paths["candidate_closure_records"], sources["candidate_level_closure_records"])
    for key, text in sources["research_doc_texts"].items():
        path_key = {
            "source_2438f_doc": "source_2438f_doc",
            "replayability_handoff_doc": "handoff_doc",
            "candidate_level_closure_doc": "candidate_closure_doc",
...
def _source_kwargs(paths: dict[str, Path]) -> dict[str, Path]:
    return {
        "source_2438f_blocker_closure_path": paths["source_2438f"],
        "replayability_handoff_manifest_path": paths["handoff"],
        "candidate_replay_output_records_path": paths["candidate_records"],
        "candidate_level_closure_records_path": paths["candidate_closure_records"],
        "source_2438f_doc_path": paths["source_2438f_doc"],
        "replayability_handoff_doc_path": paths["handoff_doc"],
        "candidate_level_closure_doc_path": paths["candidate_closure_doc"],
...
def _source_args(paths: dict[str, Path]) -> list[str]:
    return [
        "--source-2438f-blocker-closure",
        str(paths["source_2438f"]),
        "--replayability-handoff-manifest",
        str(paths["handoff"]),
        "--candidate-replay-output-records",
        str(paths["candidate_records"]),
        "--candidate-level-closure-records",
        str(paths["candidate_closure_records"]),
        "--source-2438f-doc",
        str(paths["source_2438f_doc"]),
        "--replayability-handoff-doc",
        str(paths["handoff_doc"]),
        "--candidate-level-closure-doc",
        str(paths["candidate_closure_doc"]),
