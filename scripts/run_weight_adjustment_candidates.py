...
from ai_trading_system.trading_engine.reports.weight_adjustment_candidates import (  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build observe-only weight adjustment candidate reports."
    )
    parser.add_argument("--date", required=True, help="Evaluation date in YYYY-MM-DD format.")
    parser.add_argument(
        "--reports-dir",
        default=str(REPO_ROOT / "outputs" / "reports"),
        help="Directory containing daily decision, paper quality, and shadow impact JSON files.",
    )
    parser.add_argument(
        "--policy-path",
        default=str(DEFAULT_WEIGHT_ADJUSTMENT_CANDIDATE_POLICY_PATH),
        help="Weight adjustment candidate policy YAML path.",
    )
    parser.add_argument("--daily-decision-summary-json", help="Optional daily summary JSON path.")
    parser.add_argument("--paper-signal-quality-json", help="Optional paper quality JSON path.")
    parser.add_argument("--shadow-parameter-impact-json", help="Optional shadow impact JSON path.")
    ...
    print(f"Weight adjustment candidates：{payload['gate_status']}")
